...
- D-107 giới hạn review-27 trong P-19B ở paint weight của `tree`: connector `1.6`, card thường `1.8`, root focal `2.2`; root phải mạnh hơn card thường và connector phải nhẹ hơn card. Toàn bộ geometry/text/centering/spacing ba mode của D-106 phải byte-equivalent ở emitted geometry elements. Archive review-26; chỉ 3 target HTML + 1 preview đổi artwork, 87 non-target HTML giữ artwork sau candidate normalization và 29 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-108 giới hạn review-28 trong complete paint-weight system của `tree`: connector `1.0`, card `1.2`, root `1.6`, badge `0.9`, separator `1.0`, legend thường/root `1.2/1.6`; giữ thứ bậc connector < card < root. Toàn bộ geometry/text/centering/spacing ba mode phải byte-equivalent review-27. Archive review-27; chỉ 3 target HTML + 1 preview đổi artwork, 87 non-target HTML giữ artwork sau candidate normalization và 29 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-109 giới hạn review-29 trong `story-map`: đúng 4 activity/6 backbone step/9 story/3 release slice, mỗi story thuộc đúng một release; đúng 1 labeled MVP cut và 1 high-risk story có dashed boundary + badge + exact-table redundancy; card containment/no-overlap, separator/release band và geometry ba mode phải kiểm được. Archive review-28; chỉ 3 target HTML + 1 preview đổi artwork, 87 non-target HTML giữ artwork sau candidate normalization và 29 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-110 giới hạn review-30 trong `state-machine`: đúng 1 initial marker/4 stable state/1 terminal marker/6 directed transition; 5 normal transition phải straight, return/revise transition duy nhất phải là rounded-orthogonal dashed exception có lý do; đủ 12 endpoint attachment đều gắn geometric midpoint theo D-105, primary/vertical rail căn tâm, label có knockout, đúng 1 live focal state và exact alternative table. Archive review-29; chỉ 3 target HTML + 1 preview đổi artwork, 87 non-target HTML giữ artwork sau candidate normalization và 29 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-111 giới hạn review-31 trong `sequence`: đúng 4 participant/4 lifeline/2 activation/6 ordered message; 5 cross-participant message phải straight horizontal, self-call duy nhất phải rounded-orthogonal và có reason; participant card/activation phải centered trên lifeline, label có knockout, đúng 1 focal origin + 1 primary response và exact alternative table. Archive review-30; chỉ 3 target HTML + 1 preview đổi artwork, 87 non-target HTML giữ artwork sau candidate normalization và 29 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-112 giới hạn review-32 trong paint weight của `treemap`: regular/focal tile `1.2/1.6`, footer rule `1.0`, regular/focal swatch `1.2/1.6`; focal phải mạnh hơn regular, cả sáu tile vẫn có stroke thật và đủ bốn cạnh. Exact D-101/D-103 allocation geometry, inset 4, gap 8, values, labels, legend và table phải giữ nguyên review-31. Archive review-31; chỉ 3 target HTML + 1 preview đổi paint, 87 non-target HTML giữ artwork sau candidate normalization và 29 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-113 giới hạn review-33 trong việc thêm presentation variant `scatter-chart` dưới frozen P-18 `scatter-plot`: đúng 12 cặp dữ liệu, 2 arrow-free axis, x/y tick lần lượt 6/5, 12 hollow point, 1 descending dashed OLS trend, 1 focal Platform point và exact 12-row table; ba mode cùng geometry. Archive review-32; 90 prior HTML giữ artwork sau candidate normalization và 30 prior preview byte-identical; thêm đúng 3 HTML + 1 preview để đạt 93/31 và comparison 107. Không tự duyệt owner/P-19C/G-04.
- D-114 giới hạn review-34 trong `radar`: đúng 5 arrow-free radial axis dùng chung domain 0–10, 5 ring, 4 closed profile, 20 value/marker, 1 focal profile và exact 20-row table; color + dash + marker-shape redundancy và ba mode cùng geometry. Archive review-33; chỉ 3 radar HTML + 1 preview đổi artwork, 90 non-target HTML giữ artwork sau candidate normalization và 30 non-target preview byte-identical; tổng vẫn 93/31 và comparison 107. Không tự duyệt owner/P-19C/G-04.
- D-115 giới hạn review-35 trong radar line treatment: cả 4 profile polygon và 4 legend sample phải dùng solid continuous stroke, không profile CSS/SVG nào được có `stroke-dasharray`; giữ non-color redundancy bằng circle/square/triangle/diamond, direct legend và focal role. Giữ nguyên 5 axis/5 ring/4 profile/20 value/table/geometry từ D-114. Archive review-34; chỉ 3 radar HTML + 1 preview đổi artwork, 90 non-target HTML giữ artwork sau candidate normalization và 30 non-target preview byte-identical; tổng vẫn 93/31 và comparison 107. Không tự duyệt owner/P-19C/G-04.
- D-116 giới hạn review-36 trong việc bỏ marker khỏi vùng radar: plot-marker count phải bằng 0 và standalone center dot phải bị loại bỏ trong cả ba mode; giữ 4 solid profile, 5 axis/5 ring/20 value, legend ngoài plot và exact table. Archive review-35; chỉ 3 radar HTML + 1 preview đổi artwork, 90 non-target HTML giữ artwork sau candidate normalization và 30 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-117 giới hạn review-37 trong việc thay riêng `process`: đúng 3 terminator, 4 process, 2 decision, 1 document, 1 multiple-document và 11 directed straight route; single attachment centered, hai inlet cùng cạnh chia đều quanh tâm, exact node/edge table và geometry ba mode. Archive review-36; chỉ 3 process HTML + 1 preview đổi artwork, 90 non-target HTML giữ artwork sau candidate normalization và 30 non-target preview byte-identical. Template P-18-derived và mọi scope trước phải giữ nguyên; không tự duyệt owner/P-19C/G-04.
- D-118 giới hạn review-38 trong connector continuity của `process`: 5 connector document-bound phải chạm visible boundary; đúng 2 symmetric rounded-orthogonal merge route có `Q` corner, exception reason và dedicated arrowhead; 9 route còn lại thẳng. Giữ exact node/shape/content/template/three-mode geometry và inlet x=920/1080. Archive review-37; chỉ 3 process HTML + 1 preview đổi artwork, 90 non-target HTML giữ artwork sau candidate normalization và 30 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-119 giới hạn review-39 trong paint weight của `process`: route thường `1.0`, node thường/merge `1.2`, focal `1.6`, layer `1.0/1.2`, badge/rule `0.8`; giữ hierarchy connector < node < focal và đầu mũi tên rõ. D-118 geometry/contact/routing/content/template phải byte-equivalent. Archive review-38; chỉ 3 process HTML + 1 preview đổi paint, 90 non-target HTML giữ artwork sau candidate normalization và 30 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-120 giới hạn review-40 trong capability Bubble: identity hiển thị phải là `bubble`, internal ID vẫn `CAP-V20`, parent vẫn `scatter-plot`; đúng 7 bubble/3 series/2 plain axis/8 x-tick/9 y-tick/1 focal và exact seven-row table. `radius²/value` phải constant để area mang magnitude; focal có coral + direct label + table role. Template P-18-derived không đổi. Archive review-39; chỉ 3 Bubble HTML + 1 preview đổi artwork/identity, 90 non-target HTML giữ artwork sau candidate normalization và 30 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-121 giới hạn review-41 trong capability slope-graph: identity hiển thị phải là `slope-graph`, internal ID vẫn `CAP-V18`, parent vẫn `line-chart`; đúng 7 series/14 endpoint/2 plain state axis/6 tick/5 rise/2 fall/9 crossing/1 focal và exact seven-row table trên cùng thang 0–100%. Direct endpoint label/value phải đủ hai đầu; focal có coral + endpoint + direct label + table role. Cả 7 line phải là nét liền, không ẩn/đứt và còn phân biệt được ở tỷ lệ comparison; line thường `3.4`, focal `4.5`, axis/grid vẫn mảnh. Mọi stroke phải dùng token đã khai báo trong template; gate phải FAIL nếu còn `--blue`/`--green`/`--amber`/`--plum` không tồn tại. Template P-18-derived không đổi. Archive review-40; chỉ 3 slopegraph HTML cũ được thay bằng 3 slope-graph HTML + 1 preview, 90 non-target HTML giữ artwork sau candidate normalization và 30 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-122 giới hạn review-42 trong capability dumbbell: identity hiển thị phải là `dumbbell`, internal ID vẫn `CAP-V17`, parent vẫn `bar-chart`; đúng 12 pair/24 endpoint/2 series/12 direct delta/2 statistics band/2 mean line/1 plain shared axis/6 tick/1 focal và exact 12-row table trên cùng thang 0–100%. Mỗi pair phải là một straight horizontal connector chạm đúng tâm hai endpoint; trước/sau có direct value và table redundancy. Statistics phải dùng population mean/stdev từ chính 12 giá trị; focal có coral text/link/table role. Template P-18-derived không đổi và không nhập purple/teal từ ảnh tham chiếu. Archive review-41; chỉ 3 dumbbell HTML + 1 preview đổi artwork/identity, 90 non-target HTML giữ artwork sau candidate normalization và 30 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-123 giới hạn review-43 trong capability ridgeline: identity hiển thị phải là `ridgeline`, internal ID vẫn `CAP-V19`, parent vẫn `line-chart`; đúng 12 density silhouette/12 median dot/36 nested quantile band (50/80/95%)/1 shared-median reference/1 plain shared axis/7 tick/1 focal và exact 12-row table trên cùng domain 0–120 ms. Mọi series phải dùng cùng Gaussian-KDE grid 20 điểm, bandwidth 7 ms và global-max amplitude normalization. Template P-18-derived không đổi; chỉ coral/navy/green semantic role đã có được dùng. Archive review-42; chỉ 3 ridgeline HTML + 1 preview đổi artwork/identity, 90 non-target HTML giữ artwork sau candidate normalization và 30 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-124 giới hạn review-44 trong canonical `layer-stack`: đúng 5 tầng L5→L1, 23 module phân phối `4/5/6/4/4`, L3 có đúng hai miền con 3+3, một focal layer, một focal module, một abstraction axis, 4 mũi tên thẳng căn giữa giữa các tầng kề và exact 23-row table. Ba mode phải cùng geometry; viền dùng hierarchy `1.0/1.2/1.6`; template P-18-derived và presentation variant `layers` không đổi. Archive review-43; chỉ 3 `type-layer-stack` HTML + 1 preview đổi artwork, 90 non-target HTML giữ artwork sau candidate normalization và 30 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-125 giới hạn review-45 trong canonical `nested`: đúng 5 scope/5 artifact, depth `0…4`, một parent chain liên tục, inset đều `65×75`, một focal scope, bốn ancestor file glyph và một annotation leader không arrowhead; exact 5-row table và ba-mode geometry phải khớp. Viền thường/focal dùng `1.0/1.6`; template P-18-derived không đổi. Archive review-44; chỉ 3 `type-nested` HTML + 1 preview đổi artwork, 90 non-target HTML giữ artwork sau candidate normalization và 30 non-target preview byte-identical. Không tự duyệt owner/P-19C/G-04.
- D-126 ghi nhận owner approval cho exact P-19B review-45 và đặt coexistence thành hard assertion: 14 exact P-18 anchor theo manifest review-17 và 93 P-19 HTML/31 preview phải cùng tồn tại, combined view phải đúng 107 diagram, source links phải phân biệt P-18/P-19 và không P-19 artifact nào được thay thế hoặc ghi đè P-18. D-126 đóng P-19B nhưng không tự mở P-19C hoặc đánh giá G-04.
- D-127 authorize riêng P-19C full QA/freeze/masked review. P-19C phải chứng minh exact byte preservation của 14 P-18 anchor và 93 P-19 HTML/31 preview, combined comparison đúng 107, không substitution; chạy các hard check G-04 trong môi trường khả dụng, freeze exact source/gallery candidate bằng manifest/hash và chuẩn bị owner/gate review. D-127 không tự phê duyệt candidate hoặc `G-04@1.5.0 PASS`, không cho phép artwork/package/`dist`/publication/Git/Release mutation.
- D-128 owner-approve exact P-19C review-01 với điều kiện 31 masked silhouette chỉ là mẫu QA, không phải catalog/template/output cố định; explicit user request phải được ưu tiên và implementation phải thích ứng linh động trong giới hạn an toàn/ngữ nghĩa. D-128 đóng P-19C/P-19 nhưng không tự làm `G-04@1.5.0 PASS` và không authorize package/`dist`/publication/Git/Release.
- D-129 phê duyệt `G-04@1.5.0 PASS` cho exact source/gallery candidate sau khi owner xác nhận technical/QA evidence đầy đủ; D-128 sample-not-fixed/user-request flexibility tiếp tục là điều kiện gate. G-04 closure không tự authorize hoặc pass G-05/G-06/G-07, package/`dist`/publication/Git/Release.
- D-130 mở riêng P-20 và đổi target release thành `2.0.0`. P-20 được rebind exact v1.5.0 source/gallery lineage, cập nhật package-facing guidance, tạo v2 legal/provenance/brand carry-forward candidate, xác minh official platform surface, build ba deterministic candidate ZIP trong `evidence/p20/candidate-dist/`, chạy QA và chuẩn bị gate packet. Mọi gate `@2.0.0` là version-scoped và khởi đầu `NOT-EVALUATED`; D-130 không tự pass gate, không cho phép sửa `dist/` v1.0.0, publication/Git/Release, và không được đổi frozen P-18/P-19 hoặc D-128 sample-not-fixed flexibility.
- D-131 phê duyệt toàn bộ `G-00…G-07@2.0.0` cho exact `TCD-RELEASE-2.0.0-RC1`, manifest SHA-256 `2905d4d3945a75ba9b644aece005bcb6de5bb2278ca8f7e47a4247189c77be72`. Riêng exact `G-06@2.0.0`, owner miễn independent Vietnamese-lawyer review và tự chấp nhận rủi ro; kết quả phải ghi `PASS` với owner waiver, tuyệt đối không mô tả là lawyer-reviewed. `G-07@2.0.0 PASS` chỉ phê duyệt exact release candidate; copy/promote vào `dist`, publication, commit, push, tag và tạo/thay Release vẫn bị hold cho đến lệnh thực thi riêng, rõ ràng.
- D-132 gỡ execution hold riêng cho exact private release `v2.0.0` theo chuỗi preflight → exact promotion → sanitized mirror → QA → commit/tag/push → GitHub Release → remote digest verification. Authorization không cho phép đổi approved bytes, target/visibility, v1.0.0 history hoặc bypass preflight; failure phải dừng trước bước mutation tiếp theo.
- D-133 ghi nhận P-21/G-07@2.0.0 execution `PASS`: private non-draft/non-prerelease Release `v2.0.0`, annotated tag object `6dfd116c…abe` peel đúng release commit `7f6165ff…95f`, bốn remote asset digest khớp exact candidate và historical v1.0.0 được bảo toàn. Closure không cấp quyền maintenance mutation hoặc release kế tiếp.
...
Ngoại lệ version-scoped: D-131 cho phép owner miễn independent lawyer review đối với đúng `TCD-LEGAL-2.0.0-RC1`/aggregate `93643da0…f29c0`, tự chấp nhận rủi ro và phê duyệt `G-06@2.0.0 PASS`. Ngoại lệ này không được mô tả là lawyer sign-off và không tự áp dụng cho version/hash khác.
...
- owner approval của goldens/benchmark/brand và lawyer sign-off của legal bytes vẫn khớp artifact đã đóng gói, hoặc có exact owner waiver version/hash-specific được ledger cho phép như D-131;
- chủ sở hữu duyệt ba ZIP và toàn bộ release candidate, rồi cấp release authorization riêng;
- nếu legal byte thay đổi sau lawyer sign-off/owner waiver hoặc brand byte thay đổi sau owner approval, G-06 trở lại `FAIL` cho đến khi duyệt lại;
- trước khi thực thi commit/tag/push/release, người dùng phải ra lệnh rõ ràng trong yêu cầu đang hoạt động; gate có thể `PASS` với execution hold nếu exact release candidate đã được owner phê duyệt nhưng chưa có lệnh mutation;
...
Người duyệt: chủ sở hữu; luật sư cho phần pháp lý, trừ exact owner waiver version/hash-specific được decision ledger khóa rõ.
