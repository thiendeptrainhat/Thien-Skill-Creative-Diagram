...
from bubble_layout_v15 import (
    bubble_css, bubble_table, is_detailed_bubble, layout_bubble,
    render_bubble, validate_bubble_svg,
)
...
from layer_stack_layout_v15 import (
    is_detailed_layer_stack, layer_stack_css, layer_stack_table,
    layout_layer_stack, render_layer_stack, validate_layer_stack_svg,
)
from nested_layout_v15 import (
    is_detailed_nested, layout_nested, nested_css, nested_table,
    render_nested, validate_nested_svg,
)
...
from process_layout_v15 import (
    is_detailed_process, layout_process, process_css, process_table,
    render_process, validate_process_svg,
)
...
P19B_CANDIDATE_ID = "P19B-P18-INHERITED-THREE-MODE-REVIEW-45-1.5.0"
...
def validate_target_geometry(svg: str, diagram_type: str) -> dict[str, Any]:
    ...
    if diagram_type not in ("dp-integration", "swimlane", "sequence", "bar-chart", "line-chart", "scatter-plot", "polar-chart", "wardley-map", "venn", "dp-security-matrix", "er-data-model", "high-level", "it-current-state", "kanban", "story-map", "state-machine", "nested", "layer-stack", "medallion", "tree"):
        return {}
    ...
    if diagram_type == "layer-stack":
        if root.find(".//*[@data-layer-stack-contract]") is not None:
            try:
                return validate_layer_stack_svg(svg)
            except ValueError as error:
                raise GalleryRenderError("layer-stack-invalid", str(error)) from error
    ...
    if diagram_type == "nested":
        if root.find(".//*[@data-nested-contract]") is None:
            return {}
        try:
            return validate_nested_svg(svg)
        except ValueError as error:
            raise GalleryRenderError("nested-invalid", str(error)) from error
    ...
    if diagram_type == "bar-chart":
        if root.find(".//*[@data-dumbbell-contract]") is not None:
            try:
                return validate_dumbbell_svg(svg)
            except ValueError as error:
                raise GalleryRenderError("dumbbell-invalid", str(error)) from error
    ...
    if diagram_type == "line-chart":
        if root.find(".//*[@data-ridgeline-contract]") is not None:
            try:
                return validate_ridgeline_svg(svg)
            except ValueError as error:
                raise GalleryRenderError("ridgeline-invalid", str(error)) from error
        if root.find(".//*[@data-slope-graph-contract]") is not None:
            try:
                return validate_slope_graph_svg(svg)
            except ValueError as error:
                raise GalleryRenderError("slope-graph-invalid", str(error)) from error
    ...
    if diagram_type == "scatter-plot":
        if root.find(".//*[@data-bubble-contract]") is not None:
            try:
                return validate_bubble_svg(svg)
            except ValueError as error:
                raise GalleryRenderError("bubble-invalid", str(error)) from error
    ...
    if diagram_type == "process":
        if root.find(".//*[@data-process-contract]") is None:
            return {}
        try:
            return validate_process_svg(svg)
        except ValueError as error:
            raise GalleryRenderError("process-invalid", str(error)) from error
...
def _directed(plan: Mapping[str, Any]) -> str:
    ...
    if kind == "process" and is_detailed_process(plan):
        return render_process(plan)
...
def _containment(plan: Mapping[str, Any]) -> str:
    kind = plan["adapter"]["canonical_type"]
    if kind == "nested" and is_detailed_nested(plan):
        return render_nested(plan)
    if kind == "layer-stack" and is_detailed_layer_stack(plan):
        return render_layer_stack(plan)
...
def _quantitative(plan: Mapping[str, Any]) -> str:
    ...
    if adapter_id == "capability:CAP-V17":
        if is_detailed_dumbbell(plan):
            return render_dumbbell(plan)
    ...
    if adapter_id == "capability:CAP-V18":
        if is_detailed_slope_graph(plan):
            return render_slope_graph(plan)
    ...
    if adapter_id == "capability:CAP-V19":
        if is_detailed_ridgeline(plan):
            return render_ridgeline(plan)
        ridges=[]
    ...
    if adapter_id == "capability:CAP-V20":
        if is_detailed_bubble(plan):
            return render_bubble(plan)
...
def render_gallery_html(ir_value: Mapping[str, Any], mode: str, fixture_id: str, *, connector_corner_style: str | None = None) -> str:
    ...
    identity = "layers" if is_layers_variant else "scatter-chart" if is_scatter_variant else "dumbbell" if adapter["capability_id"] == "CAP-V17" else "slope-graph" if adapter["capability_id"] == "CAP-V18" else "ridgeline" if adapter["capability_id"] == "CAP-V19" else "bubble" if adapter["capability_id"] == "CAP-V20" else adapter["capability_id"] or adapter["canonical_type"]
    ...
    detailed_types = ("gantt", "loop-flywheel", "dp-integration", "sequence", "bar-chart", "line-chart", "scatter-plot", "radar", "polar-chart", "treemap", "wardley-map", "venn", "dp-security-matrix", "er-data-model", "uml-class", "high-level", "it-current-state", "kanban", "story-map", "state-machine", "process", "nested", "layer-stack", "medallion", "tree")
    detailed_ready = (
        ...
        and (adapter["canonical_type"] != "bar-chart" or is_detailed_bar_chart(plan) or is_detailed_dumbbell(plan))
        and (adapter["canonical_type"] != "line-chart" or is_detailed_line_chart(plan) or is_detailed_slope_graph(plan) or is_detailed_ridgeline(plan))
        and (adapter["canonical_type"] != "scatter-plot" or is_detailed_scatter_chart(plan) or is_detailed_bubble(plan))
        ...
        and (adapter["canonical_type"] != "process" or is_detailed_process(plan))
        and (adapter["canonical_type"] != "nested" or is_detailed_nested(plan))
        and (adapter["canonical_type"] != "layer-stack" or is_detailed_layer_stack(plan) or is_detailed_layers(plan))
    ...
    if adapter["canonical_type"] == "bar-chart" and is_detailed_dumbbell(plan):
        dumbbell_layout = layout_dumbbell(plan)
        canvas_width, canvas_height = dumbbell_layout["width"], dumbbell_layout["height"]
        signature = ""  # Detailed dumbbell owns its shared scale, pairs, statistics and legend.
    ...
    if adapter["canonical_type"] == "line-chart" and is_detailed_slope_graph(plan):
        slope_layout = layout_slope_graph(plan)
        canvas_width, canvas_height = slope_layout["width"], slope_layout["height"]
        signature = ""  # Detailed slope graph owns its state axes, direct endpoint labels and reading note.
    if adapter["canonical_type"] == "line-chart" and is_detailed_ridgeline(plan):
        ridge_layout = layout_ridgeline(plan)
        canvas_width, canvas_height = ridge_layout["width"], ridge_layout["height"]
        signature = ""  # Detailed ridgeline owns its shared domain, density profiles, quantile bands and legend.
    ...
    if adapter["canonical_type"] == "scatter-plot" and is_detailed_bubble(plan):
        bubble_layout = layout_bubble(plan)
        canvas_width, canvas_height = bubble_layout["width"], bubble_layout["height"]
        signature = ""  # Detailed Bubble owns its three axes, area encoding, direct focal label and legend.
    ...
    if adapter["canonical_type"] == "process" and is_detailed_process(plan):
        process_layout = layout_process(plan)
        canvas_width, canvas_height = process_layout["width"], process_layout["height"]
        signature = ""  # Detailed process owns its shape taxonomy, routes and legend.
    if adapter["canonical_type"] == "nested" and is_detailed_nested(plan):
        nested_layout = layout_nested(plan)
        canvas_width, canvas_height = nested_layout["width"], nested_layout["height"]
        signature = ""  # Detailed nested owns its containment rings, artifacts and inheritance note.
    ...
    if adapter["canonical_type"] == "layer-stack" and is_detailed_layer_stack(plan):
        stack_layout = layout_layer_stack(plan)
        canvas_width, canvas_height = stack_layout["width"], stack_layout["height"]
        signature = ""  # Detailed layer-stack owns its five bands, modular domains and dependency legend.
    ...
    if adapter["canonical_type"] == "bar-chart" and is_detailed_dumbbell(plan):
        style = dumbbell_css(tokens) + style
    if adapter["canonical_type"] == "line-chart" and is_detailed_line_chart(plan):
        style = line_chart_css(tokens) + style
    if adapter["canonical_type"] == "line-chart" and is_detailed_slope_graph(plan):
        style = slope_graph_css(tokens) + style
    if adapter["canonical_type"] == "line-chart" and is_detailed_ridgeline(plan):
        style = ridgeline_css(tokens) + style
    if adapter["canonical_type"] == "scatter-plot" and is_detailed_scatter_chart(plan):
        style = scatter_chart_css(tokens) + style
    if adapter["canonical_type"] == "scatter-plot" and is_detailed_bubble(plan):
        style = bubble_css(tokens) + style
    if adapter["canonical_type"] == "radar" and is_detailed_radar(plan):
    ...
    if adapter["canonical_type"] == "process" and is_detailed_process(plan):
        style = process_css(tokens) + style
    if adapter["canonical_type"] == "nested" and is_detailed_nested(plan):
        style = nested_css(tokens) + style
    if is_layers_variant:
        style = layers_css(tokens) + style
    if adapter["canonical_type"] == "layer-stack" and is_detailed_layer_stack(plan):
        style = layer_stack_css(tokens) + style
    if adapter["canonical_type"] == "medallion" and is_detailed_medallion(plan):
    ...
    if adapter["canonical_type"] == "bar-chart" and is_detailed_dumbbell(plan):
        alternative = dumbbell_table(plan)
    if adapter["canonical_type"] == "line-chart" and is_detailed_line_chart(plan):
        alternative = line_chart_table(plan)
    if adapter["canonical_type"] == "line-chart" and is_detailed_slope_graph(plan):
        alternative = slope_graph_table(plan)
    if adapter["canonical_type"] == "line-chart" and is_detailed_ridgeline(plan):
        alternative = ridgeline_table(plan)
    if adapter["canonical_type"] == "scatter-plot" and is_detailed_scatter_chart(plan):
        alternative = scatter_chart_table(plan)
    if adapter["canonical_type"] == "scatter-plot" and is_detailed_bubble(plan):
        alternative = bubble_table(plan)
    if adapter["canonical_type"] == "radar" and is_detailed_radar(plan):
    ...
    if adapter["canonical_type"] == "process" and is_detailed_process(plan):
        alternative = process_table(plan)
    if adapter["canonical_type"] == "nested" and is_detailed_nested(plan):
        alternative = nested_table(plan)
    if is_layers_variant:
        alternative = layers_table(plan)
    if adapter["canonical_type"] == "layer-stack" and is_detailed_layer_stack(plan):
        alternative = layer_stack_table(plan)
    if adapter["canonical_type"] == "medallion" and is_detailed_medallion(plan):
...
def renderer_inventory() -> dict[str, Any]:
    ...
    return {
        ...
        "viewbox_overrides": {"gantt": "content-fit calendar; width >=1600; height from phase and row content", "loop-flywheel": "content-fit circle; width >=1600; card and shared-state clearance", "fishbone": "content-fit 5-category cause spine; exact category and label clearance", "dp-integration": "content-fit sources/platform/consumers/shared-services topology; 1800×1040", "sequence": "content-fit four-participant interaction; 2000×1140; two activations, five straight messages and one rounded self-call", "process": "content-fit five-shape process; 2000×1340; eleven straight centered/even routes", "bar-chart": "content-fit eight-category zero-baseline comparison; 1800×940", "dumbbell": "content-fit twelve-category paired comparison; 2000×1160; shared 0–100% scale, direct gaps and mean ± stdev bands", "line-chart": "content-fit three-series eight-week trend; 2000×980; 24 exact points", "slope-graph": "content-fit seven-series two-state comparison; 2000×980; direct labels, rank and crossing", "ridgeline": "content-fit twelve-service density stack; 2000×1180; shared 0–120 ms domain, global-max amplitude and 50/80/95% intervals", "scatter-chart": "content-fit twelve-team deployment-performance scatter; 2000×1020; exact linear axes and OLS trend", "bubble": "content-fit seven-product quantitative Bubble; 2000×1040; exact x/y axes and area-faithful magnitude", "polar-chart": "content-fit eight-window radial spokes; 2000×1020; exact 0–100 percent scale", "treemap": "content-fit six-continent exact-area tiling; 2000×1040; one focal and one compact-label tile", "wardley-map": "content-fit eight-component visibility/evolution map; 2000×980", "venn": "content-fit three equal sets; 2000×1040; exact nested-clipped triple intersection", "dp-security-matrix": "content-fit five-role by five-component permission matrix; 2000×820", "er-data-model": "content-fit four-entity aggregate/join model; 2000×940", "high-level": "content-fit end-to-end data-platform overview; 2000×1040; rounded 90-degree corners by default", "it-current-state": "content-fit collection/processing/dissemination landscape; 2000×1040; rounded 90-degree corners by default", "kanban": "content-fit four-column WIP board; 2000×900; explicit blocked/waiting/done states", "story-map": "content-fit four-activity story map; 2000×1040; three release slices, one MVP cut and one high-risk story", "state-machine": "content-fit four-state lifecycle; 2000×980; five straight centered transitions and one rounded return route", "nested": "content-fit five-depth enterprise containment; 2000×1080; five artifacts, focal core and inheritance leader", "layer-stack": "content-fit five-layer modular AI platform; 2000×1250; 23 modules, split model/knowledge tier and four centered dependencies", "layers": "content-fit five-band abstraction stack; 2000×920; one non-color focal layer", "medallion": "content-fit five-stage lifecycle; 2000×1020; four directed promotions and two processing paths", "tree": "content-fit three-tier centered-span hierarchy; 2000×920; P-18 org-chart fanout grammar"},
