...
PINS = {
    ...
    "evidence/p19/P-19B-SOURCE-MANIFEST.json": "b60d9e8fbbddef7a0d3d91c7f60f1bf8d3fff7b4b17e566b1cf1cff9fa2f3171",
    "evidence/p19/P-19B-PLAN-MANIFEST.json": "52294802a050b318e00d7ba27adc89abf674100e724b0616efa0ba4efaa8b8d5",
...
def source_snapshot():
    ...
    # Governance files legitimately advance during P-19C, so the historical
    # P-19B source manifest remains pinned as a receipt but is not treated as a
    # current-worktree manifest. Exact diagram bytes are checked through the
    # P-18 and P-19B gallery manifests below.
...
def generate():
    ...
    records = b["records"]
    require(len(anchors) == 14 and len(records) == 93, "Wrong candidate coverage")
    require(b["visual_parent_candidate_id"] == a["candidate_id"], "Wrong visual parent")
    require(Counter(r["mode"] for r in records) == Counter({m: 31 for m in MODES}), "Wrong mode coverage")
    ...
    require(len(groups) == 31, "Wrong unique type/capability/variant count")
    ...
    require(sum(bool(v["neutral-light"].get("presentation_variant_id")) for v in groups.values()) == 2, "Wrong presentation variant count")
    ...
    for anchor in anchors:
        ...
        for (_, fixture), modes in engine_groups:
            ...
            name = first["identity"] if not first["capability_id"] or first["capability_id"] in {"CAP-V17", "CAP-V18", "CAP-V19", "CAP-V20"} else fixture.removeprefix("cap-").upper()
        ...
        sections.append(f'<section class="{section_class}" id="{engine}" aria-labelledby="heading-{engine}">'
                        ...
                        f'<div class="current"><p class="column-label">P-19B · PHẦN BỔ SUNG · OWNER APPROVED THEO D-126</p>{empty_note}{"".join(rows)}</div></div>'
...
<meta name="color-scheme" content="light"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src data:; style-src 'unsafe-inline'; base-uri 'none'; form-action 'none'">
    <title>P-18 ↔ P-19 · Toàn bộ 107 diagram</title><style>{CSS}</style></head>
...
<h1>P-18 ↔ P-19<br>Toàn bộ diagram, cùng một trang.</h1>
    <p class="intro">14 anchor P-18 đã duyệt + 93 specimen P-19 bổ sung · Tổng cộng 39 loại + 4 capability + hai presentation variant “layers”/“scatter-chart”, không trùng loại giữa hai bộ.</p>
    <p class="note">Theo D-126: <strong>giữ đồng thời hai tập artifact riêng — 14 exact P-18 anchor và 93 P-19 specimen; P-19 không thay thế, ghi đè hoặc tái sinh P-18</strong>. Combined view có đúng 107 diagram với nguồn P-18/P-19 truy nguyên độc lập.</p>
...
<a href="{link(P18 / 'index.html')}" target="_blank" rel="noopener">Mở gallery P-18 ↗</a></div>
    <div><strong>P-19B · review-45 · Owner approved</strong><code>{html.escape(b['candidate_id'])}</code>
...
<input type="checkbox" id="large"><label class="zoom-label" for="large">Phóng lớn diagram để đọc chi tiết</label>
    <p class="hint">Mặc định ba mode P-19 nằm cạnh nhau trên màn hình rộng. Phóng lớn sẽ xếp các mode dọc, vẫn giữ đủ 107 diagram. Bấm diagram để mở bản gốc trong tab mới.</p>
<main id="diagrams">{''.join(sections)}</main>
<footer>Trang đối chiếu phụ trợ P-19C · không thay đổi hai bộ gốc hoặc trạng thái phase. Không bao gồm candidate lịch sử đã bị thay thế.
...
<br>Browser QA được thực hiện qua localhost ở desktop/mobile; xem <a href="../P-19C-BROWSER-VERIFICATION.json">P-19C browser evidence</a>.</footer>
</div></body></html>'''
    inspector = InspectHTML()
    inspector.feed(page)
    require("CAP-V17-DUMBBELL" not in page, "Legacy CAP-V17-DUMBBELL display label leaked into comparison")
    require("<h3>dumbbell<small>capability · parent: bar-chart</small></h3>" in page, "Dumbbell display identity missing")
    require("CAP-V19-RIDGELINE" not in page, "Legacy CAP-V19-RIDGELINE display label leaked into comparison")
    require("<h3>ridgeline<small>capability · parent: line-chart</small></h3>" in page, "Ridgeline display identity missing")
    require("<h3>layer-stack<small>canonical type</small></h3>" in page, "Detailed layer-stack identity missing")
    require("<h3>nested<small>canonical type</small></h3>" in page, "Detailed nested identity missing")
    require(len(inspector.images) == len(inspector.figures) == 107, "Wrong image count")
    require(len(inspector.ids) == len(set(inspector.ids)), "Duplicate viewer IDs")
    require(Counter(f["data-phase"] for f in inspector.figures) == {"p18": 14, "p19": 93}, "Wrong phase count")
    require(len({(f['data-phase'], f['data-identity'], f['data-mode']) for f in inspector.figures}) == 107, "Duplicate diagram")
    ...
    manifest = {"schema_version": "1.0", "purpose": "auxiliary-view-only-not-a-candidate",
                ...
                "source_manifest_pins": PINS, "counts": {"p18": 14, "p19": 93, "total": 107, "engines": 14, "p19_identities": 31, "reused_canonical_types": 14, "presentation_variants": 2},
                "checks": {"source_file_hashes": len(before), "source_preservation": "PASS",
                           ...
                           "browser_render_keyboard_and_responsive": "EXECUTED_SEPARATELY_SEE_P-19C-BROWSER-VERIFICATION"},
...
if __name__ == "__main__":
    ...
    print(json.dumps({"result": "PASS", "mode": "check" if args.check else "generate",
                      "diagrams": 107, "html_bytes": len(outputs["index.html"]),
                      "html_sha256": sha(outputs["index.html"]), "browser": "SEE_P-19C-BROWSER-VERIFICATION"}, indent=2))
