...
from verify_p19b_review45 import verify
...
class ReuseScopeTests(unittest.TestCase):
    ...
    def test_capabilities_and_recent_fixes_retained(self):
        ...
        for name in ('gantt', 'loop-flywheel', 'fishbone', 'dp-integration', 'bar-chart', 'line-chart', 'medallion', 'polar-chart', 'layer-stack', 'nested', 'dumbbell', 'slope-graph', 'ridgeline', 'bubble'):
    ...
    def test_exact_preservation_and_recoverable_withdrawal(self):
        ...
        self.assertEqual(report['nested_measurement']['scopes'], 5)
        self.assertEqual(report['nested_measurement']['artifacts'], 5)
        self.assertEqual(report['nested_measurement']['max_depth'], 4)
        self.assertEqual(report['display_identity'], 'nested')
        self.assertEqual(report['inset_per_depth'], {'x': 65, 'y': 75})
        self.assertEqual(report['three_mode_geometry'], 'PASS')
        self.assertEqual(report['owner_approval'], 'APPROVED_D-126_WITH_P18_P19_COEXISTENCE')
