...
const source = path.join(root, 'evidence/p19/gallery/previews/cap-cap-v17-dumbbell.svg');
const out = path.join(root, 'evidence/p19/review42-checks');
fs.mkdirSync(out, { recursive: true });
const svg = fs.readFileSync(source);
fs.writeFileSync(path.join(out, 'dumbbell.svg'), svg);
...
if (/var\(--[^)]+\)/.test(rasterSvg)) throw new Error('Unresolved template token in dumbbell proof');
await sharp(Buffer.from(rasterSvg),{density:120}).png().toFile(path.join(out,'dumbbell.svg.png'));
