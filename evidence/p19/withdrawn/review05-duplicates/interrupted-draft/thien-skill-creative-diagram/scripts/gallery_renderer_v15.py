...
from sankey_p18_v15 import APPROVED_SHA256, approved_svg, sankey_table, validate_approved_input
from visual_adapters_v15 import CAPABILITY_ADAPTERS, TYPE_ADAPTERS, adapt_visual
P19B_SCHEMA_VERSION = "1.1"
P19B_CANDIDATE_ID = "P19B-P18-INHERITED-THREE-MODE-REVIEW-05-1.5.0"
...
def _special(plan: Mapping[str, Any]) -> str:
    ...
    raise GalleryRenderError("sankey-approved-only", "Sankey now uses the approved P-18 asset; the old recipe is retired.")
...
def render_gallery_html(ir_value: Mapping[str, Any], mode: str, fixture_id: str, *, connector_corner_style: str | None = None) -> str:
    ...
    takeaway = _text(description) if adapter["canonical_type"] in ("gantt", "loop-flywheel") else f"{_text(adapter['semantic_focus'].capitalize())}. {_text(adapter['accessible_alternative'].capitalize())}."
    svg_id = f"diagram-{fixture_id}-{mode}"
    if adapter['canonical_type'] == 'sankey':
        validate_approved_input(ir_value)
        artwork = ''
    else:
    ...
    if adapter['canonical_type'] == 'sankey':
        svg = approved_svg(mode, tokens)
    adoption_attrs = f' data-approved-sankey-sha256="{APPROVED_SHA256}"' if adapter['canonical_type'] == 'sankey' else ''
    metadata = {
...
def renderer_inventory() -> dict[str, Any]:
    ...
    return {
        ...
        "viewbox_overrides": {"gantt": "content-fit calendar; width >=1600; height from phase and row content", "loop-flywheel": "content-fit circle; width >=1600; card and shared-state clearance"},
