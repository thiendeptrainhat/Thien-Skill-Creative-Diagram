...
from sankey_p18_fixture import sankey_fixture
...
def all_sources() -> list[tuple[str, dict]]:
    sources = fixtures()
    sources['sankey'] = sankey_fixture()
    values = [(f"type-{key}", value) for key, value in sources.items()]
...
class GalleryRendererV15Tests(unittest.TestCase):
    ...
    def test_every_document_is_scriptless_and_network_independent(self) -> None:
        ...
        for fixture_id, ir in all_sources():
            for mode in MODES:
                page = render_gallery_html(ir, mode, fixture_id)
                page = page.replace('xmlns="http://www.w3.org/2000/svg"', '')
    ...
    def test_every_document_has_unique_svg_accessibility_ids(self) -> None:
        for fixture_id, ir in all_sources():
            for mode in MODES:
                ...
                # Gantt also exposes an exact-date tooltip for every material mark.
                self.assertEqual(parser.tags.count("title"), 2 + (len(ir['nodes']) if ir['diagram']['type'] == 'gantt' else 0))
    ...
    def test_mode_derivation_preserves_svg_geometry(self) -> None:
        for fixture_id, ir in all_sources():
            ...
            for mode in MODES:
                ...
                normalized.append(normalize_paint(svg) if ir['diagram']['type'] == 'sankey' else svg.replace(mode, "MODE"))
