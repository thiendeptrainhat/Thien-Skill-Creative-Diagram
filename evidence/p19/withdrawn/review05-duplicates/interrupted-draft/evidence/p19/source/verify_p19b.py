...
from sankey_p18_v15 import normalize_paint
...
def main() -> None:
    ...
    check(checks, "exact-specimen-count", len(files) == len(records) == 129, f"files={len(files)} records={len(records)}")
    check(checks, "canonical-count", inventory["canonical_type_count"] == 39, str(inventory["canonical_type_count"]))
    ...
    check(checks, "engine-count", inventory["layout_engine_count"] == 14, str(inventory["layout_engine_count"]))
    mode_counts = Counter(item["mode"] for item in records)
    check(checks, "mode-balance", mode_counts == Counter({mode: 43 for mode in MODES}), str(dict(mode_counts)))
    ...
    check(checks, "canonical-three-mode-count", len(canonical_records) == 117, str(len(canonical_records)))
    check(checks, "capability-three-mode-count", len(capability_records) == 12, str(len(capability_records)))
    check(checks, "unique-fixture-count", len({item["fixture_id"] for item in records}) == 43, str(len({item["fixture_id"] for item in records})))
    check(checks, "unique-silhouette-count", len({item["silhouette"] for item in records}) == 43, str(len({item["silhouette"] for item in records})))
    ...
    for record in records:
        ...
        if re.search(r"(?:https?:|//fonts\.|<script\b|javascript:|\son[a-z]+\s*=)", value.replace('xmlns="http://www.w3.org/2000/svg"',''), re.IGNORECASE):
        ...
        geometry_groups.setdefault(record["fixture_id"], []).append(normalize_paint(svg) if record['canonical_type']=='sankey' else svg.replace(record["mode"], "MODE"))
    ...
    check(checks, "p18-typography-role-receipts", all(value in all_text for value in ("georgia · display", "avenir next · material", "menlo · technical")), "129 specimens expose P-18 resolved typography roles")
    check(checks, "p18-visual-grammar-marker", all('data-visual-grammar="p18r6-review17"' in path.read_text(encoding="utf-8") for path in files), "129/129")
    ...
    check(checks, "index-link-completeness", set(index_parser.hrefs) == expected_links and len(index_parser.hrefs) == 129, f"links={len(index_parser.hrefs)}")
