...
PINS = {
    ...
    "evidence/p19/gallery/P-19B-MANIFEST.json": "976e57bb40524c3d1b9546babf02b473a0bfce03074eca58a8245c296564716c",
    "evidence/p19/P-19B-SOURCE-MANIFEST.json": "c07f482d876f85d58c277f08947faf8d9e06bf791c060e7c1c76a92eae1136d8",
    "evidence/p19/P-19B-PLAN-MANIFEST.json": "3622ad185ccf880e100fc38d5c32b2ccf1ff2674dbbb2db1cf294e1a06a27fe9",
...
def p19_preview(page):
    ...
    if 'data-approved-sankey-sha256=' in text:
        require(len(svgs)==1 and len(styles)==2, 'Approved Sankey must retain its original embedded CSS')
        encoded=svgs[0].encode('utf-8')
        safe_svg(encoded)
        return encoded, sha(encoded), sha(styles[1].encode('utf-8'))
...
.comparison{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,3fr);gap:24px}.reference{align-self:start;position:sticky;top:16px}.column-label{margin:0 0 10px;font-size:13px;font-weight:700}
...
def generate():
    ...
    records = b["records"]
    require(len(anchors) == 14 and len(records) == 129, "Wrong candidate coverage")
    require(b["visual_parent_candidate_id"] == a["candidate_id"], "Wrong visual parent")
    require(Counter(r["mode"] for r in records) == Counter({m: 43 for m in MODES}), "Wrong mode coverage")
    ...
    require(len(groups) == 43, "Wrong unique type/capability count")
    require(sum(not v["neutral-light"]["capability_id"] for v in groups.values()) == 39, "Wrong canonical count")
    require({r["engine"] for r in anchors} == {key[0] for key in groups}, "Engine mismatch")
    ...
    for anchor in anchors:
        ...
        for (_, fixture), modes in engine_groups:
            ...
            kind = "canonical type" if not first["capability_id"] else f'capability · parent: {first["parent"]}'
            images = []
            for mode in MODES:
                record = modes[mode]
        ...
        sections.append(f'<section class="engine" id="{engine}" aria-labelledby="heading-{engine}">'
                        ...
                        f'<div class="current"><p class="column-label">P-19 · CANDIDATE HIỆN TẠI · CHƯA ĐƯỢC OWNER DUYỆT</p>{"".join(rows)}</div></div>'
...
<title>P-18 ↔ P-19 · Toàn bộ 143 diagram</title><style>{CSS}</style></head>
...
<p class="intro">14 anchor P-18 đã duyệt + 129 specimen P-19 hiện tại · 14 engine · 43 type/capability × 3 mode.</p>
<p class="note">Cùng engine, <strong>khác scenario và dữ liệu</strong>. Trang này dùng để đối chiếu thiết kế; không xác nhận hai bộ có nội dung tương đương hoặc P-19 đã đạt yêu cầu.</p>
...
<div><strong>P-19B · review-04 · Chờ owner review</strong><code>{html.escape(b['candidate_id'])}</code>
...
<p class="hint">Mặc định ba mode P-19 nằm cạnh nhau trên màn hình rộng. Phóng lớn sẽ xếp các mode dọc, vẫn giữ đủ 143 diagram. Bấm diagram để mở bản gốc trong tab mới.</p>
...
<br>Chưa kiểm chứng render trong browser do chính sách URL local-file; không xem đây là browser QA PASS.</footer>
    ...
    inspector.feed(page)
    require(len(inspector.images) == len(inspector.figures) == 143, "Wrong image count")
    require(len(inspector.ids) == len(set(inspector.ids)), "Duplicate viewer IDs")
    require(Counter(f["data-phase"] for f in inspector.figures) == {"p18": 14, "p19": 129}, "Wrong phase count")
    require(len({(f['data-phase'], f['data-identity'], f['data-mode']) for f in inspector.figures}) == 143, "Duplicate diagram")
    ...
    manifest = {"schema_version": "1.0", "purpose": "auxiliary-view-only-not-a-candidate",
                ...
                "source_manifest_pins": PINS, "counts": {"p18": 14, "p19": 129, "total": 143, "engines": 14},
...
if __name__ == "__main__":
    ...
    print(json.dumps({"result": "PASS", "mode": "check" if args.check else "generate",
                      "diagrams": 143, "html_bytes": len(outputs["index.html"]),
