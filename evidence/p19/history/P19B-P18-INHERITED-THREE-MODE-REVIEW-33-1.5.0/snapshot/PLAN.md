...
**Cập nhật:** 2026-08-30  
**Phạm vi được phép trong yêu cầu hiện tại:** D-113: thêm riêng presentation variant P-19 `scatter-chart` dưới frozen parent `scatter-plot`, giữ exact P-18 anchor, 90 prior P-19 artwork/30 preview cùng mọi frozen/release bytes; cập nhật gallery/trang tổng hợp/evidence/handoff.
**Phase đang thực hiện:** P-19B review-33 `in-progress`/owner-review-pending theo D-113, giữ scope D-084–D-112; exact review-32 archived; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
2. `P-19B` — remediation `in-progress` theo D-084–D-113: dùng trực tiếp 14 exact P-18R6 review-17 anchor, không tái sinh duplicate; 25 canonical type, bốn capability và hai presentation variant `layers`/`scatter-chart` có ba mode, tổng 93 HTML/31 preview. Active review-33 giữ toàn bộ remediation trước và bổ sung riêng `scatter-chart` dưới parent `scatter-plot`; comparison có 107 diagram, 90 prior HTML/30 preview được bảo toàn và withdrawal custody vẫn khôi phục được.
...
**Verification dự kiến:** theo D-085/D-095–D-113, union 14 P-18 canonical + 25 P-19 canonical phải đúng 39 và không trùng; P-19 có 75 canonical + 12 capability + 3 `layers` + 3 `scatter-chart` variant HTML, 31 preview, đủ ba mode cho 31 identity. Đủ 93 SVG phải khai báo D-105; mọi detailed artifact D-086–D-112 phải được giữ. Scatter-chart phải có đúng 12 point/2 axis/11 tick/1 dashed OLS trend/1 focal, exact 12-row table và geometry ba mode; Treemap tiếp tục giữ exact D-101/D-103 geometry cùng regular/focal stroke `1.2/1.6`. P-18 phải giữ exact bytes/hash ở neutral-light, không claim ba-mode coverage cho 14 anchor này. Kiểm absence of withdrawn files, links, deterministic generation, retained artwork và recoverable custody; owner vẫn phải duyệt exact current gallery.
...
**Checkpoint D-113:** active candidate `P19B-P18-INHERITED-THREE-MODE-REVIEW-33-1.5.0`. Exact review-32 đã archive 522 files; protected corpus 13056 files khớp hash. Thêm riêng presentation variant `scatter-chart` dưới frozen parent `scatter-plot`: 12 exact point, two arrow-free axis, 11 tick, một descending dashed OLS trend và một focal Platform point, cùng exact 12-row table và three-mode geometry. Giữ 90 prior HTML artworks sau candidate normalization và 30 prior preview byte-identical; thêm đúng 3 target HTML + 1 preview, tổng 93/31 và comparison 107. Owner approval pending, P-19C unauthorized, G-04 NOT-EVALUATED.
