...
**Cập nhật:** 2026-08-30  
**Phạm vi được phép trong yêu cầu hiện tại:** D-123: đổi identity hiển thị `CAP-V19` thành `ridgeline` và thay riêng artwork bằng 12 shared-domain Gaussian-KDE distribution/12 median/36 nested quantile band/1 shared median reference; giữ internal ID `CAP-V19`, parent `line-chart`, template hiện hành, exact P-18 anchor, 90 non-target P-19 artwork/30 preview cùng mọi frozen/release bytes; cập nhật gallery/trang tổng hợp/evidence/handoff.
**Phase đang thực hiện:** P-19B review-43 `in-progress`/owner-review-pending theo D-123, giữ scope D-084–D-122; exact review-42 archived; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
2. `P-19B` — remediation `in-progress` theo D-084–D-123: dùng trực tiếp 14 exact P-18R6 review-17 anchor, không tái sinh duplicate; 25 canonical type, bốn capability và hai presentation variant `layers`/`scatter-chart` có ba mode, tổng 93 HTML/31 preview. Active review-43 giữ toàn bộ remediation trước, đổi identity hiển thị CAP-V19 thành `ridgeline` và thay riêng ridgeline artwork; comparison có 107 diagram, 90 non-target HTML/30 preview được bảo toàn.
...
**Verification dự kiến:** theo D-085/D-095–D-123, union 14 P-18 canonical + 25 P-19 canonical phải đúng 39 và không trùng; P-19 có 75 canonical + 12 capability + 3 `layers` + 3 `scatter-chart` variant HTML, 31 preview, đủ ba mode cho 31 identity. Đủ 93 SVG phải khai báo D-105; mọi detailed artifact D-086–D-122 phải được giữ. `ridgeline` phải giữ internal `CAP-V19`, parent `line-chart`, đúng 12 density/12 median/36 nested 50/80/95% band/1 shared-median line/1 arrow-free axis/7 tick/1 focal và exact 12-row table trên cùng miền 0–120 ms/global-max amplitude; ba mode cùng geometry. Dumbbell/slope-graph/Bubble/process/radar/scatter-chart/Treemap contract trước vẫn giữ. P-18 phải giữ exact bytes/hash ở neutral-light, không claim ba-mode coverage cho 14 anchor này. Kiểm absence of withdrawn files, links, deterministic generation, retained artwork và recoverable custody; owner vẫn phải duyệt exact current gallery.
...
**Checkpoint D-123:** active candidate `P19B-P18-INHERITED-THREE-MODE-REVIEW-43-1.5.0`. Exact review-42 đã archive 624 files; protected corpus 18757 files khớp hash. Display identity là `ridgeline`, internal capability ID vẫn `CAP-V19`, canonical parent vẫn `line-chart`. Artwork có đúng 12 density silhouette/12 median/36 nested quantile band/1 shared-median reference/1 arrow-free shared axis/7 tick/1 focal trên miền 0–120 ms với Gaussian KDE/global-max normalization; exact 12-row table dùng cùng dữ liệu. Ba mode cùng geometry; template P-18 giữ nguyên. 90 non-target HTML/30 preview được bảo toàn; tổng vẫn 93/31 và comparison 107. Owner approval pending, P-19C unauthorized, G-04 NOT-EVALUATED.
