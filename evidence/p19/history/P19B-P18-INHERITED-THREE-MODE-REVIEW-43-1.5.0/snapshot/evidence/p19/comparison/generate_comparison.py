...
PINS = {
    ...
    "evidence/p19/gallery/P-19B-MANIFEST.json": "3563b0f11788a95fd1377d9e0c49f94a93bdf3b727910d7e29d0ebcc22ab5f0d",
    "evidence/p19/P-19B-SOURCE-MANIFEST.json": "67d2b2aab03da7e1cd4ad133e0f599faf40b566c6c035d3d51813f04b87a5898",
    "evidence/p19/P-19B-PLAN-MANIFEST.json": "203e43f4680dd0e4d2953a51977aac628b11e927b9c7149addcd9bf01b061c7f",
...
<h1>P-18 ↔ P-19<br>Toàn bộ diagram, cùng một trang.</h1>
    ...
    <p class="note">Theo D-084/D-085/D-095–D-123: <strong>giữ nguyên 14 bản P-18 và mọi remediation trước; đổi display identity CAP-V19 thành ridgeline, dùng đúng 12 phân phối/12 median/36 quantile band trên shared domain 0–120 ms với Gaussian KDE và global-max normalization trong template hiện hành</strong>. P-19 có 25 loại + 4 capability + 2 presentation variant, mỗi identity đủ 3 mode; P-18 chỉ dùng neutral-light đã duyệt, không tự tạo thêm màu.</p>
...
<a href="{link(P18 / 'index.html')}" target="_blank" rel="noopener">Mở gallery P-18 ↗</a></div>
    <div><strong>P-19B · review-43 · Chờ owner review</strong><code>{html.escape(b['candidate_id'])}</code>
...
<br>Chưa kiểm chứng render trong browser do chính sách URL local-file; không xem đây là browser QA PASS.</footer>
    ...
    manifest = {"schema_version": "1.0", "purpose": "auxiliary-view-only-not-a-candidate",
                ...
                "checks": {"source_file_hashes": len(before), "source_preservation": "PASS",
                           ...
                           "browser_render_keyboard_and_responsive": "BLOCKED_URL_POLICY"},
...
if __name__ == "__main__":
    ...
    print(json.dumps({"result": "PASS", "mode": "check" if args.check else "generate",
                      ...
                      "html_sha256": sha(outputs["index.html"]), "browser": "BLOCKED_URL_POLICY"}, indent=2))
