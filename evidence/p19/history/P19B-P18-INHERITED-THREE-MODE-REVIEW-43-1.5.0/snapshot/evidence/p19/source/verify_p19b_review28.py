#!/usr/bin/env python3
"""D-108 ultra-thin tree and review-27 preservation verification."""
...
OLD = "P19B-P18-INHERITED-THREE-MODE-REVIEW-27-1.5.0"
ARCHIVE = ROOT / "evidence/p19/history" / OLD
GALLERY = ROOT / "evidence/p19/gallery"
PROOFS = ROOT / "evidence/p19/review28-checks"
TARGET_STROKES = {
    "connector": "1.0", "card": "1.2", "root": "1.6",
    "badge": "0.9", "separator": "1.0",
    "legend_regular": "1.2", "legend_root": "1.6",
}
...
def verify():
    ...
    require(inventory["candidate_id"] == P19B_CANDIDATE_ID and len(records) == 90, "Wrong review-28 gallery")
    ...
    for record in records:
        ...
        if record["fixture_id"] == "type-tree":
            css_checks = (
                ".tree-wire{fill:none;stroke:var(--connector);stroke-width:1;",
                ".tree-card-shape{fill:var(--surface);stroke:var(--connector);stroke-width:1.2",
                ".tree-card.is-root .tree-card-shape{fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.6",
                ".tree-badge-shape{fill:var(--canvas);stroke:var(--border);stroke-width:.9}",
                ".tree-rule{stroke:var(--grid);stroke-width:1}",
                ".tree-swatch-root{fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.6}",
                ".tree-swatch-branch{fill:var(--surface);stroke:var(--connector);stroke-width:1.2}",
            )
            require(all(token in page for token in css_checks), f"Stroke system drift: {current.name}")
            ...
            previous_svg = re.search(r"<svg\b.*?</svg>", previous.read_text(), re.S).group()
            require(geometry_only(svg) == geometry_only(previous_svg), f"Tree geometry changed: {current.name}")
        else:
            previous = ARCHIVE / "snapshot" / current.relative_to(ROOT)
    ...
    require(len(target_geometries) == len(MODES) and target_geometries[0] == target_geometries[1] == target_geometries[2], "Tree geometry differs across modes")
    expected = {"nodes": 9, "edges": 8, "tiers": 3, "connector_primitives": 14, "centered_parents": 4, "single_child_straight": 1}
    require(all(item == expected for item in target_measurements), "Tree measurements mismatch")
    ...
    raster, proof_svg = PROOFS / "type-tree.svg.png", PROOFS / "type-tree.svg"
    require(raster.is_file() and proof_svg.is_file(), "Missing D-108 visual proof")
    return {
        "candidate_id": P19B_CANDIDATE_ID, "authority": "D-108", "status": "PASS",
        ...
        "tree_measurement": target_measurements[0], "stroke_contract": TARGET_STROKES,
        "geometry_vs_review27": "BYTE_EQUIVALENT_GEOMETRY_ELEMENTS",
        "parent_span_center_proofs": center_proofs, "three_mode_geometry": "PASS",
...
if __name__ == "__main__":
    report = verify()
    output = ROOT / "evidence/p19/P-19B-REVIEW-28-VERIFICATION.json"
