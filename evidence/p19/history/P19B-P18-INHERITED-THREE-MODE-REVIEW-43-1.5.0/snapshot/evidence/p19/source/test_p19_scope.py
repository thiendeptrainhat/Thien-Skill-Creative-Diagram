...
from verify_p19b_review43 import verify
...
class ReuseScopeTests(unittest.TestCase):
    ...
    def test_capabilities_and_recent_fixes_retained(self):
        ...
        for name in ('gantt', 'loop-flywheel', 'fishbone', 'dp-integration', 'bar-chart', 'line-chart', 'medallion', 'polar-chart', 'dumbbell', 'slope-graph', 'ridgeline', 'bubble'):
    ...
    def test_exact_preservation_and_recoverable_withdrawal(self):
        ...
        self.assertEqual(report['ridgeline_measurement']['ridges'], 12)
        self.assertEqual(report['ridgeline_measurement']['medians'], 12)
        self.assertEqual(report['ridgeline_measurement']['bands'], 36)
        self.assertEqual(report['ridgeline_measurement']['focal'], 1)
        self.assertEqual(report['display_identity'], 'ridgeline')
        self.assertEqual(report['internal_capability_id'], 'CAP-V19')
        self.assertEqual(report['canonical_parent'], 'line-chart')
