...
const source = path.join(root, 'evidence/p19/gallery/previews/cap-cap-v20-bubble.svg');
const out = path.join(root, 'evidence/p19/review40-checks');
fs.mkdirSync(out, { recursive: true });
const svg = fs.readFileSync(source);
fs.writeFileSync(path.join(out, 'bubble.svg'), svg);
...
await sharp(Buffer.from(rasterSvg),{density:120}).png().toFile(path.join(out,'bubble.svg.png'));
