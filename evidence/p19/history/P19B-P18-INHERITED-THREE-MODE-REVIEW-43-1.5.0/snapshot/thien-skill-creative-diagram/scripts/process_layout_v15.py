"""D-119 detailed process renderer with continuous thin-stroke connectors."""
...
def layout_process(plan):
    ...
    _require(tuple(item["id"] for item in nodes) == EXPECTED_NODE_ORDER, "D-119 process node inventory mismatch")
    _require(tuple(item["id"] for item in edges) == EXPECTED_EDGE_ORDER, "D-119 process edge inventory mismatch")
    counts = {kind: 0 for kind in EXPECTED_SHAPES}
    laid_out_nodes = []
    for item in nodes:
        shape_kind = _shape_kind(item)
        _require(shape_kind in counts, f"D-119 unsupported shape: {shape_kind}")
        counts[shape_kind] += 1
        laid_out_nodes.append({**item, "shape_kind": shape_kind, "box": NODE_BOXES[item["id"]]})
    _require(counts == EXPECTED_SHAPES, "D-119 process shape taxonomy mismatch")
    routes = {
...
def process_css(tokens):
    return """
    .pr-node{fill:var(--surface);stroke:var(--connector);stroke-width:1.2;vector-effect:non-scaling-stroke}
    .pr-node.focal{fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.6}
    .pr-node.muted{fill:var(--surface-alt);stroke:var(--border)}
    .pr-layer{fill:var(--surface-alt);stroke:var(--connector);stroke-width:1}
    .pr-layer.focal{fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.2}
    .pr-route{fill:none;stroke:var(--connector);stroke-width:1;stroke-linecap:round;stroke-linejoin:round;vector-effect:non-scaling-stroke}
    .pr-route.merge{stroke-width:1.2}
    ...
    .pr-badge{fill:var(--surface-alt);stroke:var(--border);stroke-width:.8}
    ...
    .pr-rule{stroke:var(--grid);stroke-width:.8}
...
def render_process(plan):
    ...
    parts = [
        '<g data-process-contract="D-119-thin-continuous-document-connectors" '
...
def validate_process_svg(svg):
    ...
    _require(tuple(item.attrib["data-process-node"] for item in nodes) == EXPECTED_NODE_ORDER, "Serialized D-119 node order mismatch")
    _require(tuple(item.attrib["data-process-edge"] for item in edges) == EXPECTED_EDGE_ORDER, "Serialized D-119 edge order mismatch")
    counts = {kind: 0 for kind in EXPECTED_SHAPES}
    for item in nodes:
        counts[item.attrib["data-shape-kind"]] += 1
    _require(counts == EXPECTED_SHAPES, "Serialized D-119 shape count mismatch")
    ...
    _require(len(straight) == 9 and all(item.tag == "line" for item in straight), "D-119 straight route count mismatch")
    _require(len(orthogonal) == 2 and all(item.tag == "path" for item in orthogonal), "D-119 merge route count mismatch")
    _require(all(item.attrib.get("marker-end") == "url(#pr-arrow-merge)" for item in orthogonal), "D-119 merge arrowhead mismatch")
    _require(all("Q" in item.attrib.get("d", "") for item in orthogonal), "D-119 merge routes must have rounded 90-degree corners")
    ...
    _require(pack_x == [920.0, 1080.0] and abs((pack_x[0] + pack_x[1]) / 2 - 1000) < .01, "D-119 multiple inlet spacing mismatch")
    _require(all(item.attrib.get("data-boundary-contact") == "true" for item in edges if item.attrib["data-process-edge"] in {"flow-incomplete-document", "flow-document-return", "flow-standard-pack", "flow-control-pack", "flow-pack-log"}), "D-119 document boundary contact mismatch")
    return {
        "nodes": 11,
        "edges": 11,
        "shape_counts": counts,
