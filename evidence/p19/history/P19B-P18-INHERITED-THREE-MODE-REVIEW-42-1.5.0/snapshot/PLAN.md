...
**Cập nhật:** 2026-08-30  
**Phạm vi được phép trong yêu cầu hiện tại:** D-122: đổi identity hiển thị `CAP-V17` thành `dumbbell` và thay riêng artwork bằng 12 pair/24 endpoint/2 series/12 direct delta/shared 0–100% scale/mean±population-stdev bands; giữ internal ID `CAP-V17`, parent `bar-chart`, template hiện hành, exact P-18 anchor, 90 non-target P-19 artwork/30 preview cùng mọi frozen/release bytes; cập nhật gallery/trang tổng hợp/evidence/handoff.
**Phase đang thực hiện:** P-19B review-42 `in-progress`/owner-review-pending theo D-122, giữ scope D-084–D-121; exact review-41 archived; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
2. `P-19B` — remediation `in-progress` theo D-084–D-122: dùng trực tiếp 14 exact P-18R6 review-17 anchor, không tái sinh duplicate; 25 canonical type, bốn capability và hai presentation variant `layers`/`scatter-chart` có ba mode, tổng 93 HTML/31 preview. Active review-42 giữ toàn bộ remediation trước, đổi identity hiển thị CAP-V17 thành `dumbbell` và thay riêng dumbbell artwork; comparison có 107 diagram, 90 non-target HTML/30 preview được bảo toàn.
...
**Verification dự kiến:** theo D-085/D-095–D-122, union 14 P-18 canonical + 25 P-19 canonical phải đúng 39 và không trùng; P-19 có 75 canonical + 12 capability + 3 `layers` + 3 `scatter-chart` variant HTML, 31 preview, đủ ba mode cho 31 identity. Đủ 93 SVG phải khai báo D-105; mọi detailed artifact D-086–D-121 phải được giữ. `dumbbell` phải giữ internal `CAP-V17`, parent `bar-chart`, đúng 12 pair/24 endpoint/2 series/12 delta/2 statistics band/2 mean line/1 arrow-free axis/6 tick/1 focal và exact 12-row table trên cùng thang 0–100%; ba mode cùng geometry. Slope-graph/Bubble/process/radar/scatter-chart/Treemap contract trước vẫn giữ. P-18 phải giữ exact bytes/hash ở neutral-light, không claim ba-mode coverage cho 14 anchor này. Kiểm absence of withdrawn files, links, deterministic generation, retained artwork và recoverable custody; owner vẫn phải duyệt exact current gallery.
...
**Checkpoint D-122:** active candidate `P19B-P18-INHERITED-THREE-MODE-REVIEW-42-1.5.0`. Exact review-41 đã archive 613 files; protected corpus 18143 files khớp hash. Display identity là `dumbbell`, internal capability ID vẫn `CAP-V17`, canonical parent vẫn `bar-chart`. Artwork có đúng 12 horizontal pair/24 endpoint/2 series/12 direct delta/2 statistics band/2 mean line/1 arrow-free shared axis/6 tick/1 focal trên thang 0–100%; mọi line chạm đúng tâm endpoint và exact 12-row table dùng cùng dữ liệu. Ba mode cùng geometry; template P-18 giữ nguyên. 90 non-target HTML/30 preview được bảo toàn; tổng vẫn 93/31 và comparison 107. Owner approval pending, P-19C unauthorized, G-04 NOT-EVALUATED.
