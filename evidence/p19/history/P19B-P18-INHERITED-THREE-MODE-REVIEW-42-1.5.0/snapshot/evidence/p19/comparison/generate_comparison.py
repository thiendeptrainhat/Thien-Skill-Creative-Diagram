...
PINS = {
    ...
    "evidence/p19/gallery/P-19B-MANIFEST.json": "f15fa7b03e0f647727cbe274d00fcda39cdafd0f5d34cde3ce6659c966446220",
    "evidence/p19/P-19B-SOURCE-MANIFEST.json": "6906500321b0569fe8c3c4d52e8c825845db841b7afb21955256642cd6c9afc5",
    "evidence/p19/P-19B-PLAN-MANIFEST.json": "3fae28b52bc6a7e591ba5001acfc19936486f99505e8e62c780334cf786e04fb",
...
def generate():
    ...
    for anchor in anchors:
        ...
        for (_, fixture), modes in engine_groups:
            ...
            name = first["identity"] if not first["capability_id"] or first["capability_id"] in {"CAP-V17", "CAP-V18", "CAP-V20"} else fixture.removeprefix("cap-").upper()
...
<h1>P-18 ↔ P-19<br>Toàn bộ diagram, cùng một trang.</h1>
    ...
    <p class="note">Theo D-084/D-085/D-095–D-122: <strong>giữ nguyên 14 bản P-18, bỏ các bản P-19 trùng loại, giữ mọi remediation trước; đổi display identity CAP-V17 thành dumbbell và dùng đúng biểu đồ 12 cặp/24 endpoint/12 đoạn nối thẳng/direct delta/statistical bands trong template hiện hành</strong>. P-19 có 25 loại + 4 capability + 2 presentation variant, mỗi identity đủ 3 mode; P-18 chỉ dùng neutral-light đã duyệt, không tự tạo thêm màu.</p>
...
<a href="{link(P18 / 'index.html')}" target="_blank" rel="noopener">Mở gallery P-18 ↗</a></div>
    <div><strong>P-19B · review-42 · Chờ owner review</strong><code>{html.escape(b['candidate_id'])}</code>
