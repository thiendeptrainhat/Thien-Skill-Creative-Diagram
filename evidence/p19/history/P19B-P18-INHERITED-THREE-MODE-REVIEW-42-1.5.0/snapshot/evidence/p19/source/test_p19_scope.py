...
from verify_p19b_review42 import verify
...
class ReuseScopeTests(unittest.TestCase):
    ...
    def test_capabilities_and_recent_fixes_retained(self):
        ...
        for name in ('gantt', 'loop-flywheel', 'fishbone', 'dp-integration', 'bar-chart', 'line-chart', 'medallion', 'polar-chart', 'dumbbell', 'slope-graph', 'CAP-V19', 'bubble'):
    ...
    def test_exact_preservation_and_recoverable_withdrawal(self):
        ...
        self.assertEqual(report['dumbbell_measurement']['pairs'], 12)
        self.assertEqual(report['dumbbell_measurement']['endpoints'], 24)
        self.assertEqual(report['dumbbell_measurement']['focal'], 1)
        self.assertEqual(report['display_identity'], 'dumbbell')
        self.assertEqual(report['internal_capability_id'], 'CAP-V17')
        self.assertEqual(report['canonical_parent'], 'bar-chart')
