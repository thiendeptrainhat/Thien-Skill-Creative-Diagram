"""Focused D-112 tests for the thin, uniformly inset exact-area Treemap."""
import copy
import re
import unittest
...
class DetailedTreemapTests(unittest.TestCase):
    ...
    def test_every_tile_has_complete_outline_and_uniform_inset(self):
        ...
        self.assertIn('.tm-tile{stroke:var(--connector);stroke-width:1.2', css)
        self.assertIn('.tm-tile.tm-focal{fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.6}', css)
        self.assertIn('.tm-rule{stroke:var(--grid);stroke-width:1}', css)
        self.assertIn('.tm-swatch-focal{fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.6}', css)
        self.assertIn('stroke:var(--connector);stroke-width:1.2}', css)
        self.assertIn('data-treemap-contract="D-112-thin-complete-borders"', svg)
        self.assertIn('data-geometry-contract="D-103-uniform-inset-complete-borders"', svg)
        for tile in layout["tiles"]:
