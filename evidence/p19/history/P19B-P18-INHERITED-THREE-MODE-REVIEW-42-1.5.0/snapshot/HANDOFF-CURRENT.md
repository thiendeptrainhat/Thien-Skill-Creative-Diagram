# Handoff phiên mới — P-19B D-122 review-42 dumbbell; owner review pending; P-19C unauthorized
...
2. `PROJECT-CONTRACT.md` — yêu cầu, phạm vi và decision ledger D-001 đến D-122.
...
- P-19: `in-progress`; P-19A `passed`. Active P-19B `P19B-P18-INHERITED-THREE-MODE-REVIEW-42-1.5.0` theo D-122, giữ scope D-084–D-121: 14 P-18 anchor + 93 P-19 HTML/31 preview. Review-42 đổi display identity `CAP-V17-DUMBBELL` thành `dumbbell`, giữ internal `CAP-V17`/parent `bar-chart`, và thay riêng artwork bằng dumbbell chart 12 pair/24 endpoint/12 straight connector/12 delta label/2 statistical band/2 mean line/1 focal trên cùng thang 0–100%; template hiện tại giữ nguyên. 90 non-target HTML/30 preview được bảo toàn. Exact review-41 đã archive 613 file; 18143 protected file khớp. Owner approval pending. P-19C `not-started`/unauthorized, G-04 NOT-EVALUATED; không package/dist/publication/Git/Release.
...
- `PLAN.md`
- `PROJECT-CONTRACT.md` — D-001 đến D-122; D-077 khóa owner approval/G-03/P-18 closure, D-078 authorize/đóng P-19A, D-105 khóa global connector policy, D-113 khóa scatter-chart, D-114 khóa detailed radar, D-115 khóa solid-line radar, D-116 khóa marker-free radar plot, D-117 khóa detailed five-shape process, D-118 khóa continuous document connectors, D-119 khóa thin-stroke process, D-120 khóa Bubble identity/chart, D-121 khóa slope-graph identity/chart và D-122 khóa dumbbell identity/chart
...
### D-122 checkpoint hiện hành

Exact review-41 đã archive byte-bound trước mutation: 613 snapshot file và 18143 protected file khớp. Active review-42 đổi display identity thành `dumbbell` nhưng giữ internal `CAP-V17`/parent `bar-chart`; chart có đúng 12 pair, 24 endpoint, 12 straight connector, 12 delta label, 2 series, 2 statistical band, 2 mean line, 1 plain axis, 6 tick, 1 focal và 12 exact table row trên cùng thang 0–100%. Ba mode cùng geometry; neutral-light raster 2000×1160 đã inspect. Focused/scope 11/11, static 34/34, full regression 403/403 và exact verifier PASS. Owner approval pending; P-19C/G-04/package/release chưa được phép.
...
- P-17 semantic source vẫn đủ 39 type + bốn capability; P-19A registry không đổi. Gallery hiện hành giữ trực tiếp 14 P-18 anchor ở neutral-light, sinh 93 HTML P-19/31 identity theo D-085/D-095–D-122, gồm hai presentation variant `layers`/`scatter-chart`, display identity `dumbbell` với internal `CAP-V17`, `slope-graph` với internal `CAP-V18` và `bubble` với internal `CAP-V20`, đồng thời giữ global connector policy D-105 cùng mọi detailed remediation trước; owner approval pending.
