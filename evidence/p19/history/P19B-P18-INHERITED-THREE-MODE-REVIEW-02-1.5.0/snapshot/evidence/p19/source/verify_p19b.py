...
def main() -> None:
    ...
    sources = [(f"type-{key}", value) for key, value in fixtures().items()]
    sources.extend((f"cap-{key.lower()}-{name}", variant_fixtures()[key]) for key, name in {"CAP-V17": "dumbbell", "CAP-V18": "slopegraph", "CAP-V19": "ridgeline", "CAP-V20": "bubble"}.items())
    memory_hashes = {
