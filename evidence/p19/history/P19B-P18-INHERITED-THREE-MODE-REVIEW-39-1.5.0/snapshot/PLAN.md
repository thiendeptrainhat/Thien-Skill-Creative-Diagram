...
**Cập nhật:** 2026-08-30  
**Phạm vi được phép trong yêu cầu hiện tại:** D-119: giảm riêng paint weight của P-19 `process` về thin hierarchy, giữ nguyên D-118 geometry/connector continuity/template/nội dung, exact P-18 anchor, 90 non-target P-19 artwork/30 preview cùng mọi frozen/release bytes; cập nhật gallery/trang tổng hợp/evidence/handoff.
**Phase đang thực hiện:** P-19B review-39 `in-progress`/owner-review-pending theo D-119, giữ scope D-084–D-118; exact review-38 archived; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
2. `P-19B` — remediation `in-progress` theo D-084–D-119: dùng trực tiếp 14 exact P-18R6 review-17 anchor, không tái sinh duplicate; 25 canonical type, bốn capability và hai presentation variant `layers`/`scatter-chart` có ba mode, tổng 93 HTML/31 preview. Active review-39 giữ toàn bộ remediation trước và giảm riêng paint weight của `process`; comparison có 107 diagram, 90 non-target HTML/30 preview được bảo toàn.
...
**Verification dự kiến:** theo D-085/D-095–D-119, union 14 P-18 canonical + 25 P-19 canonical phải đúng 39 và không trùng; P-19 có 75 canonical + 12 capability + 3 `layers` + 3 `scatter-chart` variant HTML, 31 preview, đủ ba mode cho 31 identity. Đủ 93 SVG phải khai báo D-105; mọi detailed artifact D-086–D-118 phải được giữ. `Process` phải giữ đúng năm loại ô/11 node/11 directed route, gồm 9 straight + 2 symmetric rounded-orthogonal merge route; 5 document-bound connector chạm visible boundary, hai inlet x=920/1080, exact node/edge table và geometry ba mode; stroke hierarchy phải đúng D-119 `1.0 < 1.2 < 1.6`. Radar/scatter-chart/Treemap contract trước vẫn giữ. P-18 phải giữ exact bytes/hash ở neutral-light, không claim ba-mode coverage cho 14 anchor này. Kiểm absence of withdrawn files, links, deterministic generation, retained artwork và recoverable custody; owner vẫn phải duyệt exact current gallery.
...
**Checkpoint D-119:** active candidate `P19B-P18-INHERITED-THREE-MODE-REVIEW-39-1.5.0`. Exact review-38 đã archive 583 files; protected corpus 16364 files khớp hash. `Process` giữ exact D-118 geometry/connector continuity nhưng giảm route/node/focal về `1.0/1.2/1.6`, merge/layer/badge/rule theo contract; đầu mũi tên rõ và hierarchy giữ đúng. 90 non-target HTML/30 preview được bảo toàn; tổng vẫn 93/31 và comparison 107. Focused 11/11, scope 8/8, static 34/34, full regression 394/394 và exact review verification PASS; neutral-light raster đã inspect. Owner approval pending, P-19C unauthorized, G-04 NOT-EVALUATED.
