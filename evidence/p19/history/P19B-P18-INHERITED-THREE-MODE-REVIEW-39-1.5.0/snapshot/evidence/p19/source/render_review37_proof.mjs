...
const source = path.join(root, 'evidence/p19/gallery/previews/type-process.svg');
const out = path.join(root, 'evidence/p19/review37-checks');
fs.mkdirSync(out, { recursive: true });
const svg = fs.readFileSync(source);
fs.writeFileSync(path.join(out, 'type-process.svg'), svg);
...
await sharp(Buffer.from(rasterSvg),{density:120}).png().toFile(path.join(out,'type-process.svg.png'));
