"""D-114 content-fit radar chart with five shared axes and four profiles."""
...
def radar_css(tokens):
    ...
    .rd-profile.series-three{stroke:var(--series-3);fill:var(--series-3);stroke-dasharray:14 7}
    .rd-profile.series-one{stroke:var(--series-1);fill:var(--series-1);stroke-dasharray:4 6}
    .rd-profile.series-four{stroke:var(--series-4);fill:var(--series-4);stroke-dasharray:18 6 4 6}
...
def render_radar(plan):
    ...
    parts = ['<g data-radar-contract="D-114-five-axis-four-profile">']
    ...
    for profile in reversed(layout["profiles"]):
        parts.append(
            ...
            f'data-focal="{str(profile["focal"]).lower()}" data-marker-shape="{profile["marker"]}" points="{_points(profile["points"])}"/>'
        )
        parts.extend(_marker(point, profile) for point in profile["points"])
    parts.append(f'<circle cx="{cx}" cy="{cy}" r="4.5" fill="var(--connector)"/>')
    ...
    parts.append(_text(1894, 1000, "Màu + kiểu nét + hình marker", "rd-note", "end"))
    parts.append("</g>")
    return "".join(parts)
...
def validate_radar_svg(svg):
    ...
    _require(len(spokes) == 5 and len(profiles) == 4 and len(markers) == 24, "Serialized D-114 material count mismatch")
    ...
    _require(len(chart_markers) == 20, "D-114 requires exactly twenty plotted markers")
    ...
    return {"profiles": 4, "values": 20, "axes": 5, "rings": 5, "markers": 20, "focal": 1}
...
def radar_table(plan):
    ...
    return (
        ...
        '<p>Bốn hồ sơ dùng cùng năm tiêu chí và cùng miền tuyến tính 0–10. Hai mươi giá trị được lặp lại dưới dạng bảng; màu, kiểu nét và hình marker cùng mã hóa từng hồ sơ.</p>'
