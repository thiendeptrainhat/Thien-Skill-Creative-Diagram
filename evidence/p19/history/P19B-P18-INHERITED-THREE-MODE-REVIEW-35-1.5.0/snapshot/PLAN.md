...
**Cập nhật:** 2026-08-30  
**Phạm vi được phép trong yêu cầu hiện tại:** D-115: sửa riêng P-19 `radar` để cả bốn profile/legend sample dùng nét liền như ảnh tham chiếu, giữ 5 trục/5 ring/4 profile/20 value, exact P-18 anchor, 90 non-target P-19 artwork/30 preview cùng mọi frozen/release bytes; cập nhật gallery/trang tổng hợp/evidence/handoff.
**Phase đang thực hiện:** P-19B review-35 `in-progress`/owner-review-pending theo D-115, giữ scope D-084–D-114; exact review-34 archived; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
2. `P-19B` — remediation `in-progress` theo D-084–D-115: dùng trực tiếp 14 exact P-18R6 review-17 anchor, không tái sinh duplicate; 25 canonical type, bốn capability và hai presentation variant `layers`/`scatter-chart` có ba mode, tổng 93 HTML/31 preview. Active review-35 giữ toàn bộ remediation trước và sửa riêng `radar` để cả bốn profile dùng solid line; comparison có 107 diagram, 90 non-target HTML/30 preview được bảo toàn và withdrawal custody vẫn khôi phục được.
...
**Verification dự kiến:** theo D-085/D-095–D-115, union 14 P-18 canonical + 25 P-19 canonical phải đúng 39 và không trùng; P-19 có 75 canonical + 12 capability + 3 `layers` + 3 `scatter-chart` variant HTML, 31 preview, đủ ba mode cho 31 identity. Đủ 93 SVG phải khai báo D-105; mọi detailed artifact D-086–D-114 phải được giữ. Radar phải có đúng 5 axis/5 ring/4 profile/20 value/20 plotted marker/1 focal, common domain 0–10, exact 20-row table, bốn solid profile không `stroke-dasharray`, marker/direct-label redundancy và geometry ba mode; scatter-chart/Treemap contract trước vẫn giữ. P-18 phải giữ exact bytes/hash ở neutral-light, không claim ba-mode coverage cho 14 anchor này. Kiểm absence of withdrawn files, links, deterministic generation, retained artwork và recoverable custody; owner vẫn phải duyệt exact current gallery.
...
**Checkpoint D-115:** active candidate `P19B-P18-INHERITED-THREE-MODE-REVIEW-35-1.5.0`. Exact review-34 đã archive 548 files; protected corpus 14117 files khớp hash. Sửa riêng radar line treatment: cả 4 profile và legend sample dùng solid continuous stroke, không còn `stroke-dasharray`; bốn marker shape/direct legend/focal role giữ non-color redundancy. Giữ nguyên 5 axis/5 ring/4 profile/20 value, exact table và geometry ba mode; 90 non-target HTML/30 preview được bảo toàn, tổng vẫn 93/31 và comparison 107. Focused radar 9/9, scope 8/8, static 34/34 và full regression 382/382 PASS; neutral-light raster inspect và exact review verification PASS. Owner approval pending, P-19C unauthorized, G-04 NOT-EVALUATED.
