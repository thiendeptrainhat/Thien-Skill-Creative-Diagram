#!/usr/bin/env python3
"""Generate 90 P-19 specimens and link 14 unchanged approved P-18 anchors."""
...
def main() -> None:
    ...
    if len(sources) != 30:
        raise RuntimeError("D-106 requires 25 canonical fixtures, four capability fixtures, one Layers variant, and the centered-span tree")

    records = []
    full_order = [f"type-{kind}" for kind in CANONICAL_TYPES] + [f"cap-{cap.lower()}-{CAPABILITY_NAMES[cap]}" for cap in P19A_CAPABILITIES] + ["type-layers"]
    for fixture_id, identity, ir in sources:
        ...
        for mode_index, mode in enumerate(MODES):
            ...
            records.append({
                ...
                "parent": "layer-stack" if is_layers else plan["adapter"]["canonical_type"] if plan["adapter"]["capability_id"] else None,
                "presentation_variant_id": "layers" if is_layers else None,
                "mode": mode,
                "layout_engine": plan["adapter"]["layout_engine"],
                "silhouette": "five-band-abstraction-stack" if is_layers else plan["adapter"]["silhouette"],
    ...
    if len(records) != 90:
        raise RuntimeError("P-19B generation did not produce exactly 90 specimens")
    ...
    if len(preview_files) != 30:
        raise RuntimeError("P-19B contact sheet requires exactly 30 neutral-light SVG previews")
    ...
    inventory = {
        ...
        "reuse_authority": "D-084/D-085/D-095/D-096/D-097/D-098/D-099/D-100/D-105/D-106/D-108/D-109/D-110/D-111",
        "capability_count": 4,
        "mode_count": 3,
    ...
    write_json(MANIFEST_PATH, {
        ...
        "note": "D-111: sequence is a detailed four-participant interaction with four centered lifelines, two activations, five straight cross-participant messages and one rounded-orthogonal self-call exception; independently reimplemented from the owner structural rubric in P-18 review-17 visual grammar; retained D-110–D-085 scope; 90 P-19 HTML, no owner approval or P-19C claim.",
    })
