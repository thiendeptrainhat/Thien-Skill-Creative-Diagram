...
**Cập nhật:** 2026-08-30  
**Phạm vi được phép trong yêu cầu hiện tại:** D-111: thay riêng P-19 `sequence` bằng interaction chi tiết, kế thừa exact P-18 visual grammar và D-105 global connector policy; giữ 14 P-18 approved anchor, non-target artwork cùng mọi frozen/release bytes; cập nhật gallery/trang tổng hợp/evidence/handoff.
**Phase đang thực hiện:** P-19B review-31 `in-progress`/owner-review-pending theo D-111, giữ scope D-084–D-110; exact review-30 archived; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
2. `P-19B` — remediation `in-progress` theo D-084–D-111: dùng trực tiếp 14 exact P-18R6 review-17 anchor, không tái sinh duplicate; 25 canonical type, bốn capability và presentation variant `layers` có ba mode, tổng 90 HTML/30 preview. Active review-31 giữ toàn bộ remediation trước, thay riêng `sequence` bằng detailed four-participant interaction có centered lifeline/activation, five straight cross-participant message và một rounded self-call exception; comparison vẫn có 104 diagram, non-target artwork được bảo toàn và withdrawal custody vẫn khôi phục được.
...
**Verification dự kiến:** theo D-085/D-095–D-111, union 14 P-18 canonical + 25 P-19 canonical phải đúng 39 và không trùng; P-19 có 75 canonical + 12 capability + 3 `layers` variant HTML, 30 preview, đủ ba mode cho 30 identity. Đủ 90 SVG phải khai báo D-105; `uml-class` giữ exact centered/even/straight-first proof. `state-machine` phải giữ D-110. `sequence` phải có 4 participant/4 lifeline/2 activation/6 ordered message, 5 straight cross-participant route và 1 documented rounded self-call, 4 card-lifeline center proof; mọi detailed artifact D-086–D-110 phải được giữ. P-18 phải giữ exact bytes/hash ở neutral-light, không claim ba-mode coverage cho 14 anchor này. Kiểm absence of withdrawn files, links, deterministic generation, retained artwork và recoverable custody; owner vẫn phải duyệt exact current gallery.
...
**Checkpoint D-111:** active candidate `P19B-P18-INHERITED-THREE-MODE-REVIEW-31-1.5.0`. Exact review-30 đã archive 501 files; protected corpus 12040 files khớp hash. `sequence` được thay riêng bằng interaction có bốn participant/lifeline, hai activation, năm straight cross-participant message và một rounded-orthogonal self-call exception. Chỉ 3 target HTML + 1 preview đổi artwork; 87 non-target HTML giữ nguyên sau candidate normalization và 29 preview byte-identical. Geometry ba mode và card/lifeline centering phải khớp; neutral-light proof phải inspect. Owner approval pending, P-19C unauthorized, G-04 NOT-EVALUATED.
