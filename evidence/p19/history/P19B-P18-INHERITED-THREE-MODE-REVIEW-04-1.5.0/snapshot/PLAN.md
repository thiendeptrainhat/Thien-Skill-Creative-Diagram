...
**Cập nhật:** 2026-08-29  
**Phạm vi được phép trong yêu cầu hiện tại:** D-083 authorize loop-flywheel-only review-04 với bộ dữ liệu minh họa đã được owner cho phép; tái sinh exact 129 HTML/trang đối chiếu, kiểm focused cycle/shared-state/geometry/static/raster/regression; giữ Gantt D-082, các sửa lỗi D-081 và P-18 visual grammar; không mở P-19C hoặc package/release
**Phase đang thực hiện:** P-19B review-04 `in-progress`/owner-review-pending theo D-083; exact review-03 đã archive trước sửa flywheel; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
4. `P-19A` — 39+4 type/capability adapters, đã hoàn tất theo D-078; `P-19B` — remediation three-mode exact 129 HTML đang `in-progress` theo D-080 sau owner rejection của candidate đầu; `P-19C` — full QA/freeze/owner review, chưa được phép và vẫn cần authorization riêng.
...
2. `P-19B` — remediation `in-progress` theo D-080: candidate đầu theo D-079 đã tạo đúng 129 standalone HTML và qua focused QA nhưng bị owner bác bỏ về hướng thiết kế; bản đó được archive historical. Candidate kế nhiệm phải kế thừa trực tiếp exact P-18R6 review-17 cho neutral-light, derive neutral-dark/editorial bằng semantic role mapping trong cùng visual grammar, giữ geometry/IR bất biến và tái sinh exact 129 HTML + 43 preview + contact sheet trước owner review.
...
**Verification dự kiến:** 39/39 canonical type × ba mode; 4/4 capability mới × ba mode; zero hard failure; gallery file/link/standalone checks pass; owner duyệt exact full gallery/contact sheet.
