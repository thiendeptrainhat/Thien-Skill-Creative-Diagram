...
10. Read [references/visual-system.md](references/visual-system.md) and [references/visual-coverage.md](references/visual-coverage.md). After semantic validation, use `scripts/visual_adapters_v15.py::adapt_visual` to create the locked engine-specific layout plan for any of the 39 canonical types or four v1.5 capabilities. P-19B remediation adds `scripts/gallery_renderer_v15.py::render_gallery_html` and the generated [P-19B renderer registry](references/gallery-renderer-v15.json) for the QA-only exact 129-specimen three-mode successor that inherits exact P-18R6 review-17 visual roles; do not treat that candidate as owner-approved, P-19C-frozen, packaged, or released. Use `scripts/full_renderer.py::render_static` only for the historical 27-type P-07 renderer and `scripts/pilot_renderer.py` only to reproduce approved P-06 pilot evidence.
...
Use [references/variants-v15.md](references/variants-v15.md) for `Dumbbell`, `Slopegraph`, `Ridgeline`, and `Bubble`. Use [references/capability-map.json](references/capability-map.json) for the locked 111-capability parent/selector/fallback/test mapping, [references/semantic-v15-coverage-map.json](references/semantic-v15-coverage-map.json) for the 12+4 P-17 semantic/render disposition, [references/visual-adapters-v15.json](references/visual-adapters-v15.json) for the generated P-19A 39+4 → 14-engine adapter registry, and [references/gallery-renderer-v15.json](references/gallery-renderer-v15.json) for the P-19B three-mode QA renderer binding. The historical 97-specimen inventory remains in [references/specimen-map.json](references/specimen-map.json); historical P-07/P-08/P-11 records remain in their existing coverage maps.

## Respect the current implementation boundary

The common request/router/IR core and semantic validators cover 39 canonical types plus the four P-17 variants. P-19A provides one deterministic engine-specific adapter for each of those 39 types and four capabilities, with unique silhouette declarations and accessible-alternative contracts. Active P-19B candidate `P19B-P18-INHERITED-THREE-MODE-REVIEW-01-1.5.0` provides a scriptless, network-independent exact 129-specimen QA gallery under `evidence/p19/gallery/`, directly bound to the approved P-18R6 review-17 visual parent. It is technically ready for owner visual review but remains `in-progress`; it is not P-19C-frozen or owner-approved and must not be counted as G-04@1.5.0, package, distribution, publication, or release evidence. Historical P-07/P-08 output remains the released v1.0.0 path; `CAP-V15` remains a text-equivalent fallback.
