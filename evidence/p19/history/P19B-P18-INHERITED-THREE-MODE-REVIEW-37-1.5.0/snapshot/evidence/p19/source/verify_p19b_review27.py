#!/usr/bin/env python3
"""D-107 thin-stroke tree and review-26 preservation verification."""
...
OLD = "P19B-P18-INHERITED-THREE-MODE-REVIEW-26-1.5.0"
ARCHIVE = ROOT / "evidence/p19/history" / OLD
GALLERY = ROOT / "evidence/p19/gallery"
PROOFS = ROOT / "evidence/p19/review27-checks"
TARGET_STROKES = {"connector": "1.6", "card": "1.8", "root": "2.2"}
...
def verify():
    ...
    require(inventory["candidate_id"] == P19B_CANDIDATE_ID and len(records) == 90, "Wrong review-27 gallery")
    preserved_html = 0
    ...
    for record in records:
        ...
        if record["fixture_id"] == "type-tree":
            ...
            require(".tree-wire{fill:none;stroke:var(--connector);stroke-width:1.6" in page, "Connector stroke drift")
            require(".tree-card-shape{fill:var(--surface);stroke:var(--connector);stroke-width:1.8" in page, "Card stroke drift")
            require(".tree-card.is-root .tree-card-shape{fill:var(--accent-soft);stroke:var(--accent);stroke-width:2.2" in page, "Root stroke drift")
    ...
    require(all(item == expected for item in target_measurements), "Tree serialized measurements mismatch")
    ...
    require(raster.is_file() and proof_svg.is_file(), "Missing D-107 visual proof")
    return {
        ...
        "stroke_contract": TARGET_STROKES,
        "geometry_vs_review26": "BYTE_EQUIVALENT_GEOMETRY_ELEMENTS",
...
if __name__ == "__main__":
    report = verify()
    output = ROOT / "evidence/p19/P-19B-REVIEW-27-VERIFICATION.json"
