...
**Cập nhật:** 2026-08-30  
**Phạm vi được phép trong yêu cầu hiện tại:** D-117: thay riêng P-19 `process` bằng quy trình độc lập có đủ 5 loại ô nhìn thấy trong ảnh tham chiếu, giữ nguyên template hiện hành, exact P-18 anchor, 90 non-target P-19 artwork/30 preview cùng mọi frozen/release bytes; cập nhật gallery/trang tổng hợp/evidence/handoff.
**Phase đang thực hiện:** P-19B review-37 `in-progress`/owner-review-pending theo D-117, giữ scope D-084–D-116; exact review-36 archived; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
2. `P-19B` — remediation `in-progress` theo D-084–D-117: dùng trực tiếp 14 exact P-18R6 review-17 anchor, không tái sinh duplicate; 25 canonical type, bốn capability và hai presentation variant `layers`/`scatter-chart` có ba mode, tổng 93 HTML/31 preview. Active review-37 giữ toàn bộ remediation trước và thay riêng `process` bằng quy trình chi tiết đủ năm loại ô; comparison có 107 diagram, 90 non-target HTML/30 preview được bảo toàn.
...
**Verification dự kiến:** theo D-085/D-095–D-117, union 14 P-18 canonical + 25 P-19 canonical phải đúng 39 và không trùng; P-19 có 75 canonical + 12 capability + 3 `layers` + 3 `scatter-chart` variant HTML, 31 preview, đủ ba mode cho 31 identity. Đủ 93 SVG phải khai báo D-105; mọi detailed artifact D-086–D-116 phải được giữ. `Process` phải có đúng 3 terminator/4 process/2 decision/1 document/1 multiple-document và 11 directed straight route; single attachment centered, hai inlet cùng cạnh chia đều quanh tâm, exact node/edge table và geometry ba mode. Radar/scatter-chart/Treemap contract trước vẫn giữ. P-18 phải giữ exact bytes/hash ở neutral-light, không claim ba-mode coverage cho 14 anchor này. Kiểm absence of withdrawn files, links, deterministic generation, retained artwork và recoverable custody; owner vẫn phải duyệt exact current gallery.
...
**Checkpoint D-117:** active candidate `P19B-P18-INHERITED-THREE-MODE-REVIEW-37-1.5.0`. Exact review-36 đã archive 564 files; protected corpus 15223 files khớp hash. `Process` có đúng 11 node gồm 3 terminator/4 process/2 decision/1 document/1 multiple-document và 11 directed straight route; single attachment centered, hai inlet cùng cạnh chia đều quanh tâm, exact node/edge table và ba mode cùng geometry. Giữ 90 non-target HTML/30 preview; tổng vẫn 93/31 và comparison 107. Focused 10/10, scope 8/8, static 34/34, regression 393/393 và exact review verification PASS; neutral-light raster đã inspect. Owner approval pending, P-19C unauthorized, G-04 NOT-EVALUATED.
