...
P19B_CANDIDATE_ID = "P19B-P18-INHERITED-THREE-MODE-REVIEW-08-1.5.0"
...
def validate_target_geometry(svg: str, diagram_type: str) -> dict[str, Any]:
    ...
    if diagram_type not in ("dp-integration", "swimlane", "bar-chart"):
        return {}
...
def render_gallery_html(ir_value: Mapping[str, Any], mode: str, fixture_id: str, *, connector_corner_style: str | None = None) -> str:
    ...
    takeaway = _text(description) if adapter["canonical_type"] in ("gantt", "loop-flywheel", "dp-integration", "bar-chart") and (adapter["canonical_type"] != "dp-integration" or is_detailed_dp_integration(plan)) and (adapter["canonical_type"] != "bar-chart" or is_detailed_bar_chart(plan)) else f"{_text(adapter['semantic_focus'].capitalize())}. {_text(adapter['accessible_alternative'].capitalize())}."
