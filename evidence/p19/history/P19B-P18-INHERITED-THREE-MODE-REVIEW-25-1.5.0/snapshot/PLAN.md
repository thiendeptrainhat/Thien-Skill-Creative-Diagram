...
**Cập nhật:** 2026-08-30  
**Phạm vi được phép trong yêu cầu hiện tại:** D-105: áp dụng connector policy căn giữa/chia đều/ưu tiên đường thẳng cho mọi P-19 SVG và sửa exact UML proof; giữ 14 P-18 approved anchor, non-target artwork cùng mọi frozen/release bytes; cập nhật gallery/trang tổng hợp/evidence/handoff.
**Phase đang thực hiện:** P-19B review-25 `in-progress`/owner-review-pending theo D-105, giữ scope D-084–D-104; exact review-24 archived; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
4. `P-19A` — 39+4 type/capability adapters, đã hoàn tất theo D-078; `P-19B` — remediation theo D-084–D-105 đang `in-progress`, kết hợp 14 P-18 anchor với 90 P-19 HTML sau owner rejection của candidate đầu; `P-19C` — full QA/freeze/owner review, chưa được phép và vẫn cần authorization riêng.
...
2. `P-19B` — remediation `in-progress` theo D-084–D-105: dùng trực tiếp 14 exact P-18R6 review-17 anchor, không tái sinh duplicate; 25 canonical type, bốn capability và presentation variant `layers` có ba mode, tổng 90 HTML/30 preview. Active review-25 giữ toàn bộ D-086–D-104, áp dụng global centered/even straight-first connector contract và sửa exact UML proof; comparison vẫn có 104 diagram, non-target artwork được bảo toàn sau policy normalization và withdrawal custody vẫn khôi phục được.
...
**Verification dự kiến:** theo D-085/D-095–D-105, union 14 P-18 canonical + 25 P-19 canonical phải đúng 39 và không trùng; P-19 có 75 canonical + 12 capability + 3 `layers` variant HTML, 30 preview, đủ ba mode cho 30 identity. Đủ 90 SVG phải khai báo D-105; `uml-class` phải có exact centered single port, equal multi-port intervals, straight-first routing, 7 container/17 member/5 continuous relationship, 4 cardinality inline, legend đủ 6 loại và geometry ba mode; mọi detailed artifact D-086–D-104 phải được giữ. P-18 phải giữ exact bytes/hash ở neutral-light, không claim ba-mode coverage cho 14 anchor này. Kiểm absence of withdrawn files, links, deterministic generation, retained artwork và recoverable custody; owner vẫn phải duyệt exact current gallery.
