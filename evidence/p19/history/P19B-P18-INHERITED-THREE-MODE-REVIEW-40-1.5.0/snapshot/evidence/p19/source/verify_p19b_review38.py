#!/usr/bin/env python3
"""Verify D-118 process connector continuity and exact review-37 preservation."""
import hashlib
import json
import re
import sys
...
from process_layout_v15 import EXPECTED_SHAPES, layout_process, process_table, validate_process_svg
from process_review37_fixture import process_fixture
from visual_adapters_v15 import adapt_visual

OLD = "P19B-P18-INHERITED-THREE-MODE-REVIEW-37-1.5.0"
ARCHIVE = ROOT / "evidence/p19/history" / OLD
GALLERY = ROOT / "evidence/p19/gallery"
PROOFS = ROOT / "evidence/p19/review38-checks"
...
def verify():
    ...
    require(inventory["candidate_id"] == P19B_CANDIDATE_ID and len(records) == 93, "Wrong review-38 gallery")
    ...
    for record in records:
        ...
        if record["fixture_id"] == "type-process":
            for token in (
                'data-process-contract="D-118-continuous-document-connectors"',
                'data-template-contract="p18r6-review17-preserved"',
                'data-route-kind="rounded-orthogonal"',
                'data-route-exception="owner-requested-document-merge-clarity"',
                'marker-end="url(#pr-arrow-merge)"',
                "Luồng thẳng / vuông góc bo tròn",
            ):
                require(token in page, f"Process connector contract drift: {current.name}")
    ...
    require(all(item == expected for item in measurements), "Process connector measurement mismatch")
    ...
    inlet_x = sorted(item["end"][0] for item in merge)
    ...
    require(raster.is_file() and proof_svg.is_file(), "Missing D-118 visual proof")
    return {
        ...
        "connector_policy": "nine straight routes; two symmetric rounded-orthogonal merge routes; five continuous document-boundary contacts",
...
if __name__ == "__main__":
    report = verify()
    output = ROOT / "evidence/p19/P-19B-REVIEW-38-VERIFICATION.json"
