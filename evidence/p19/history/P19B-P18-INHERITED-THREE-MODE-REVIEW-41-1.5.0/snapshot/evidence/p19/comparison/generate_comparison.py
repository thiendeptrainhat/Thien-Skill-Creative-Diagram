...
def generate():
    ...
    for anchor in anchors:
        ...
        for (_, fixture), modes in engine_groups:
            ...
            name = first["identity"] if not first["capability_id"] or first["capability_id"] in {"CAP-V18", "CAP-V20"} else fixture.removeprefix("cap-").upper()
...
<h1>P-18 ↔ P-19<br>Toàn bộ diagram, cùng một trang.</h1>
    ...
    <p class="note">Theo D-084/D-085/D-095–D-121: <strong>giữ nguyên 14 bản P-18, bỏ các bản P-19 trùng loại, giữ mọi remediation trước; đổi display identity CAP-V18 thành slope-graph và dùng đúng biểu đồ bảy chuỗi/đúng hai trạng thái/direct labels/rank-crossing trong template hiện hành</strong>. P-19 có 25 loại + 4 capability + 2 presentation variant, mỗi identity đủ 3 mode; P-18 chỉ dùng neutral-light đã duyệt, không tự tạo thêm màu.</p>
...
<a href="{link(P18 / 'index.html')}" target="_blank" rel="noopener">Mở gallery P-18 ↗</a></div>
    <div><strong>P-19B · review-41 · Chờ owner review</strong><code>{html.escape(b['candidate_id'])}</code>
