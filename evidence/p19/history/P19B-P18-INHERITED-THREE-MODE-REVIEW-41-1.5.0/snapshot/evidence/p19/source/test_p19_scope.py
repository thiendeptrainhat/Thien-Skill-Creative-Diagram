...
from verify_p19b_review41 import verify
...
class ReuseScopeTests(unittest.TestCase):
    ...
    def test_capabilities_and_recent_fixes_retained(self):
        ...
        for name in ('gantt', 'loop-flywheel', 'fishbone', 'dp-integration', 'bar-chart', 'line-chart', 'medallion', 'polar-chart', 'CAP-V17', 'slope-graph', 'CAP-V19', 'bubble'):
    ...
    def test_exact_preservation_and_recoverable_withdrawal(self):
        ...
        self.assertEqual(report['slope_graph_measurement']['series'], 7)
        self.assertEqual(report['slope_graph_measurement']['endpoints'], 14)
        self.assertEqual(report['slope_graph_measurement']['focal'], 1)
        self.assertEqual(report['display_identity'], 'slope-graph')
        self.assertEqual(report['internal_capability_id'], 'CAP-V18')
