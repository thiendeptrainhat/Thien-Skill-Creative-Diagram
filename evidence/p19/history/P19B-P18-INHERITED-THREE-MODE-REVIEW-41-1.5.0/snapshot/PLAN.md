...
**Cập nhật:** 2026-08-30  
**Phạm vi được phép trong yêu cầu hiện tại:** D-121: đổi identity hiển thị `CAP-V18` thành `slope-graph` và thay riêng artwork bằng bảy series/đúng hai trạng thái/14 direct-labeled endpoint/cùng thang 0–100%/direction-rank-crossing; bảo đảm cả 7 line nét liền còn phân biệt rõ ở tỷ lệ comparison (`3.4` thường, `4.5` focal); giữ internal ID `CAP-V18`, parent `line-chart`, template hiện hành, exact P-18 anchor, 90 non-target P-19 artwork/30 preview cùng mọi frozen/release bytes; cập nhật gallery/trang tổng hợp/evidence/handoff.
**Phase đang thực hiện:** P-19B review-41 `in-progress`/owner-review-pending theo D-121, giữ scope D-084–D-120; exact review-40 archived; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
2. `P-19B` — remediation `in-progress` theo D-084–D-121: dùng trực tiếp 14 exact P-18R6 review-17 anchor, không tái sinh duplicate; 25 canonical type, bốn capability và hai presentation variant `layers`/`scatter-chart` có ba mode, tổng 93 HTML/31 preview. Active review-41 giữ toàn bộ remediation trước, đổi identity hiển thị CAP-V18 thành `slope-graph` và thay riêng slope artwork; comparison có 107 diagram, 90 non-target HTML/30 preview được bảo toàn.
...
**Verification dự kiến:** theo D-085/D-095–D-121, union 14 P-18 canonical + 25 P-19 canonical phải đúng 39 và không trùng; P-19 có 75 canonical + 12 capability + 3 `layers` + 3 `scatter-chart` variant HTML, 31 preview, đủ ba mode cho 31 identity. Đủ 93 SVG phải khai báo D-105; mọi detailed artifact D-086–D-120 phải được giữ. `slope-graph` phải giữ internal `CAP-V18`, parent `line-chart`, đúng 7 series/14 endpoint/2 state axis/6 tick/5 rise/2 fall/9 crossing/1 focal, exact seven-row table và cùng thang 0–100%; ba mode cùng geometry. Bubble/process/radar/scatter-chart/Treemap contract trước vẫn giữ. P-18 phải giữ exact bytes/hash ở neutral-light, không claim ba-mode coverage cho 14 anchor này. Kiểm absence of withdrawn files, links, deterministic generation, retained artwork và recoverable custody; owner vẫn phải duyệt exact current gallery.
...
**Checkpoint D-121:** active candidate `P19B-P18-INHERITED-THREE-MODE-REVIEW-41-1.5.0`. Exact review-40 đã archive 602 files; protected corpus 17540 files khớp hash. Display identity là `slope-graph`, internal capability ID vẫn `CAP-V18`, canonical parent vẫn `line-chart`. Artwork có đúng 7 series/14 endpoint/2 arrow-free state axis/6 tick/5 rise/2 fall/9 crossing/1 focal; cả 7 line là nét liền và dùng hierarchy `3.4` thường/`4.5` focal để không biến mất khi comparison thu nhỏ. Browser-token defect đã được sửa: bảy stroke hiện chỉ bind vào token template đã khai báo; verifier cấm bốn token thiếu `--blue`/`--green`/`--amber`/`--plum`. Exact seven-row table và geometry ba mode khớp. 90 non-target HTML/30 preview được bảo toàn; tổng vẫn 93/31 và comparison 107. Focused/exact verification PASS; neutral-light raster 2000×980 đã inspect. Owner approval pending, P-19C unauthorized, G-04 NOT-EVALUATED.
