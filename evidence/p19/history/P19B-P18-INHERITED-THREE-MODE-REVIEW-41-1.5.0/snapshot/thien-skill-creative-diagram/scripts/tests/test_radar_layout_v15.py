...
from radar_layout_v15 import layout_radar, radar_css, radar_table, render_radar, validate_radar_svg
...
class RadarLayoutTests(unittest.TestCase):
    ...
    def test_all_profile_lines_are_solid(self):
        self.assertNotIn("stroke-dasharray", radar_css({}))
        rendered = render_radar(self.plan)
        self.assertEqual(rendered.count('data-line-style="solid"'), 4)

    def test_plot_has_no_markers(self):
        rendered = render_radar(self.plan)
        self.assertEqual(rendered.count('data-marker-series='), 4)
        self.assertEqual(rendered.count('data-datum-id="legend-'), 4)
        self.assertEqual(rendered.count("<circle"), 1)  # legend circle only; page wrapper adds the canvas pattern
    ...
    def test_serialized_contract(self):
        ...
        self.assertEqual(report, {"profiles": 4, "values": 20, "axes": 5, "rings": 5, "markers": 0, "focal": 1})
