...
**Cập nhật:** 2026-08-30  
**Phạm vi được phép trong yêu cầu hiện tại:** D-104: thay riêng P-19 `uml-class` bằng seven-container/five-relation typed UML model có interface, member compartments, inline cardinality, continuous connector và legend đủ sáu loại quan hệ; giữ 14 P-18 approved anchor, 87 non-target P-19 HTML/29 non-target preview và mọi frozen/release bytes; cập nhật gallery/trang tổng hợp/evidence/handoff.
**Phase đang thực hiện:** P-19B review-24 `in-progress`/owner-review-pending theo D-104, giữ scope D-084–D-103; exact review-23 archived; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
4. `P-19A` — 39+4 type/capability adapters, đã hoàn tất theo D-078; `P-19B` — remediation theo D-084–D-104 đang `in-progress`, kết hợp 14 P-18 anchor với 90 P-19 HTML sau owner rejection của candidate đầu; `P-19C` — full QA/freeze/owner review, chưa được phép và vẫn cần authorization riêng.
...
2. `P-19B` — remediation `in-progress` theo D-084–D-104: dùng trực tiếp 14 exact P-18R6 review-17 anchor, không tái sinh duplicate; 25 canonical type, bốn capability và presentation variant `layers` có ba mode, tổng 90 HTML/30 preview. Active review-24 giữ toàn bộ D-086–D-103 và thay riêng `uml-class` bằng seven-container/five-relation typed UML model; comparison vẫn có 104 diagram, 87 non-target artwork được bảo toàn và withdrawal custody vẫn khôi phục được.
...
**Verification dự kiến:** theo D-085/D-095–D-104, union 14 P-18 canonical + 25 P-19 canonical phải đúng 39 và không trùng; P-19 có 75 canonical + 12 capability + 3 `layers` variant HTML, 30 preview, đủ ba mode cho 30 identity. `uml-class` phải có đúng 7 container/17 member/5 continuous relationship, quan hệ 1 dependency + 2 realization + 1 composition + 1 association, 4 cardinality inline, legend đủ 6 loại và geometry ba mode; mọi detailed artifact D-086–D-103 phải được giữ. P-18 phải giữ exact bytes/hash ở neutral-light, không claim ba-mode coverage cho 14 anchor này. Kiểm absence of withdrawn files, links, deterministic generation, retained artwork và recoverable custody; owner vẫn phải duyệt exact current gallery.
