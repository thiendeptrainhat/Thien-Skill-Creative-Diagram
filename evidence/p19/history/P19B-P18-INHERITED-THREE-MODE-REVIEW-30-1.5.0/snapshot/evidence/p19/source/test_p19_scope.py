...
from verify_p19b_review30 import verify
...
class ReuseScopeTests(unittest.TestCase):
    ...
    def test_exact_preservation_and_recoverable_withdrawal(self):
        ...
        self.assertEqual(report['state_machine_measurement']['states'], 4)
        self.assertEqual(report['state_machine_measurement']['initial_markers'], 1)
        self.assertEqual(report['state_machine_measurement']['terminal_markers'], 1)
        self.assertEqual(report['state_machine_measurement']['straight_transitions'], 5)
        self.assertEqual(report['state_machine_measurement']['return_transitions'], 1)
        self.assertEqual(report['state_machine_measurement']['centered_attachments'], 12)
