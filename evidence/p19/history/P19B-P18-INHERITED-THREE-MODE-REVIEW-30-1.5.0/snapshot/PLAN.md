...
**Cập nhật:** 2026-08-30  
**Phạm vi được phép trong yêu cầu hiện tại:** D-110: thay riêng P-19 `state-machine` bằng lifecycle chi tiết, kế thừa exact P-18 visual grammar và D-105 global connector policy; giữ 14 P-18 approved anchor, non-target artwork cùng mọi frozen/release bytes; cập nhật gallery/trang tổng hợp/evidence/handoff.
**Phase đang thực hiện:** P-19B review-30 `in-progress`/owner-review-pending theo D-110, giữ scope D-084–D-109; exact review-29 archived; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
2. `P-19B` — remediation `in-progress` theo D-084–D-110: dùng trực tiếp 14 exact P-18R6 review-17 anchor, không tái sinh duplicate; 25 canonical type, bốn capability và presentation variant `layers` có ba mode, tổng 90 HTML/30 preview. Active review-30 giữ toàn bộ remediation trước, thay riêng `state-machine` bằng detailed four-state lifecycle có centered straight-first connector và một rounded return-route exception; comparison vẫn có 104 diagram, non-target artwork được bảo toàn và withdrawal custody vẫn khôi phục được.
...
**Verification dự kiến:** theo D-085/D-095–D-110, union 14 P-18 canonical + 25 P-19 canonical phải đúng 39 và không trùng; P-19 có 75 canonical + 12 capability + 3 `layers` variant HTML, 30 preview, đủ ba mode cho 30 identity. Đủ 90 SVG phải khai báo D-105; `uml-class` giữ exact centered/even/straight-first proof. `state-machine` phải có 4 state/6 transition/1 initial/1 terminal, 5 straight route và 1 documented rounded return route, đủ 12 centered attachment; mọi detailed artifact D-086–D-109 phải được giữ. P-18 phải giữ exact bytes/hash ở neutral-light, không claim ba-mode coverage cho 14 anchor này. Kiểm absence of withdrawn files, links, deterministic generation, retained artwork và recoverable custody; owner vẫn phải duyệt exact current gallery.
...
**Checkpoint D-110:** active candidate `P19B-P18-INHERITED-THREE-MODE-REVIEW-30-1.5.0`. Exact review-29 đã archive 489 files; protected corpus 11550 files khớp hash. `state-machine` được thay riêng bằng lifecycle có một initial marker, bốn state card, một terminal marker, năm straight transition và một rounded-orthogonal dashed return transition; đủ 12 centered attachment theo D-105. Chỉ 3 target HTML + 1 preview đổi artwork; 87 non-target HTML giữ nguyên sau candidate normalization và 29 preview byte-identical. Geometry ba mode khớp; neutral-light proof 2000×980 đã inspect. Focused 3/3, scope 8/8, static 34/34, full regression 363/363 và review verification PASS. Browser `BLOCKED_URL_POLICY`, owner approval pending, P-19C unauthorized, G-04 NOT-EVALUATED.
