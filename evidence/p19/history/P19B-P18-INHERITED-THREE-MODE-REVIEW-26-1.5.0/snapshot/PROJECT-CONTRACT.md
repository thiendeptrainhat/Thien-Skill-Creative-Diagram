...
**Ngày cập nhật candidate:** 2026-08-30  
...
- Gallery hiện hành theo D-084–D-096 dùng trực tiếp 14 anchor P-18 review-17 đã duyệt ở `neutral-light`, không tạo P-19 trùng loại. P-19 sinh 25 canonical type × ba mode = 75 HTML, bốn capability × ba mode = 12 HTML và presentation variant `layers` của canonical parent `layer-stack` × ba mode = 3 HTML, tổng 90 specimen; trang tổng hợp có 14 + 90 = 104 diagram. D-086–D-095 giữ các remediation/addition chi tiết; D-096 thay riêng artwork `line-chart` mà không sửa taxonomy/semantic grammar/P-19A frozen. Yêu cầu 129 HTML P-19 trước đây là historical; taxonomy/semantic source vẫn 39 type + bốn capability.
...
| D-094 | Ngày 2026-08-29, chủ sở hữu yêu cầu P-19 `kanban` theo board bốn cột của ảnh tham chiếu. Cho phép P-19B review-14 sửa riêng Kanban bằng dữ liệu/geometry độc lập: đúng bốn cột Tồn đọng/Đang thực hiện/Rà soát/Hoàn tất, 11 work item theo phân bố 3/4/2/2, một cột vượt WIP hiển thị `4/3`, một blocked card, một waiting-external card và hai done card. Trạng thái không được truyền bằng màu đơn độc: blocked/waiting dùng dashed boundary khác nhau, blocked có coral rail, done có fill/stroke riêng và legend chữ; mọi card nằm trọn trong cột sở hữu, alternative table phải đủ cột/item/WIP/state. Vì P-17 grammar frozen từ chối group vượt structured `wip_limit`, giới hạn vận hành 3 phải được khai báo bằng annotation có semantic target trực tiếp tới cột, không sửa frozen grammar/P-19A source. Hình tham khảo chỉ là dữ liệu/rubric, không sao chép prose/product/exact coordinate/CSS/SVG/template/asset. Archive exact review-13 trước mutation; chứng minh ba mode cùng geometry, 84 non-target HTML giữ artwork ngoài candidate metadata và 28 preview byte-identical. Không mở P-19C hoặc package/dist/publication/Git/Release; owner approval review-14 vẫn riêng. | LOCKED |
| D-095 | Ngày 2026-08-29, chủ sở hữu xác nhận gallery hiện chưa có diagram mang identity `layers` như ảnh tham chiếu và yêu cầu bổ sung. Cho phép P-19B review-15 thêm một presentation variant `layers` dưới canonical parent `layer-stack`, không thêm canonical type, capability hoặc sửa P-17 grammar/P-19A adapter registry frozen. Variant phải có đúng năm dải ngang liên tục L5→L1, một trục từ nền tảng lên trừu tượng, mỗi dải có level/title/scope trực tiếp, đúng một focal layer được encode dư thừa bằng coral boundary/fill, nhãn `TRỌNG TÂM` và note chữ; ba mode dùng cùng geometry và có alternative table đủ năm layer. Nội dung tiếng Việt, semantic data, code, CSS và geometry phải được viết độc lập; ảnh chỉ là rubric về ordered bands/axis/focal emphasis, không sao chép prose/product/exact coordinates/CSS/SVG/template/asset. Archive exact review-14 trước mutation; giữ nguyên 87 HTML và 29 preview hiện hữu ngoài candidate metadata, thêm đúng 3 HTML + 1 preview `layers`, nâng P-19 lên 90 HTML/30 preview và comparison lên 104 diagram. Không mở P-19C hoặc package/dist/publication/Git/Release; owner approval review-15 vẫn riêng. | LOCKED |
| D-096 | Ngày 2026-08-30, chủ sở hữu yêu cầu P-19 `line-chart` chi tiết như ảnh tham chiếu. Cho phép P-19B review-16 sửa riêng artwork và fixture minh họa của `line-chart`: đúng ba series dùng chung tám tuần có thứ tự, 24 giá trị exact, x-axis ordinal, y-axis tuyến tính 0–240 `lượt` với sáu tick 40→240 và cả hai axis không có arrow; một focal series dùng coral line + circle markers + restrained area fill + direct endpoint label, hai comparison series phải phân biệt không chỉ bằng màu mà còn bằng long-dash/square và dot-dash/diamond; legend và alternative table phải chứa đủ 24 giá trị. Ba mode dùng cùng geometry. Ảnh chỉ là rubric về multi-series trend/axis/focal-area/legend, không sao chép exact data/prose/coordinates/CSS/SVG/template/asset. Archive exact review-15 trước mutation; chỉ ba `type-line-chart` HTML và một preview được đổi artwork, 87 non-target HTML giữ nguyên sau candidate-ID normalization và 29 non-target preview byte-identical; tổng vẫn 90 P-19 HTML/30 preview và 104 diagram comparison. Không sửa P-17/P-19A/P-18/package/`dist`/publication/Git/Release, không mở P-19C; owner approval review-16 vẫn riêng. | LOCKED |
| D-097 | Ngày 2026-08-30, chủ sở hữu yêu cầu thay đổi P-19 `medallion` theo ảnh tham chiếu. Cho phép P-19B review-17 sửa riêng artwork và fixture minh họa của `medallion` thành vòng đời độc lập gồm đúng năm stage có thứ tự `tiếp nhận → ẩn danh → chuẩn hóa → tổng hợp → lưu trữ`, bốn promotion arc có hướng liền mạch giữa stage kề nhau và hai processing-path callout cho SQL lặp lại cùng notebook khám phá. Mỗi stage phải hiển thị trực tiếp tên kỹ thuật, công cụ, định dạng, chủ trì và hai ví dụ; chỉ stage tổng hợp là focal, chỉ stage cuối là archive, cả hai có non-color tag/boundary redundancy. Ba mode dùng cùng geometry và alternative table đủ năm stage. Ảnh chỉ là rubric về tall-card lifecycle/promotion arc/focal/archive/detail/path-callout, không sao chép prose/data/product/exact coordinates/CSS/SVG/template/asset. Archive exact review-16 trước mutation; chỉ ba `type-medallion` HTML và một preview được đổi artwork, 87 non-target HTML giữ nguyên sau candidate-ID normalization và 29 non-target preview byte-identical; tổng vẫn 90 P-19 HTML/30 preview và 104 diagram comparison. Không sửa P-17/P-19A/P-18/package/`dist`/publication/Git/Release, không mở P-19C; owner approval review-17 vẫn riêng. | LOCKED |
| D-098 | Ngày 2026-08-30, chủ sở hữu yêu cầu P-19 `polar-chart` phải theo cấu trúc chi tiết của ảnh tham chiếu. Cho phép P-19B review-18 sửa riêng artwork và fixture minh họa của `polar-chart`: đúng một series với tám cửa sổ UTC có thứ tự, tám tia thẳng chung tâm, bán kính tuyến tính 0–100%, năm vòng 20/40/60/80/100, nhãn cửa sổ và giá trị trực tiếp, marker tròn rỗng tại mỗi endpoint, đúng một đỉnh 100%. Đỉnh phải được encode dư thừa bằng coral, nét dày, marker viền, nhãn `ĐỈNH` và role trong exact eight-row alternative table; tia/trục không có arrowhead, không dùng filled wedge. Ba mode dùng cùng geometry. Ảnh chỉ là rubric về radial-spoke composition/scale/direct label/focal maximum, không sao chép exact data/prose/coordinates/CSS/SVG/template/asset. Archive exact review-17 trước mutation; chỉ ba `type-polar-chart` HTML và một preview được đổi artwork, 87 non-target HTML giữ nguyên sau candidate-ID normalization và 29 non-target preview byte-identical; tổng vẫn 90 P-19 HTML/30 preview và 104 diagram comparison. Không sửa P-17/P-19A/P-18/package/`dist`/publication/Git/Release, không mở P-19C; owner approval review-18 vẫn riêng. | LOCKED |
| D-099 | Ngày 2026-08-30, chủ sở hữu yêu cầu thay P-19 `wardley-map` theo cấu trúc của ảnh tham chiếu. Cho phép P-19B review-19 sửa riêng artwork và fixture minh họa của `wardley-map` thành một bản đồ độc lập gồm đúng tám component, chín dependency, hai trục chuẩn hóa `0–1` cho visibility/evolution, bốn stage `Khởi nguyên / Tự xây dựng / Sản phẩm / Hàng hóa` và ba stage boundary nét đứt. Dependency và hai axis phải là plain line không arrowhead; đúng một component `Điều phối tác vụ` là evolving, được encode dư thừa bằng coral open-circle, direct state label và một coral dashed evolution arrow. Tất cả component dùng direct label; alternative table duy nhất phải chứa exact tám component và chín dependency; ba mode dùng cùng geometry. Ảnh chỉ là rubric về Wardley axes/stages/open-node/dependency/evolution signal, không sao chép exact data/prose/coordinates/CSS/SVG/template/asset. Archive exact review-18 trước mutation; chỉ ba `type-wardley-map` HTML và một preview được đổi artwork, 87 non-target HTML giữ nguyên sau candidate-ID normalization và 29 non-target preview byte-identical; tổng vẫn 90 P-19 HTML/30 preview và 104 diagram comparison. Không sửa P-17/P-19A/P-18/package/`dist`/publication/Git/Release, không mở P-19C; owner approval review-19 vẫn riêng. | LOCKED |
| D-100 | Ngày 2026-08-30, chủ sở hữu yêu cầu thay P-19 `venn` theo cấu trúc ba vòng giao nhau của ảnh tham chiếu. Cho phép P-19B review-20 sửa riêng artwork và fixture minh họa của `venn`: đúng ba tập equal-radius `Đáng mong muốn / Khả thi / Bền vững`, bốn semantic member, mỗi tập có một member riêng và cùng chứa duy nhất member `Sẵn sàng triển khai`; vùng giao ba tập phải là exact geometric intersection bằng nested clip, không dùng shape ước lượng. Ba tập và vùng giao dùng direct title/subtitle; vùng giao có direct non-color label `ĐIỂM CÂN BẰNG`; một exact alternative membership table phải chứa đủ bốn member; ba mode dùng cùng SVG geometry. Ảnh chỉ là rubric về equal-circle balance, label hierarchy và triple-intersection emphasis, không sao chép exact prose/data/coordinates/CSS/SVG/template/asset. Archive exact review-19 trước mutation; chỉ ba `type-venn` HTML và một preview được đổi artwork, 87 non-target HTML giữ nguyên sau candidate-ID normalization và 29 non-target preview byte-identical; tổng vẫn 90 P-19 HTML/30 preview và 104 diagram comparison. Không sửa P-17/P-19A/P-18/package/`dist`/publication/Git/Release, không mở P-19C; owner approval review-20 vẫn riêng. | LOCKED |
| D-101 | Ngày 2026-08-30, chủ sở hữu yêu cầu chuyển P-19 `treemap` theo cấu trúc phân bổ diện tích của ảnh tham chiếu. Cho phép P-19B review-21 sửa riêng artwork và fixture minh họa của `treemap`: đúng sáu leaf châu lục dưới một group định lượng, mọi giá trị dương và reconcile về exact parent total; diện tích từng rectangle phải tỷ lệ trực tiếp với value, không dùng kích thước ước lượng. Châu Á là duy nhất focal tile và phải được encode dư thừa bằng diện tích, coral outline, soft fill và direct label; năm tile còn lại dùng direct label trừ Châu Đại Dương là duy nhất compact-label tile có ký hiệu `i` cùng exact disclosure ở legend/table. Alternative table phải chứa đủ sáu value/unit/share; ba mode dùng cùng SVG geometry. Ảnh chỉ là rubric về asymmetric tiling, area hierarchy, focal emphasis, small-tile disclosure và footer legend; dữ liệu hiển thị được lấy từ ảnh owner như illustrative rounded input nhưng code, prose tiếng Việt, CSS, SVG và coordinates được viết độc lập. Archive exact review-20 trước mutation; chỉ ba `type-treemap` HTML và một preview được đổi artwork, 87 non-target HTML giữ nguyên sau candidate-ID normalization và 29 non-target preview byte-identical; tổng vẫn 90 P-19 HTML/30 preview và 104 diagram comparison. Không sửa P-17/P-19A/P-18/package/`dist`/publication/Git/Release, không mở P-19C; owner approval review-21 vẫn riêng. | LOCKED |
| D-102 | Ngày 2026-08-30, chủ sở hữu yêu cầu mọi tile của P-19 `treemap` phải có viền nhìn thấy như ảnh tham chiếu. Cho phép P-19B review-22 sửa riêng paint treatment của ba `type-treemap` mode: giữ nguyên toàn bộ geometry/value/area/label/legend/table D-101, dùng canvas-colored under-stroke để duy trì khe tách và connector-colored outline cho cả năm tile thường; tile Châu Á tiếp tục dùng coral outline riêng. Viền phải hiện diện bằng stroke thực, không chỉ dựa vào chênh lệch fill; sáu tile và sáu gutter phải kiểm chứng được ở cả ba mode. Ảnh chỉ là rubric về visible tile boundary; không sao chép prose, CSS, SVG, coordinates hoặc asset. Archive exact review-21 trước mutation; chỉ ba Treemap HTML và một preview đổi artwork, 87 non-target HTML giữ nguyên sau candidate-ID normalization và 29 non-target preview byte-identical. Không sửa P-17/P-19A/P-18/package/`dist`/publication/Git/Release, không mở P-19C; owner approval review-22 vẫn riêng. | LOCKED |
| D-103 | Ngày 2026-08-30, chủ sở hữu xác nhận review-22 chưa đạt vì các cạnh viền chưa hiện đủ và khoảng cách giữa các vùng quá hẹp. Cho phép P-19B review-23 sửa riêng geometry hiển thị của ba `type-treemap` mode: retire canvas under-stroke, giữ exact allocation rectangles D-101 làm quantitative metadata, inset mỗi visible rectangle đúng 4 SVG unit ở cả bốn cạnh để tạo real inter-tile gap 8 unit tại mọi shared boundary. Mỗi visible rectangle phải có đủ `top/right/bottom/left` outline; năm tile thường dùng connector stroke, Châu Á dùng coral stroke. Gap/inset phải đồng nhất ở cả ba mode; allocation-area/value reconciliation, total, labels, legend và exact table không đổi. Ảnh chỉ là rubric về complete four-edge boundaries và uniform physical spacing; không sao chép prose, CSS, SVG, coordinates, template hoặc asset. Archive exact review-22 trước mutation; chỉ ba Treemap HTML và một preview đổi artwork, 87 non-target HTML giữ nguyên sau candidate-ID normalization và 29 non-target preview byte-identical. Không sửa P-17/P-19A/P-18/package/`dist`/publication/Git/Release, không mở P-19C; owner approval review-23 vẫn riêng. | LOCKED |
| D-104 | Ngày 2026-08-30, chủ sở hữu yêu cầu thay riêng `uml-class` P-19 theo cấu trúc chi tiết của ảnh tham chiếu. Cho phép P-19B review-24 tạo independent seven-container UML model gồm một service class, một focal interface có stereotype trực tiếp, hai implementation, ba domain class; đúng năm quan hệ semantic gồm 1 dependency, 2 realization, 1 composition và 1 association. Composition/association phải có cardinality inline; association dùng một continuous rounded-orthogonal path theo quy tắc connector P-18. Legend phải biểu diễn đủ inheritance, realization, composition, aggregation, association và dependency bằng line/marker semantics, không chỉ màu. Ảnh chỉ là rubric về loại quan hệ, mật độ và bố cục; không sao chép prose, class/member names, CSS, SVG, tọa độ, template hoặc asset. Archive exact review-23 trước mutation; chỉ ba UML-class HTML và một preview đổi artwork, 87 non-target HTML giữ nguyên sau candidate-ID normalization và 29 non-target preview byte-identical. Không sửa P-17/P-19A/P-18/package/`dist`/publication/Git/Release, không mở P-19C; owner approval review-24 vẫn riêng. | LOCKED |
| D-105 | Ngày 2026-08-30, chủ sở hữu đặt quy tắc connector cho `uml-class` và mọi diagram P-19: một connector gắn vào một cạnh node phải ở đúng trung điểm; từ hai connector trở lên trên cùng cạnh phải dùng vị trí `i/(n+1)` để khoảng mép ngoài và khoảng giữa bằng nhau; luôn ưu tiên một segment thẳng. Route có góc 90° chỉ dùng khi tuyến thẳng gây va chạm node/boundary/label hoặc reading order mơ hồ; mặc định góc 90° bo tròn theo P-18 và exception phải có reason machine-readable. Review-25 phải archive exact review-24, tạo helper/test dùng chung, khai báo policy trên đủ 90 SVG P-19, và sửa UML proof để dependency đơn căn giữa, hai realization port chia đúng ba khoảng bằng nhau, bốn relation thẳng và chỉ association dưới hàng domain là rounded-orthogonal exception. Ảnh chỉ là rubric alignment/spacing; không sao chép nội dung hay tọa độ. Không sửa P-17/P-19A/P-18/package/`dist`/publication/Git/Release, không mở P-19C; owner approval review-25 vẫn riêng. | LOCKED |
| D-106 | Ngày 2026-08-30, chủ sở hữu yêu cầu thay riêng `tree` P-19 thành cây chi tiết ba tầng theo ảnh tham chiếu nhưng tuân thủ nguyên tắc căn giữa của exact P-18 `org-chart`. Cho phép P-19B review-26 tạo independent fixture 9 node/8 parent relation gồm một root, ba branch và năm leaf; root phải nằm giữa toàn bộ branch span, mỗi parent nhiều child phải nằm đúng midpoint giữa hai child ngoài cùng, sibling cùng tầng phân bố cân đối, parent một child phải thẳng tâm với child. Multi-child fanout dùng một centered trunk + shared horizontal bus + centered drops như P-18 org-chart; single-child dùng một line thẳng; không dùng arrowhead vì rank đã mang nghĩa. Ba mode phải cùng geometry, mọi connector bám edge center; ảnh chỉ là rubric về ba tầng, mật độ và bố cục, không sao chép prose, nhãn, CSS, SVG, tọa độ, template hoặc asset. Archive exact review-25 trước mutation; chỉ ba `type-tree` HTML và một preview đổi artwork, 87 non-target HTML giữ nguyên sau candidate-ID normalization và 29 non-target preview byte-identical. Không sửa P-17/P-19A/P-18/package/`dist`/publication/Git/Release, không mở P-19C; owner approval review-26 vẫn riêng. | LOCKED |
