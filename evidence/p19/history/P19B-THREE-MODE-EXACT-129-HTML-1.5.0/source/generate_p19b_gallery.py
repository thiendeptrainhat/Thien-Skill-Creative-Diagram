...
from gallery_renderer_v15 import MODES, P19B_CANDIDATE_ID, render_gallery_html, renderer_inventory  # noqa: E402
...
def build_index(records: list[dict]) -> str:
    ...
    return f'''<!doctype html><html lang="vi"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>P-19B · Exact 129 HTML gallery</title><style>
    :root{{--paper:#eeece7;--surface:#fffdf9;--ink:#202735;--muted:#657083;--line:#cbc8c0;--accent:#a3422e}}*{{box-sizing:border-box}}html,body{{margin:0;background:var(--paper);color:var(--ink)}}body{{font-family:ui-sans-serif,system-ui,-apple-system,'Segoe UI',sans-serif;padding:42px 22px 70px}}main{{width:min(100%,1680px);margin:auto}}.eyebrow{{margin:0 0 8px;color:var(--accent);font:750 13px ui-monospace,monospace;letter-spacing:.14em}}h1{{margin:0;font:500 clamp(38px,6vw,70px)/1 Georgia,serif}}.lede{{max-width:900px;margin:14px 0 30px;color:var(--muted);font-size:17px;line-height:1.6}}.counts{{display:flex;flex-wrap:wrap;gap:10px;margin-bottom:26px}}.counts span{{border:1px solid var(--line);border-radius:999px;padding:8px 12px;font:700 12px ui-monospace,monospace}}.grid{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px}}.card{{min-width:0;border:1px solid var(--line);background:var(--surface);overflow:hidden}}.preview{{aspect-ratio:16/10;border-bottom:1px solid var(--line);background:#f5f7fa;overflow:hidden}}.preview img{{display:block;width:100%;height:100%;object-fit:contain}}.meta{{display:grid;grid-template-columns:38px 1fr;gap:12px;padding:15px 16px 8px}}.meta>span{{display:grid;place-items:center;width:34px;height:34px;border-radius:50%;background:#ece8e0;font:750 12px ui-monospace,monospace}}h2{{margin:0;font-size:18px}}.meta p{{margin:4px 0 0;color:var(--muted);font:12px ui-monospace,monospace;overflow-wrap:anywhere}}nav{{display:flex;flex-wrap:wrap;gap:8px;padding:8px 16px 16px}}nav a{{color:var(--accent);font:750 12px ui-monospace,monospace}}a:focus-visible{{outline:3px solid var(--accent);outline-offset:3px}}@media(max-width:1050px){{.grid{{grid-template-columns:repeat(2,minmax(0,1fr))}}}}@media(max-width:650px){{body{{padding:24px 12px 48px}}.grid{{grid-template-columns:1fr}}}}
    </style></head><body><main><p class="eyebrow">P‑19B · QA-ONLY CONTACT SHEET</p><h1>43 silhouettes × 3 modes.</h1><p class="lede">Exact 129 standalone HTML specimens derived from validated semantic fixtures. This index is a navigation/contact sheet and is not counted as a specimen. P-19C full QA, freeze, masked review, owner review, packaging and release remain outside this candidate.</p><div class="counts" aria-label="Gallery counts"><span>39 canonical</span><span>4 capabilities</span><span>3 modes</span><span>129 specimens</span><span>14 engines</span></div><section class="grid">{''.join(cards)}</section></main></body></html>'''
...
def main() -> None:
    ...
    write_json(MANIFEST_PATH, {
        ...
        "note": "P-19B technical candidate only; P-19C freeze/owner review not performed.",
    })
