#!/usr/bin/env python3
"""Generate 87 P-19 specimens and link 14 unchanged approved P-18 anchors."""
...
def main() -> None:
    ...
    if len(sources) != 29:
        raise RuntimeError("D-085 requires exactly 25 canonical fixtures plus four capability fixtures")

    records = []
    full_order = [f"type-{kind}" for kind in CANONICAL_TYPES] + [f"cap-{cap.lower()}-{CAPABILITY_NAMES[cap]}" for cap in P19A_CAPABILITIES]
    ...
    if len(records) != 87:
        raise RuntimeError("P-19B generation did not produce exactly 87 specimens")
    ...
    if len(preview_files) != 29:
        raise RuntimeError("P-19B contact sheet requires exactly 29 neutral-light SVG previews")
    ...
    write_json(MANIFEST_PATH, {
        ...
        "note": "D-093: it-current-state collection/processing/dissemination remediation with explicit current-state bottlenecks and P-18 continuous orthogonal connectors; rounded 90-degree corners by default, straight only when explicitly requested; retaining D-092–D-085 scope; 87 P-19 HTML, no owner approval or P-19C claim.",
    })
