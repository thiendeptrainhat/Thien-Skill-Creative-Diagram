...
PINS = {
    ...
    "evidence/p19/gallery/P-19B-MANIFEST.json": "5687519fc416d1689c6fa7c3c1a900611641af1faf8b844e05d87ead6a850b84",
    "evidence/p19/P-19B-SOURCE-MANIFEST.json": "f901712236ee2351a2b4ffb756a705d4e179cc422bd93a468151e5e822e3e306",
    "evidence/p19/P-19B-PLAN-MANIFEST.json": "b194124788a59872dc1c53db5f3fbfba773611256624589fedf8b9d1ad695579",
...
def generate():
    ...
    records = b["records"]
    require(len(anchors) == 14 and len(records) == 87, "Wrong candidate coverage")
    require(b["visual_parent_candidate_id"] == a["candidate_id"], "Wrong visual parent")
    require(Counter(r["mode"] for r in records) == Counter({m: 29 for m in MODES}), "Wrong mode coverage")
    ...
    require(len(groups) == 29, "Wrong unique type/capability count")
    require(sum(not v["neutral-light"]["capability_id"] for v in groups.values()) == 25, "Wrong canonical count")
...
<title>P-18 ↔ P-19 · Toàn bộ 101 diagram</title><style>{CSS}</style></head>
...
<p class="intro">14 anchor P-18 đã duyệt + 87 specimen P-19 bổ sung · Tổng cộng 39 loại + 4 capability, không trùng loại giữa hai bộ.</p>
<p class="note">Theo D-084/D-085: <strong>giữ nguyên 14 bản P-18, bỏ các bản P-19 trùng loại</strong>. P-19 còn 25 loại + 4 capability × 3 mode; P-18 chỉ dùng neutral-light đã duyệt, không tự tạo thêm màu. Các loại khác nhau trong cùng engine vẫn có scenario và dữ liệu riêng.</p>
...
<div><strong>P-19B · review-13 · Chờ owner review</strong><code>{html.escape(b['candidate_id'])}</code>
...
<p class="hint">Mặc định ba mode P-19 nằm cạnh nhau trên màn hình rộng. Phóng lớn sẽ xếp các mode dọc, vẫn giữ đủ 101 diagram. Bấm diagram để mở bản gốc trong tab mới.</p>
...
<br>Chưa kiểm chứng render trong browser do chính sách URL local-file; không xem đây là browser QA PASS.</footer>
    ...
    inspector.feed(page)
    require(len(inspector.images) == len(inspector.figures) == 101, "Wrong image count")
    require(len(inspector.ids) == len(set(inspector.ids)), "Duplicate viewer IDs")
    require(Counter(f["data-phase"] for f in inspector.figures) == {"p18": 14, "p19": 87}, "Wrong phase count")
    require(len({(f['data-phase'], f['data-identity'], f['data-mode']) for f in inspector.figures}) == 101, "Duplicate diagram")
    ...
    manifest = {"schema_version": "1.0", "purpose": "auxiliary-view-only-not-a-candidate",
                ...
                "source_manifest_pins": PINS, "counts": {"p18": 14, "p19": 87, "total": 101, "engines": 14, "p19_identities": 29, "reused_canonical_types": 14},
...
if __name__ == "__main__":
    ...
    print(json.dumps({"result": "PASS", "mode": "check" if args.check else "generate",
                      "diagrams": 101, "html_bytes": len(outputs["index.html"]),
