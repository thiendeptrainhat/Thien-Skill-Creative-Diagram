...
from verify_p19b_review44 import verify
...
class ReuseScopeTests(unittest.TestCase):
    ...
    def test_capabilities_and_recent_fixes_retained(self):
        ...
        for name in ('gantt', 'loop-flywheel', 'fishbone', 'dp-integration', 'bar-chart', 'line-chart', 'medallion', 'polar-chart', 'layer-stack', 'dumbbell', 'slope-graph', 'ridgeline', 'bubble'):
            self.assertIn(name, sources)
        self.assertEqual(sources['dumbbell']['diagram']['type'], 'bar-chart')
        self.assertEqual(sources['dumbbell']['diagram']['variant_ids'], ['CAP-V17'])
        self.assertEqual(sources['slope-graph']['diagram']['type'], 'line-chart')
        self.assertEqual(sources['slope-graph']['diagram']['variant_ids'], ['CAP-V18'])
        self.assertEqual(sources['ridgeline']['diagram']['type'], 'line-chart')
        self.assertEqual(sources['ridgeline']['diagram']['variant_ids'], ['CAP-V19'])
    ...
    def test_exact_preservation_and_recoverable_withdrawal(self):
        ...
        self.assertEqual(report['layer_stack_measurement']['layers'], 5)
        self.assertEqual(report['layer_stack_measurement']['modules'], 23)
        self.assertEqual(report['layer_stack_measurement']['domains'], 2)
        self.assertEqual(report['layer_stack_measurement']['dependencies'], 4)
        self.assertEqual(report['display_identity'], 'layer-stack')
        self.assertEqual(report['layer_distribution'], [4, 5, 6, 4, 4])
        self.assertEqual(report['presentation_layers_variant'], 'PRESERVED_AS_NON_TARGET')
