...
const source = path.join(root, 'evidence/p19/gallery/previews/cap-cap-v19-ridgeline.svg');
const out = path.join(root, 'evidence/p19/review43-checks');
fs.mkdirSync(out, { recursive: true });
const svg = fs.readFileSync(source);
fs.writeFileSync(path.join(out, 'ridgeline.svg'), svg);
...
if (/var\(--[^)]+\)/.test(rasterSvg)) throw new Error('Unresolved template token in ridgeline proof');
await sharp(Buffer.from(rasterSvg),{density:120}).png().toFile(path.join(out,'ridgeline.svg.png'));
