...
**Cập nhật:** 2026-08-30  
**Phạm vi được phép trong yêu cầu hiện tại:** D-099: sửa riêng P-19 `wardley-map` thành bản đồ visibility/evolution chi tiết ở đủ ba mode; giữ 14 P-18 approved anchor, 87 non-target P-19 HTML/29 non-target preview và mọi frozen/release bytes; cập nhật gallery/trang tổng hợp/evidence/handoff.
**Phase đang thực hiện:** P-19B review-19 `in-progress`/owner-review-pending theo D-099, giữ scope D-084–D-098; exact review-18 archived; P-19 tổng thể `in-progress`; P-19C `not-started` và unauthorized
...
4. `P-19A` — 39+4 type/capability adapters, đã hoàn tất theo D-078; `P-19B` — remediation theo D-084–D-099 đang `in-progress`, kết hợp 14 P-18 anchor với 90 P-19 HTML sau owner rejection của candidate đầu; `P-19C` — full QA/freeze/owner review, chưa được phép và vẫn cần authorization riêng.
...
2. `P-19B` — remediation `in-progress` theo D-084–D-099: dùng trực tiếp 14 exact P-18R6 review-17 anchor, không tái sinh duplicate; 25 canonical type, bốn capability và presentation variant `layers` có ba mode, tổng 90 HTML/30 preview. Active review-19 giữ toàn bộ D-086–D-098 và thay riêng `wardley-map` bằng bản đồ visibility/evolution chi tiết; comparison vẫn có 104 diagram, 87 non-target artwork được bảo toàn và withdrawal custody vẫn khôi phục được.
...
**Verification dự kiến:** theo D-085/D-095–D-099, union 14 P-18 canonical + 25 P-19 canonical phải đúng 39 và không trùng; P-19 có 75 canonical + 12 capability + 3 `layers` variant HTML, 30 preview, đủ ba mode cho 30 identity. `wardley-map` phải có 8 component/9 dependency/2 normalized axis/4 stage/3 boundary/1 evolving signal, exact table và geometry ba mode; detailed `line-chart` D-096, `medallion` D-097 và `polar-chart` D-098 phải được giữ. P-18 phải giữ exact bytes/hash ở neutral-light, không claim ba-mode coverage cho 14 anchor này. Kiểm absence of withdrawn files, links, deterministic generation, retained artwork và recoverable custody; owner vẫn phải duyệt exact current gallery.
