...
P19B_CANDIDATE_ID = "P19B-P18-INHERITED-THREE-MODE-REVIEW-23-1.5.0"
...
def validate_target_geometry(svg: str, diagram_type: str) -> dict[str, Any]:
    ...
    if diagram_type not in ("dp-integration", "swimlane", "bar-chart", "line-chart", "polar-chart", "wardley-map", "venn", "dp-security-matrix", "er-data-model", "high-level", "it-current-state", "kanban", "layer-stack", "medallion"):
        return {}
...
def render_gallery_html(ir_value: Mapping[str, Any], mode: str, fixture_id: str, *, connector_corner_style: str | None = None) -> str:
    ...
    identity = "layers" if is_layers_variant else adapter["capability_id"] or adapter["canonical_type"]
    parent = "layer-stack" if is_layers_variant else adapter["canonical_type"] if adapter["capability_id"] else "none"
    silhouette = "five-band-abstraction-stack" if is_layers_variant else adapter["silhouette"]
    title = str(ir_value["diagram"]["title"])
    description = str(plan["accessibility_contract"]["description"])
    takeaway = _text(description) if adapter["canonical_type"] in ("gantt", "loop-flywheel", "dp-integration", "bar-chart", "line-chart", "polar-chart", "treemap", "wardley-map", "venn", "dp-security-matrix", "er-data-model", "high-level", "it-current-state", "kanban", "layer-stack", "medallion") and (adapter["canonical_type"] != "dp-integration" or is_detailed_dp_integration(plan)) and (adapter["canonical_type"] != "bar-chart" or is_detailed_bar_chart(plan)) and (adapter["canonical_type"] != "line-chart" or is_detailed_line_chart(plan)) and (adapter["canonical_type"] != "polar-chart" or is_detailed_polar_chart(plan)) and (adapter["canonical_type"] != "treemap" or is_detailed_treemap(plan)) and (adapter["canonical_type"] != "wardley-map" or is_detailed_wardley_map(plan)) and (adapter["canonical_type"] != "venn" or is_detailed_venn(plan)) and (adapter["canonical_type"] != "dp-security-matrix" or is_detailed_dp_security_matrix(plan)) and (adapter["canonical_type"] != "er-data-model" or is_detailed_er_data_model(plan)) and (adapter["canonical_type"] != "high-level" or is_detailed_high_level(plan)) and (adapter["canonical_type"] != "it-current-state" or is_detailed_it_current_state(plan)) and (adapter["canonical_type"] != "kanban" or is_detailed_kanban(plan)) and (adapter["canonical_type"] != "layer-stack" or is_detailed_layers(plan)) and (adapter["canonical_type"] != "medallion" or is_detailed_medallion(plan)) else f"{_text(adapter['semantic_focus'].capitalize())}. {_text(adapter['accessible_alternative'].capitalize())}."
    ...
    presentation_attr = ' data-presentation-variant="layers"' if is_layers_variant else ""
    fact_label = "TYPE / VARIANT" if is_layers_variant else "TYPE / CAPABILITY"
...
def renderer_inventory() -> dict[str, Any]:
    ...
    return {
        ...
        "viewbox_overrides": {"gantt": "content-fit calendar; width >=1600; height from phase and row content", "loop-flywheel": "content-fit circle; width >=1600; card and shared-state clearance", "fishbone": "content-fit 5-category cause spine; exact category and label clearance", "dp-integration": "content-fit sources/platform/consumers/shared-services topology; 1800×1040", "bar-chart": "content-fit eight-category zero-baseline comparison; 1800×940", "line-chart": "content-fit three-series eight-week trend; 2000×980; 24 exact points", "polar-chart": "content-fit eight-window radial spokes; 2000×1020; exact 0–100 percent scale", "treemap": "content-fit six-continent exact-area tiling; 2000×1040; one focal and one compact-label tile", "wardley-map": "content-fit eight-component visibility/evolution map; 2000×980", "venn": "content-fit three equal sets; 2000×1040; exact nested-clipped triple intersection", "dp-security-matrix": "content-fit five-role by five-component permission matrix; 2000×820", "er-data-model": "content-fit four-entity aggregate/join model; 2000×940", "high-level": "content-fit end-to-end data-platform overview; 2000×1040; rounded 90-degree corners by default", "it-current-state": "content-fit collection/processing/dissemination landscape; 2000×1040; rounded 90-degree corners by default", "kanban": "content-fit four-column WIP board; 2000×900; explicit blocked/waiting/done states", "layers": "content-fit five-band abstraction stack; 2000×920; one non-color focal layer", "medallion": "content-fit five-stage lifecycle; 2000×1020; four directed promotions and two processing paths"},
