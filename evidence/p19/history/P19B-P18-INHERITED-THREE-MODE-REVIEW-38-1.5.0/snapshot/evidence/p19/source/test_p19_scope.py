...
from verify_p19b_review38 import verify
...
class ReuseScopeTests(unittest.TestCase):
    ...
    def test_exact_preservation_and_recoverable_withdrawal(self):
        ...
        self.assertEqual(report['process_measurement']['nodes'], 11)
        self.assertEqual(report['process_measurement']['edges'], 11)
        self.assertEqual(report['process_measurement']['straight_routes'], 9)
        self.assertEqual(report['process_measurement']['rounded_orthogonal_exceptions'], 2)
        self.assertEqual(report['process_measurement']['document_boundary_contacts'], 5)
        self.assertEqual(report['process_measurement']['shape_counts']['terminator'], 3)
        self.assertEqual(report['process_measurement']['shape_counts']['multiple-document'], 1)
