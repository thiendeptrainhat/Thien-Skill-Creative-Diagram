...
def main() -> None:
    ...
    check(checks, "exact-specimen-count", len(files) == len(records) == 93, f"files={len(files)} records={len(records)}")
    ...
    check(checks, "presentation-variant-count", inventory["presentation_variant_count"] == 2, str(inventory["presentation_variant_count"]))
    ...
    check(checks, "mode-balance", mode_counts == Counter({mode: 31 for mode in MODES}), str(dict(mode_counts)))
    canonical_records = [item for item in records if item["capability_id"] is None and item.get("presentation_variant_id") is None]
    capability_records = [item for item in records if item["capability_id"] is not None]
    variant_records = [item for item in records if item.get("presentation_variant_id") is not None]
    ...
    check(checks, "variant-three-mode-count", len(variant_records) == 6 and {item["identity"] for item in variant_records} == {"layers", "scatter-chart"}, str(len(variant_records)))
    check(checks, "unique-fixture-count", len({item["fixture_id"] for item in records}) == 31, str(len({item["fixture_id"] for item in records})))
    check(checks, "unique-silhouette-count", len({item["silhouette"] for item in records}) == 31, str(len({item["silhouette"] for item in records})))
    ...
    for record in records:
        ...
        if record.get("presentation_variant_id"):
            required["data-presentation-variant"] = record["presentation_variant_id"]
    ...
    check(checks, "p18-typography-role-receipts", all(value in all_text for value in ("georgia · display", "avenir next · material", "menlo · technical")), "93 specimens expose P-18 resolved typography roles")
    check(checks, "p18-visual-grammar-marker", all('data-visual-grammar="p18r6-review17"' in path.read_text(encoding="utf-8") for path in files), "93/93")
    ...
    check(checks, "index-link-completeness", set(index_parser.hrefs) == expected_links and len(index_parser.hrefs) == 107, f"links={len(index_parser.hrefs)}")
    check(checks, "index-card-count", index_parser.tags["article"] == 45, f"cards={index_parser.tags['article']}")
