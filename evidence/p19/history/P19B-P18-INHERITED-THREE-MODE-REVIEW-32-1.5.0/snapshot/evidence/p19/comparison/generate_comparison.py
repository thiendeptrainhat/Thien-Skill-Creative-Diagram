...
<h1>P-18 ↔ P-19<br>Toàn bộ diagram, cùng một trang.</h1>
    ...
    <p class="note">Theo D-084/D-085/D-095–D-112: <strong>giữ nguyên 14 bản P-18, bỏ các bản P-19 trùng loại, giữ mọi remediation trước, áp dụng connector policy căn giữa/chia đều/ưu tiên đường thẳng và giảm riêng viền Treemap về hệ nét mảnh trong khi giữ nguyên geometry</strong>. P-19 có 25 loại + 4 capability + 1 presentation variant, mỗi identity đủ 3 mode; P-18 chỉ dùng neutral-light đã duyệt, không tự tạo thêm màu.</p>
...
<a href="{link(P18 / 'index.html')}" target="_blank" rel="noopener">Mở gallery P-18 ↗</a></div>
    <div><strong>P-19B · review-32 · Chờ owner review</strong><code>{html.escape(b['candidate_id'])}</code>
