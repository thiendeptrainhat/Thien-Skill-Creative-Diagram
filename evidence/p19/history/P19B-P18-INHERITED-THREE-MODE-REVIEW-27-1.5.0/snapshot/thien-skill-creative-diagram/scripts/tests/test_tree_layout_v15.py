"""Focused D-107 tests for the centered three-tier tree."""
import re
import unittest
...
class DetailedTreeTests(unittest.TestCase):
    ...
    def test_thin_stroke_hierarchy_is_locked(self):
        ...
        self.assertIn(".tree-wire{fill:none;stroke:var(--connector);stroke-width:1.6", page)
        self.assertIn(".tree-card-shape{fill:var(--surface);stroke:var(--connector);stroke-width:1.8", page)
        self.assertIn(".tree-card.is-root .tree-card-shape{fill:var(--accent-soft);stroke:var(--accent);stroke-width:2.2", page)
        self.assertNotIn("stroke-width:2.6", page)
        self.assertNotIn("stroke-width:2.4;vector-effect:non-scaling-stroke", page)
