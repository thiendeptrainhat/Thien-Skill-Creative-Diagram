...
from verify_p19b_review26 import verify
...
class ReuseScopeTests(unittest.TestCase):
    ...
    def test_exact_preservation_and_recoverable_withdrawal(self):
        ...
        self.assertEqual(report['tree_measurement']['nodes'], 9)
        self.assertEqual(report['tree_measurement']['edges'], 8)
        self.assertEqual(report['tree_measurement']['centered_parents'], 4)
        self.assertEqual(report['branch_center_intervals'], [640, 640])
        self.assertEqual(report['two_child_offsets'], [-150, 150])
