"""D-102 bordered exact-area continent Treemap with direct labels and data alternative."""
...
def treemap_css(tokens):
    return """
    .tm-gutter{fill:none;stroke:var(--canvas);stroke-width:8;stroke-linejoin:round}
    .tm-tile{stroke:var(--connector);stroke-width:2.4;stroke-linejoin:round;vector-effect:non-scaling-stroke}
    .tm-tile.tm-focal{fill:var(--accent-soft);stroke:var(--accent);stroke-width:3.2}
...
def render_treemap(plan):
    ...
    for index, tile in enumerate(layout["tiles"]):
        ...
        parts.append(
            f'<rect class="tm-gutter" x="{tile["x"]:.6f}" y="{tile["y"]:.6f}" '
            f'width="{tile["width"]:.6f}" height="{tile["height"]:.6f}" rx="5"/>'
        )
        parts.append(
