"""Focused D-102 tests for the bordered exact-area continent Treemap."""
import copy
import re
import unittest
...
class DetailedTreemapTests(unittest.TestCase):
    ...
    def test_every_tile_has_visible_outline_and_preserved_gutter(self):
        svg = render_treemap(self.plan)
        css = treemap_css({})
        self.assertEqual(svg.count('class="tm-gutter"'), 6)
