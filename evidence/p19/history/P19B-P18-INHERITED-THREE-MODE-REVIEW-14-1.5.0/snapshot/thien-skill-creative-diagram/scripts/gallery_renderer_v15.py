...
P19B_CANDIDATE_ID = "P19B-P18-INHERITED-THREE-MODE-REVIEW-14-1.5.0"
...
def validate_target_geometry(svg: str, diagram_type: str) -> dict[str, Any]:
    ...
    if diagram_type not in ("dp-integration", "swimlane", "bar-chart", "dp-security-matrix", "er-data-model", "high-level", "it-current-state", "kanban"):
        return {}
...
def render_gallery_html(ir_value: Mapping[str, Any], mode: str, fixture_id: str, *, connector_corner_style: str | None = None) -> str:
    ...
    takeaway = _text(description) if adapter["canonical_type"] in ("gantt", "loop-flywheel", "dp-integration", "bar-chart", "dp-security-matrix", "er-data-model", "high-level", "it-current-state", "kanban") and (adapter["canonical_type"] != "dp-integration" or is_detailed_dp_integration(plan)) and (adapter["canonical_type"] != "bar-chart" or is_detailed_bar_chart(plan)) and (adapter["canonical_type"] != "dp-security-matrix" or is_detailed_dp_security_matrix(plan)) and (adapter["canonical_type"] != "er-data-model" or is_detailed_er_data_model(plan)) and (adapter["canonical_type"] != "high-level" or is_detailed_high_level(plan)) and (adapter["canonical_type"] != "it-current-state" or is_detailed_it_current_state(plan)) and (adapter["canonical_type"] != "kanban" or is_detailed_kanban(plan)) else f"{_text(adapter['semantic_focus'].capitalize())}. {_text(adapter['accessible_alternative'].capitalize())}."
