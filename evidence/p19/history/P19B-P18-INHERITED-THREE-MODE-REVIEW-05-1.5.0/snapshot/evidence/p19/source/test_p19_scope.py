...
from verify_p19b_review05 import verify
...
class ReuseScopeTests(unittest.TestCase):
    ...
    def test_generator_excludes_all_duplicates(self):
        ...
        self.assertEqual(sum(not identity.startswith('CAP-') for _, identity, _ in sources), 25)

    def test_capabilities_and_recent_fixes_retained(self):
        sources = {identity: ir for _, identity, ir in specimen_sources()}
        for name in ('gantt', 'loop-flywheel', 'dp-integration', 'CAP-V17', 'CAP-V18', 'CAP-V19', 'CAP-V20'):
    ...
    def test_index_has_no_duplicate_source_links(self):
        ...
        self.assertEqual(parser.tags['article'], 43)
        self.assertEqual(len(parser.hrefs), 101)
        self.assertEqual(len(set(parser.hrefs)), 101)
    ...
    def test_exact_preservation_and_recoverable_withdrawal(self):
        ...
        self.assertEqual(report['retained_html_unchanged_except_candidate_id'], 87)
        self.assertEqual(report['withdrawn_files_recoverable'], 56)
