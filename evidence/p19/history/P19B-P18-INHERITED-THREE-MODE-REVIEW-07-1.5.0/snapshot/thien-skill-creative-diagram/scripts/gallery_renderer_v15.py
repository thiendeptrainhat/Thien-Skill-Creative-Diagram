...
P19B_CANDIDATE_ID = "P19B-P18-INHERITED-THREE-MODE-REVIEW-07-1.5.0"
...
def render_gallery_html(ir_value: Mapping[str, Any], mode: str, fixture_id: str, *, connector_corner_style: str | None = None) -> str:
    ...
    takeaway = _text(description) if adapter["canonical_type"] in ("gantt", "loop-flywheel", "dp-integration") and (adapter["canonical_type"] != "dp-integration" or is_detailed_dp_integration(plan)) else f"{_text(adapter['semantic_focus'].capitalize())}. {_text(adapter['accessible_alternative'].capitalize())}."
