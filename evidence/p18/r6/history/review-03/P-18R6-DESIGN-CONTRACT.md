...
**Authority:** D-051, D-052, D-058, D-059, D-060, D-061  
...
- Diagram 04 uses one chart-level `connector_corner_style`. The default is `rounded`, so every 90° turn uses the same controlled-radius corner. If the user explicitly requests `straight`, that choice outranks the default and every 90° turn in the chart is serialized sharp; styles are never mixed within one chart.
...
All P-18R6 prose, data, CSS, SVG and engine geometry are independently authored for this repository. No upstream code, CSS, SVG, template, screenshot or asset is copied or traced. Upstream comparison remains limited to abstract function/quality criteria. The exact P-18R5 Swimlane SVG is reused only as an internal owner-approved parent artifact and is byte-checked against its frozen source; no R5 file is edited. Review-01 and review-02 remain byte-bound at `history/review-01/` and `history/review-02/`. Review-03 supersedes review-02 only for owner review and records both historical manifest SHA-256 values in its lineage.
