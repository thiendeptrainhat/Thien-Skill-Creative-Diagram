...
def hierarchy_anchor() -> Anchor:
    ...
    for index, label in enumerate(("COMMAND", "DOMAINS", "PODS")):
        parts.append(_text(58, centers[index] + 5, label, "kicker", **{"data-band-label": label.lower(), "data-band-center-y": f"{centers[index]:.2f}"}))
    parts.append("</g>")
    root, _ = _node(570, 118, "Operating lead", "priority · arbitration", "FRONT", width=460, focal=True, node_id="hierarchy-operating-lead", metadata={"data-band-member": "command", "data-band-index": 0, "data-band-center-y": f"{centers[0]:.2f}", "data-band-member-kind": "card"})
    parts.append(root)
...
def special_anchor() -> Anchor:
    ...
    ribbons = (
        sankey_ribbon("budget-unit", "budget", "unit", 5200, 210.0, 210.0, TOKENS["line_soft"], .74),
        sankey_ribbon("budget-e2e", "budget", "e2e", 4000, 340.0, 430.0, TOKENS["line_soft"], .74),
        sankey_ribbon("budget-build", "budget", "build", 2800, 440.0, 650.0, TOKENS["line_soft"], .74),
        sankey_ribbon("unit-passed", "unit", "passed", 4200, 210.0, 210.0, TOKENS["line_soft"], .72),
        sankey_ribbon("e2e-passed", "e2e", "passed", 3600, 430.0, 315.0, TOKENS["line_soft"], .72),
        sankey_ribbon("build-passed", "build", "passed", 1600, 650.0, 405.0, TOKENS["line_soft"], .72),
        sankey_ribbon("e2e-failed", "e2e", "failed", 400, 520.0, 550.0, TOKENS["line_soft"], .72),
        sankey_ribbon("build-failed", "build", "failed", 1200, 690.0, 560.0, TOKENS["line_soft"], .72),
        sankey_ribbon("unit-flaked", "unit", "flaked", 1000, 315.0, 680.0, TOKENS["accent"], .32),
    )
    parts = [
    ...
    for node_id, column, x, y, nw, value, title, sub, focal in nodes:
        ...
        parts.extend(
            (
                ...
                _text(x + nw / 2, title_y, title, "node-title", "middle", **{"data-node-label": node_id, "data-label-role": "title", "data-label-placement": "above"}),
