...
ANCHORS = R6 / "anchors"
REVIEW = R6 / "review"
CANDIDATE_ID = "P18R6-FOURTEEN-ENGINE-NEUTRAL-LIGHT-1.5.0"
...
def main() -> None:
    ...
    subprocess.run([sys.executable, str(SOURCE / "generate_p18r6.py")], cwd=ROOT, check=True)
    after = generated_hashes()
    deterministic = before == after
    subprocess.run([sys.executable, str(SOURCE / "p18r6_qa.py")], cwd=ROOT, check=True)
    ...
    quicklook = {
        ...
        "implementer_visual_review": "PASS_AFTER_DEPENDENCY_CORRIDOR_REMEDIATION",
    ...
    verification = {
        ...
        "implementer_visual_precheck": {"status": "PASS", "score": 89.5, "minimum_dimension": 4.3},
