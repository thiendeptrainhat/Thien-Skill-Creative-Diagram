...
**Candidate:** `P18R6-FOURTEEN-ENGINE-NEUTRAL-LIGHT-REVIEW-14-1.5.0`  
...
Review-14 resolves D-072 for diagram 14. `Monthly budget`, `Unit tests` and `Passed` now share exact `top-y = 210px`, so the first source, stage and outcome bars align visually along one top row. The source bar moves upward by 40px and its three outgoing source-side intervals move by the same amount; no value, ribbon thickness, target interval or column geometry changes. Static and Chrome geometry checks cap the three-bar top-edge spread at `0.01px` while repeating every D-071 scale, occupancy, label and square-corner assertion. All thirteen non-target anchor HTML/SVG pairs remain byte-identical to review-13.
...
- Static QA: `PASS` — 333/333 checks, including D-071 scale, conservation, exact interface tiling, above-bar label placement and square-corner serialization plus D-072 exact top-row alignment.
- Local Quick Look raster review of all fourteen canonical SVGs: `PASS` after D-072 review-14 regeneration. Diagram 14 was inspected at its 2000px canonical render: the three upper bars align at one top edge; ribbons still meet every bar without an unallocated segment; labels remain above bars; no bar has a rounded corner.
- Browser automation: `PASS` — 42/42 engine × viewport cases at canonical, desktop and mobile sizes; zero console error, external request, inline-cardinality failure, D-071 runtime geometry failure or D-072 top-row alignment failure.
