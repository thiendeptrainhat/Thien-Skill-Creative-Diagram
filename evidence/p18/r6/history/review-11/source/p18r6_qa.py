...
from gallery_kernel import compartment_anchor, containment_anchor, dependency_anchor, deployment_anchor, integration_anchor, matrix_anchor, topology_anchor
...
def main() -> None:
    ...
    cardinality_placement_ok = len(cardinality_labels) == 6
    ...
    for label in cardinality_labels:
        ...
        line_clearance = float(label.attrib["data-line-clearance"])
        minimum = float(label.attrib["data-minimum-clearance"])
        if line.attrib["data-axis"] == "horizontal":
            boundary = float(label.attrib["data-node-boundary-x"])
            actual_node = bbox_x - boundary if endpoint == "source" else boundary - (bbox_x + bbox_width)
            actual_line = float(line.attrib["y1"]) - (bbox_y + bbox_height)
            placement = label.attrib.get("data-label-placement") == "above"
        else:
            boundary = float(label.attrib["data-node-boundary-y"])
            actual_node = bbox_y - boundary if endpoint == "source" else boundary - (bbox_y + bbox_height)
            actual_line = bbox_x - float(line.attrib["x1"])
            placement = label.attrib.get("data-label-placement") == "right"
        cardinality_clearances.append((relationship_id, endpoint, round(actual_node, 2), round(actual_line, 2)))
        cardinality_placement_ok = cardinality_placement_ok and (
            placement and actual_node >= minimum and actual_line >= minimum
            and abs(actual_node - node_clearance) <= 0.02
            and abs(actual_line - line_clearance) <= 0.02
        )
    check(
        cardinality_placement_ok,
        "R6-D069-SCHEMA-CARDINALITY-ENDPOINT-PLACEMENT",
        f"relationship/endpoint node-line clearances={cardinality_clearances}; minimum=8px",
        results,
    )
    ...
    check(
        ...
        "R6-D069-DETERMINISTIC-SCHEMA-CARDINALITY-CONTRACT",
        "source builder regenerates the same D-069 cardinality/name placement contract",
        results,
    )
