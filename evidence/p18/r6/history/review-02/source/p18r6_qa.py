...
def main() -> None:
    ...
    for engine, expected_count in expected_rounded_counts.items():
        ...
        check(
            ...
            f"R6-D060-NO-BROAD-CURVE-{engine}",
    ...
    dependency_route = next(
        (item for item in dependency.iter(f"{SVG_NS}path") if item.attrib.get("data-edge-id") == "dependency-types-zod"),
        None,
    )
    bridge_group = next(
        (item for item in dependency.iter(f"{SVG_NS}g") if "bridge-mark" in item.attrib.get("class", "").split()),
        None,
    )
    check(
        dependency_route is not None and dependency_route.attrib.get("data-path-bridges-integrated") == "true",
        "R6-D060-DEPENDENCY-BRIDGE-INTEGRATED",
        "dependency crossing replaces the chord inside the routed path",
        results,
    )
    check(
        bridge_group is not None
        ...
        "R6-D060-DEPENDENCY-BRIDGE-CONTRACT",
        "shared hop geometry + crown-only underlay + join continuity",
        results,
    )
    if dependency_route is not None and bridge_group is not None:
        ...
        check(
            bool(hop_commands) and hop_commands in dependency_route.attrib.get("d", ""),
            "R6-D060-DEPENDENCY-HOP-SHARED-BYTES",
            "route and repaint use the same cubic hop commands",
            results,
        )
        check(
            underlay is not None and underlay.attrib.get("class") == "bridge-hop-underlay",
            "R6-D060-DEPENDENCY-CROWN-UNDERLAY",
            "underlay exists only as the bridge crown repaint",
            results,
        )
