  ...
  for (const item of inventory.engines) {
    ...
    for (const viewport of viewports) {
      ...
      const metrics = await page.evaluate(() => {
        ...
        if (svg.dataset.layoutEngine === "hierarchy") {
          const contract = svg.querySelector('[data-five-second-contract="D-076"][data-hierarchy-count-contract="1-4-5"]');
          const visibleText = [...svg.querySelectorAll("text")].map((node) => node.textContent.trim());
          if (
            !contract
            || contract.dataset.frontDoorCount !== "1"
            || contract.dataset.domainCount !== "4"
            || contract.dataset.specialistPodCount !== "5"
            || !["1 FRONT DOOR", "4 DOMAINS", "5 SPECIALIST PODS"].every((label) => visibleText.includes(label))
          ) failures.push("hierarchy-five-second-count-contract");
        }
        ...
        if (svg.dataset.layoutEngine === "special-geometry") {
          ...
          const focalContract = svg.querySelector('[data-five-second-contract="D-076"][data-focal-contrast-policy="one-coral-high-opacity"]');
          const focalRibbon = svg.querySelector('[data-sankey-ribbon="unit-flaked"]');
          const nonFocalRibbons = ribbons.filter((ribbon) => ribbon.dataset.sankeyRibbon !== "unit-flaked");
          const focalAnnotation = svg.querySelector('[data-focal-takeaway-annotation="true"][data-five-second-contract="D-076"]');
          const focalText = focalAnnotation?.textContent.replace(/\s+/g, " ").trim() || "";
          if (
            !focalContract
            || focalContract.dataset.focalRibbon !== "unit-flaked"
            || focalContract.dataset.focalValue !== "1000"
            || focalContract.dataset.totalValue !== "12000"
            || !focalRibbon
            || Number(getComputedStyle(focalRibbon).fillOpacity) < 0.8
            || nonFocalRibbons.some((ribbon) => Number(getComputedStyle(ribbon).fillOpacity) > 0.5)
          ) failures.push("sankey-five-second-focal-contrast");
          if (
            !focalAnnotation
            || focalAnnotation.dataset.value !== "1000"
            || focalAnnotation.dataset.total !== "12000"
            || focalAnnotation.dataset.sharePercent !== "8.3"
            || !focalText.includes("FLAKED RERUNS")
            || !focalText.includes("1,000 / 12,000 MIN · 8.3% OF BUDGET")
          ) failures.push("sankey-five-second-direct-annotation");
        }
