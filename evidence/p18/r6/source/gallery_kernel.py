...
def dependency_anchor(corner_style: str = "rounded") -> Anchor:
    ...
    if corner_style not in CORNER_STYLES:
        raise ValueError(f"Unsupported connector corner style: {corner_style}")
    boundaries = (40.0, 280.0, 520.0, 760.0, 1000.0)
    centers = tuple((top + bottom) / 2 for top, bottom in zip(boundaries, boundaries[1:]))
    parts = [
        '<g data-band-layout-contract="D-073" data-band-axis="y" data-band-count="4" '
        'data-band-boundaries="40 280 520 760 1000" data-band-member-center-tolerance="0.01" '
        'data-minimum-separator-clearance="22">'
    ]
    for index, boundary in enumerate(boundaries):
        parts.append(_line(180, boundary, 1480, boundary, "grid", **{"data-band-separator-index": index, "data-separator-y": f"{boundary:.2f}"}))
    for index, center in enumerate(centers):
        parts.append(_text(62, center + 5, f"RANK {index}", "kicker", **{"data-band-label": f"rank-{index}", "data-band-center-y": f"{center:.2f}"}))
    parts.append("</g>")
    positions = {
        "web": (300, 98), "admin": (930, 98), "api": (260, 338), "ui": (900, 338),
        "types": (390, 578), "db": (930, 578), "tokens": (180, 818), "utils": (650, 818), "zod": (1080, 818),
    }
    specs = [
    ...
    for key, title, sub, badge, focal, muted in specs:
        x, y = positions[key]
        rank_index = 0 if key in {"web", "admin"} else 1 if key in {"api", "ui"} else 2 if key in {"types", "db"} else 3
        item, width = _node(
            x,
            y,
            title,
            ...
            focal=focal,
            muted=muted,
            node_id=f"dependency-{key}",
            metadata={
                "data-band-member": f"rank-{rank_index}",
                "data-band-index": rank_index,
                "data-band-center-y": f"{centers[rank_index]:.2f}",
                "data-band-member-kind": "card",
            },
        )
        boxes[key] = (x, y, width)
        parts.append(item)
    ...
    connector_attrs = {
        ...
        "data-rank-step": "240",
        "data-inter-rank-gap": "116",
        "data-corridor-midpoint-step": "240",
        "data-lower-corridor-midpoint": "760",
        "data-lower-corridor-pitch": "44",
        "data-crossing-count": "2",
        "data-bridge-paint-order": "base-routes then bridge-repaints",
    }
    base_routes = [
        _orthogonal_path(((400, 222), (400, 338)), marker="url(#arrow)", corner_style=corner_style, **route_attrs, **{"data-edge-id": "dependency-web-api"}),
        _orthogonal_path(((480, 222), (480, 309), (970, 309), (970, 338)), marker="url(#arrow)", corner_style=corner_style, **route_attrs, **{"data-edge-id": "dependency-web-ui"}),
        _orthogonal_path(((1070, 222), (1070, 338)), marker="url(#arrow)", corner_style=corner_style, **route_attrs, **{"data-edge-id": "dependency-admin-ui"}),
        _orthogonal_path(((400, 462), (400, 549), (480, 549), (480, 578)), marker="url(#arrow)", corner_style=corner_style, **route_attrs, **{"data-edge-id": "dependency-api-types"}),
        _orthogonal_path(((1040, 462), (1040, 549), (580, 549), (580, 578)), marker="url(#arrow)", corner_style=corner_style, **route_attrs, **{"data-edge-id": "dependency-ui-types"}),
        _orthogonal_path(((930, 640), (670, 640)), marker="url(#arrow)", corner_style=corner_style, **route_attrs, **{"data-edge-id": "dependency-db-types"}),
        _orthogonal_path(((480, 702), (480, 738), (320, 738), (320, 818)), marker="url(#arrow)", corner_style=corner_style, **route_attrs, **{"data-edge-id": "dependency-types-tokens"}),
    ]
    utils_route, utils_bridge = _bridged_orthogonal_components(
        ((530, 702), (530, 738), (790, 738), (790, 818)),
        (580, 738, 1, 14),
        edge_id="dependency-types-utils",
        corner_style=corner_style,
    )
    zod_route, zod_bridge = _bridged_orthogonal_components(
        ((580, 702), (580, 782), (1220, 782), (1220, 818)),
        (790, 782, 1, 14),
        edge_id="dependency-types-zod",
        corner_style=corner_style,
    )
    cycle_route = _orthogonal_path(
        ((790, 942), (790, 1020), (320, 1020), (320, 942)),
        "wire accent dashed",
    ...
    parts += [
        _text(555, 1060, "CYCLE · MUST BREAK", "kicker", "middle"),
...
def experience_anchor() -> Anchor:
    ...
    boundaries = (204.0, 384.0, 564.0, 744.0)
    centers = tuple((top + bottom) / 2 for top, bottom in zip(boundaries, boundaries[1:]))
    parts = []
    for x, label in columns:
        parts += [_rect(x, 78, 320, 104, "band", 14), _text(x + 160, 126, label, "node-title", "middle"), _text(x + 160, 157, "moment", "kicker", "middle"), _line(x - 20, boundaries[0], x - 20, boundaries[-1], "grid")]
    parts.append(
        '<g data-band-layout-contract="D-073" data-band-axis="y" data-band-count="3" '
        'data-band-boundaries="204 384 564 744" data-band-member-center-tolerance="0.01" '
        'data-minimum-separator-clearance="27">'
    )
    for index, boundary in enumerate(boundaries):
        parts.append(_line(170, boundary, 1600, boundary, "grid", **{"data-band-separator-index": index, "data-separator-y": f"{boundary:.2f}"}))
    for index, label in enumerate(("ACTION", "THOUGHT", "EMOTION")):
        parts.append(_text(58, centers[index] + 5, label, "kicker", **{"data-band-label": label.lower(), "data-band-center-y": f"{centers[index]:.2f}"}))
    parts.append("</g>")
    ...
    for index, (x, _) in enumerate(columns):
        parts += [
            f'<g data-band-member="action" data-band-index="0" data-band-center-y="{centers[0]:.2f}" data-band-member-kind="card" data-box-x="{x + 18:.2f}" data-box-y="255.00" data-box-width="284.00" data-box-height="78.00">',
            _rect(x + 18, 255, 284, 78, "node-boundary", 12),
            _text(x + 160, 303, actions[index], "material-strong", "middle"),
            "</g>",
        ]
        focus = index == 2
        parts += [
            f'<g data-band-member="thought" data-band-index="1" data-band-center-y="{centers[1]:.2f}" data-band-member-kind="card" data-box-x="{x + 18:.2f}" data-box-y="435.00" data-box-width="284.00" data-box-height="78.00">',
            _rect(x + 18, 435, 284, 78, "band focal" if focus else "node-boundary", 12),
            _text(x + 160, 483, thoughts[index], "material-strong", "middle"),
            "</g>",
        ]
        radius = 14 + emotion[index] * 3
        parts += [
            _circle(
                x + 160,
                centers[2],
                radius,
                "accent-dot" if focus else "dot",
                **{
                    "data-band-member": "emotion",
                    "data-band-index": "2",
                    "data-band-center-y": f"{centers[2]:.2f}",
                    "data-band-member-kind": "icon",
                    "data-box-x": f"{x + 160 - radius:.2f}",
                    "data-box-y": f"{centers[2] - radius:.2f}",
                    "data-box-width": f"{radius * 2:.2f}",
                    "data-box-height": f"{radius * 2:.2f}",
                },
            ),
            _text(x + 160, 714, f"confidence {emotion[index]}/5", "mono", "middle"),
        ]
    parts += [_path("M350 654 C520 584 710 702 1070 654 S1390 596 1430 654", "wire accent"), _legend(820, w, (("line", "experience trace"), ("accent", "decision moment"), ("dot", "confidence")), "Rows separate behavior, cognition and feeling.")]
...
def hierarchy_anchor() -> Anchor:
    w, h = 1600, 960
    boundaries = (60.0, 300.0, 540.0, 780.0)
    centers = tuple((top + bottom) / 2 for top, bottom in zip(boundaries, boundaries[1:]))
    parts = [
        '<g data-band-layout-contract="D-073" data-band-axis="y" data-band-count="3" '
        'data-band-boundaries="60 300 540 780" data-band-member-center-tolerance="0.01" '
        'data-minimum-separator-clearance="58">'
    ]
    for index, boundary in enumerate(boundaries):
        parts.append(_line(180, boundary, 1540, boundary, "grid", **{"data-band-separator-index": index, "data-separator-y": f"{boundary:.2f}"}))
    for index, label in enumerate(("1 FRONT DOOR", "4 DOMAINS", "5 SPECIALIST PODS")):
        parts.append(_text(58, centers[index] + 5, label, "kicker", **{"data-band-label": label.lower(), "data-band-center-y": f"{centers[index]:.2f}"}))
    parts.append("</g>")
    parts.append(
        '<g data-five-second-contract="D-076" data-hierarchy-count-contract="1-4-5" '
        'data-front-door-count="1" data-domain-count="4" data-specialist-pod-count="5">'
    )
    root, _ = _node(570, 118, "Operating lead", "1 front door · 4 domains · 5 specialist pods", "FRONT", width=460, focal=True, node_id="hierarchy-operating-lead", metadata={"data-band-member": "command", "data-band-index": 0, "data-band-center-y": f"{centers[0]:.2f}", "data-band-member-kind": "card"})
    parts.append(root)
    mids = [(173, "Growth", "acquisition"), (593, "Content", "editorial"), (913, "Commerce", "orders"), (1268, "Systems", "platform")]
    for x, title, sub in mids:
        node, _ = _node(x, 358, title, sub, "POD", width=260, node_id=f"hierarchy-{title.lower()}", metadata={"data-band-member": "domains", "data-band-index": 1, "data-band-center-y": f"{centers[1]:.2f}", "data-band-member-kind": "card"})
        parts.append(node)
    parts += [
        _line(800, 242, 800, 270, "wire", **{"data-parent": "Operating lead", "data-role": "trunk"}),
        _line(303, 270, 1398, 270, "wire", **{"data-role": "domain-bus"}),
        *[_line(center, 270, center, 358, "wire", **{"data-entry-alignment": "center"}) for center in (303, 723, 1043, 1398)],
    ]
    leaves = [(40, "Media", "ads"), (320, "CRM", "lifecycle"), (600, "Writer", "copy"), (920, "Store", "checkout"), (1275, "Runtime", "agents")]
    for x, title, sub in leaves:
        node, _ = _node(x, 598, title, sub, "SPEC", width=230, muted=True, node_id=f"hierarchy-{title.lower()}", metadata={"data-band-member": "pods", "data-band-index": 2, "data-band-center-y": f"{centers[2]:.2f}", "data-band-member-kind": "card"})
        parts.append(node)
    parts += [
        _line(303, 482, 303, 570, "wire", **{"data-parent": "Growth", "data-role": "child-trunk"}),
        _line(163, 570, 443, 570, "wire", **{"data-role": "child-bus"}),
        _line(163, 570, 163, 598, "wire", **{"data-parent": "Growth", "data-child": "Media", "data-entry-alignment": "center"}),
        _line(443, 570, 443, 598, "wire", **{"data-parent": "Growth", "data-child": "CRM", "data-entry-alignment": "center"}),
        _line(723, 482, 723, 598, "wire", **{"data-parent": "Content", "data-child": "Writer", "data-entry-alignment": "center", "data-route-priority": "straight"}),
        _line(1043, 482, 1043, 598, "wire", **{"data-parent": "Commerce", "data-child": "Store", "data-entry-alignment": "center", "data-route-priority": "straight"}),
        _line(1398, 482, 1398, 598, "wire", **{"data-parent": "Systems", "data-child": "Runtime", "data-entry-alignment": "center", "data-route-priority": "straight"}),
        "</g>",
    ...
    svg = _svg_shell("hierarchy", "org-chart", "Mô hình điều phối theo domain", "Một front door phân nhánh thành đúng bốn domain và năm specialist pod.", w, h, "".join(parts), semantic_ratio=.88)
    return Anchor(9, "hierarchy", "org-chart", "09-hierarchy--neutral-light", "Hierarchy anchor", "1 front door → 4 domains → 5 specialist pods.", svg, (("root", "1 Operating lead"), ("domains", "4: Growth; Content; Commerce; Systems"), ("leaves", "5 specialist pods")))
...
def compartment_anchor() -> Anchor:
    ...
    field_step = 38.0
    cardinality_along_line_padding = 8.0
    cardinality_perpendicular_padding = 4.0
    cardinality_knockout_node_clearance = 8.0
    ...
    parts = [
        ...
        'data-schema-cardinality-contract="D-069" '
        'data-inline-cardinality-contract="D-070" '
        'data-cardinality-knockout-fill-role="canvas" '
        'data-cardinality-along-line-padding="8.00" '
        'data-cardinality-perpendicular-padding="4.00">'
    ]
    ...
    def inline_cardinality(
        ...
        """Place a cardinality on the connector with a measured canvas knockout."""

        measured = METRICS.measure("technical", value)
        bbox_width = measured.width
        bbox_height = measured.ascent + measured.descent
        baseline_y = round(center_y + (measured.ascent - measured.descent) / 2, 1)
        if text_anchor == "start":
            bbox_x = text_x
        elif text_anchor == "end":
            bbox_x = text_x - bbox_width
        elif text_anchor == "middle":
            bbox_x = text_x - bbox_width / 2
        else:
            raise ValueError(f"Unsupported cardinality text anchor: {text_anchor}")
        bbox_y = baseline_y - measured.ascent
        bbox_center_x = bbox_x + bbox_width / 2
        bbox_center_y = bbox_y + bbox_height / 2

        if axis == "horizontal":
            knockout_x = bbox_x - cardinality_along_line_padding
            knockout_y = bbox_y - cardinality_perpendicular_padding
            knockout_width = bbox_width + 2 * cardinality_along_line_padding
            knockout_height = bbox_height + 2 * cardinality_perpendicular_padding
            node_clearance = bbox_x - node_boundary if endpoint == "source" else node_boundary - (bbox_x + bbox_width)
            knockout_node_clearance = knockout_x - node_boundary if endpoint == "source" else node_boundary - (knockout_x + knockout_width)
            axis_alignment_error = abs(bbox_center_y - center_y)
        elif axis == "vertical":
            knockout_x = bbox_x - cardinality_perpendicular_padding
            knockout_y = bbox_y - cardinality_along_line_padding
            knockout_width = bbox_width + 2 * cardinality_perpendicular_padding
            knockout_height = bbox_height + 2 * cardinality_along_line_padding
            node_clearance = bbox_y - node_boundary if endpoint == "source" else node_boundary - (bbox_y + bbox_height)
            knockout_node_clearance = knockout_y - node_boundary if endpoint == "source" else node_boundary - (knockout_y + knockout_height)
            axis_alignment_error = abs(bbox_center_x - text_x)
        else:
            raise ValueError(f"Unsupported cardinality axis: {axis}")

        binding = f"{relationship_id}:{endpoint}"
        knockout_attrs = {
            "class": "cardinality-knockout",
            "x": f"{knockout_x:.2f}",
            "y": f"{knockout_y:.2f}",
            "width": f"{knockout_width:.2f}",
            "height": f"{knockout_height:.2f}",
            "rx": "2.00",
            "fill": TOKENS["canvas"],
            "stroke": "none",
            "data-cardinality-knockout": binding,
            "data-relationship-id": relationship_id,
            "data-endpoint-role": endpoint,
            "data-axis": axis,
            "data-fill-role": "canvas",
            "data-along-line-padding": f"{cardinality_along_line_padding:.2f}",
            "data-perpendicular-padding": f"{cardinality_perpendicular_padding:.2f}",
            "data-node-clearance": f"{knockout_node_clearance:.2f}",
            "data-minimum-node-clearance": f"{cardinality_knockout_node_clearance:.2f}",
            "data-visual-role": "line-interruption-underlay",
        }
        label_attrs = {
            "data-relationship-cardinality": binding,
            "data-relationship-id": relationship_id,
            "data-endpoint-role": endpoint,
            "data-cardinality-value": value,
            "data-label-placement": "inline",
            "data-inline-cardinality-contract": "D-070",
            "data-bbox-x": f"{bbox_x:.2f}",
            "data-bbox-y": f"{bbox_y:.2f}",
            "data-bbox-width": f"{bbox_width:.2f}",
            "data-bbox-height": f"{bbox_height:.2f}",
            "data-bbox-center-x": f"{bbox_center_x:.2f}",
            "data-bbox-center-y": f"{bbox_center_y:.2f}",
            "data-node-clearance": f"{node_clearance:.2f}",
            "data-knockout-node-clearance": f"{knockout_node_clearance:.2f}",
            "data-line-clearance": "0.00",
            "data-axis-alignment-error": f"{axis_alignment_error:.2f}",
            "data-node-boundary": f"{node_boundary:.2f}",
            "data-minimum-node-clearance": "8.00",
            "data-knockout-binding": binding,
        }
        knockout = f"<rect {_attrs(knockout_attrs)}/>"
        label = _text(
            text_x,
            baseline_y,
            value,
            "relationship-label cardinality-label",
            text_anchor,
            **label_attrs,
        )
        return knockout + label

    def horizontal_relationship(source_id: str, target_id: str, relationship_name: str) -> str:
        ...
        relationship_id = f"{source_id}-to-{target_id}"
        label_center_x = (corridor_left + corridor_right) / 2
        measured = METRICS.measure("technical", relationship_name)
        rendered_width = measured.width + max(0, len(relationship_name) - 1) * 0.5
        label_line_clearance = 24.0
        label_baseline_y = source_center_y - measured.descent - label_line_clearance
        ...
        source_cardinality_x = corridor_left + 16.0
        target_cardinality_x = corridor_right - 16.0
        return "".join(
            (
                f'<g data-schema-relationship-group="{relationship_id}" data-cardinality-layout="inline-knockout">',
                _line(
                    corridor_left,
                    source_center_y,
                    corridor_right,
                    ...
                    "wire",
                    **{
                        "data-schema-relationship": relationship_id,
                        "data-source": source_id,
                        "data-target": target_id,
                        "data-entry-alignment": "center",
                        "data-axis": "horizontal",
                        "data-semantic-continuity": "single-line",
                        "data-visual-interruption": "knockout-underlay",
                    },
                ),
                _text(
                    label_center_x,
                    ...
                    "middle",
                    **{
                        "data-relationship-label": relationship_id,
                        "data-relationship-name": relationship_name,
                        ...
                        "data-line-clearance": f"{label_line_clearance:.2f}",
                        "data-minimum-node-clearance": "8.00",
                        "data-minimum-line-clearance": "8.00",
    ...
    vertical_label_x = order_center_x + 44
    vertical_label_baseline = (order_y + order_h + item_y) / 2 + 6
    vertical_bbox_y = vertical_label_baseline - vertical_measured.ascent
    vertical_cardinality = {value: METRICS.measure("technical", value) for value in ("1", "N")}
    vertical_cardinality_x = order_center_x
    vertical_source_center_y = (
        order_y + order_h
        + cardinality_knockout_node_clearance
        + cardinality_along_line_padding
        + (vertical_cardinality["1"].ascent + vertical_cardinality["1"].descent) / 2
    )
    vertical_target_center_y = (
        item_y
        - cardinality_knockout_node_clearance
        - cardinality_along_line_padding
        - (vertical_cardinality["N"].ascent + vertical_cardinality["N"].descent) / 2
    )
    parts += [
        '<g data-schema-relationships="true" data-remediation="D-066" '
        'data-cardinality-remediation="D-069" data-inline-cardinality-remediation="D-070">',
        horizontal_relationship("customer", "order", "PLACES"),
        horizontal_relationship("order", "payment", "PAID BY"),
        '<g data-schema-relationship-group="order-to-order-item" data-cardinality-layout="inline-knockout">',
        _line(
            order_center_x,
            order_y + order_h,
            item_x + item_w / 2,
        ...
        _text(
            ...
            **{
                ...
                "data-relationship-name": vertical_label,
                ...
                "data-minimum-node-clearance": "8.00",
                "data-line-clearance": f"{vertical_label_x - order_center_x:.2f}",
                "data-minimum-line-clearance": "8.00",
...
def quantitative_anchor() -> Anchor:
    ...
    axis_contract = {"data-origin-arrowhead": "none", "data-remediation": "D-074"}
    parts = [
        _line(left, bottom, right, bottom, "axis", **axis_contract, **{"data-axis-id": "automation"}),
        _line(left, top, left, bottom, "axis", **axis_contract, **{"data-axis-id": "control-confidence"}),
    ]
...
def special_anchor() -> Anchor:
    ...
    scale = 0.025
    nodes = (
        ("budget", "source", 90.0, 210.0, 42.0, 12000, "Monthly budget", "12,000 min", False),
        ("unit", "stage", 720.0, 210.0, 30.0, 5200, "Unit tests", "5,200 min", False),
        ("e2e", "stage", 720.0, 430.0, 30.0, 4000, "E2E", "4,000 min", False),
        ("build", "stage", 720.0, 650.0, 30.0, 2800, "Build + lint", "2,800 min", False),
        ("passed", "outcome", 1420.0, 210.0, 30.0, 9400, "Passed", "9,400 min", False),
        ("failed", "outcome", 1420.0, 550.0, 30.0, 1600, "Failed", "1,600 min", False),
        ("flaked", "outcome", 1420.0, 680.0, 30.0, 1000, "Flaked", "1,000 min", True),
    )
    node_lookup = {
        node_id: {"column": column, "x": x, "y": y, "width": nw, "height": value * scale, "value": value}
        for node_id, column, x, y, nw, value, _, _, _ in nodes
    }
    ...
    def sankey_ribbon(
        ...
        source_y1 = source_y0 + thickness
        target_y1 = target_y0 + thickness
        span = tx - sx
        c1 = sx + span * 0.38
        c2 = tx - span * 0.38
        d = (
            f"M {sx:.2f} {source_y0:.2f} C {c1:.2f} {source_y0:.2f} {c2:.2f} {target_y0:.2f} {tx:.2f} {target_y0:.2f} "
            f"L {tx:.2f} {target_y1:.2f} C {c2:.2f} {target_y1:.2f} {c1:.2f} {source_y1:.2f} {sx:.2f} {source_y1:.2f} Z"
        )
        return _path(
            d,
            "sankey-ribbon",
            ...
            **{
                ...
                "stroke": "none",
                "data-sankey-ribbon": ribbon_id,
                "data-source-node": source,
                "data-target-node": target,
                "data-value": value,
                "data-scale-px-per-minute": f"{scale:.3f}",
                "data-thickness": f"{thickness:.2f}",
                "data-source-y0": f"{source_y0:.2f}",
                "data-source-y1": f"{source_y1:.2f}",
                "data-target-y0": f"{target_y0:.2f}",
                "data-target-y1": f"{target_y1:.2f}",
            },
        )

    ribbons = (
        sankey_ribbon("budget-unit", "budget", "unit", 5200, 210.0, 210.0, TOKENS["line_soft"], .50),
        sankey_ribbon("budget-e2e", "budget", "e2e", 4000, 340.0, 430.0, TOKENS["line_soft"], .50),
        sankey_ribbon("budget-build", "budget", "build", 2800, 440.0, 650.0, TOKENS["line_soft"], .50),
        sankey_ribbon("unit-passed", "unit", "passed", 4200, 210.0, 210.0, TOKENS["line_soft"], .48),
        sankey_ribbon("e2e-passed", "e2e", "passed", 3600, 430.0, 315.0, TOKENS["line_soft"], .48),
        sankey_ribbon("build-passed", "build", "passed", 1600, 650.0, 405.0, TOKENS["line_soft"], .48),
        sankey_ribbon("e2e-failed", "e2e", "failed", 400, 520.0, 550.0, TOKENS["line_soft"], .48),
        sankey_ribbon("build-failed", "build", "failed", 1200, 690.0, 560.0, TOKENS["line_soft"], .48),
        sankey_ribbon("unit-flaked", "unit", "flaked", 1000, 315.0, 680.0, TOKENS["accent"], .88),
    )
    parts = [
        '<g data-sankey-contract="D-072" data-inherited-contract="D-071" '
        'data-total-value="12000" data-scale-px-per-minute="0.025" '
        'data-node-interface-occupancy="100%" data-node-label-placement="above" '
        'data-node-corner-style="square" data-occupancy-tolerance-px="0.01" '
        'data-top-row-alignment="top" data-top-row-y="210.00" '
        'data-top-row-node-ids="budget unit passed" data-top-row-alignment-tolerance-px="0.01">',
        _text(84, 100, "INTAKE", "kicker"),
        _text(710, 100, "ROUTING", "kicker"),
        _text(1400, 100, "OUTCOME", "kicker"),
        '<g data-sankey-ribbons="true" data-five-second-contract="D-076" '
        'data-focal-ribbon="unit-flaked" data-focal-value="1000" data-total-value="12000" '
        'data-focal-share-percent="8.3" data-focal-contrast-policy="one-coral-high-opacity">',
        *ribbons,
        "</g>",
        '<g data-sankey-nodes="true">',
    ]
    for node_id, column, x, y, nw, value, title, sub, focal in nodes:
        nh = value * scale
        color = TOKENS["accent"] if focal else TOKENS["ink"]
        title_y = y - 46.0
        sub_y = y - 18.0
        title_metrics = METRICS.measure("material", title)
        sub_metrics = METRICS.measure("technical", sub)
        label_top = title_y - title_metrics.ascent
        label_bottom = sub_y + sub_metrics.descent
        left_occupied = nh if column in {"stage", "outcome"} else 0.0
        right_occupied = nh if column in {"source", "stage"} else 0.0
        node_attrs = {
            "data-sankey-node": node_id,
            "data-column": column,
            "data-value": value,
            "data-box-x": f"{x:.2f}",
            "data-box-y": f"{y:.2f}",
            "data-box-width": f"{nw:.2f}",
            "data-box-height": f"{nh:.2f}",
            "data-left-interface-occupied": f"{left_occupied:.2f}",
            "data-right-interface-occupied": f"{right_occupied:.2f}",
            "data-node-interface-occupancy": "100%",
            "data-node-corner-style": "square",
            "data-label-placement": "above",
            "data-label-bbox-top": f"{label_top:.2f}",
            "data-label-bbox-bottom": f"{label_bottom:.2f}",
            "data-label-bottom-clearance": f"{y - label_bottom:.2f}",
            "data-top-row-member": "true" if node_id in {"budget", "unit", "passed"} else "false",
        }
        if node_id in {"budget", "unit", "passed"}:
            node_attrs["data-top-row-y"] = "210.00"
        parts.extend(
            (
                f'<g {_attrs(node_attrs)}>',
                f'<rect class="sankey-node-bar" x="{x:.2f}" y="{y:.2f}" width="{nw:.2f}" height="{nh:.2f}" fill="{color}" '
                f'data-sankey-node-bar="{node_id}" data-node-corner-style="square"/>',
                _text(x + nw / 2, title_y, title, "node-title", "middle", **({"data-node-label": node_id, "data-label-role": "title", "data-label-placement": "above", "style": f"fill:{TOKENS['accent_text']}"} if focal else {"data-node-label": node_id, "data-label-role": "title", "data-label-placement": "above"})),
                _text(x + nw / 2, sub_y, sub, "mono", "middle", **{"data-node-label": node_id, "data-label-role": "value", "data-label-placement": "above"}),
                "</g>",
            )
        )
    parts.extend(("</g>", "</g>"))
    parts.extend(
        (
            '<g data-focal-takeaway-annotation="true" data-five-second-contract="D-076" '
            'data-focal-ribbon="unit-flaked" data-value="1000" data-total="12000" data-share-percent="8.3">',
            _path("M 1120.00 765.00 L 1240.00 765.00 L 1315.00 705.00", "wire accent", **{"data-focal-leader": "unit-flaked"}),
            _text(850, 760, "FLAKED RERUNS", "mono", **{"data-focal-label": "title", "style": f"fill:{TOKENS['accent_text']};font-weight:750;letter-spacing:1.1px"}),
            _text(850, 792, "1,000 / 12,000 MIN · 8.3% OF BUDGET", "mono", **{"data-focal-label": "value", "style": "font-weight:650"}),
            "</g>",
        )
    )
