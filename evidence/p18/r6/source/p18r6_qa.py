...
from gallery_kernel import compartment_anchor, containment_anchor, dependency_anchor, deployment_anchor, experience_anchor, hierarchy_anchor, integration_anchor, matrix_anchor, quantitative_anchor, special_anchor, topology_anchor
...
def main() -> None:
    ...
    check(
        ...
        and connector_group.attrib.get("data-rank-step") == "240"
        and connector_group.attrib.get("data-inter-rank-gap") == "116"
        and connector_group.attrib.get("data-corridor-midpoint-step") == "240",
        "R6-D061-DEPENDENCY-BALANCED-RANKS",
        "four ranks use 240px steps, 116px node gaps and aligned corridor midpoints",
        results,
    )
    check(
        connector_group is not None
        and connector_group.attrib.get("data-lower-corridor-midpoint") == "760"
        and connector_group.attrib.get("data-lower-corridor-pitch") == "44",
        "R6-D061-DEPENDENCY-BALANCED-LOWER-LADDER",
        "bridged lower corridors are 738/782 around separator midpoint 760",
        results,
    )
    check(
    ...
    expected_bridge_points = {
        "dependency-types-utils": ("580.00", "738.00"),
        "dependency-types-zod": ("790.00", "782.00"),
    }
    ...
    expected_corridors = {
        "dependency-web-ui": ("309.00", "338.00"),
        "dependency-api-types": ("549.00", "578.00"),
        "dependency-ui-types": ("549.00", "578.00"),
        "dependency-types-tokens": ("738.00", "818.00"),
        "dependency-types-utils": ("738.00", "818.00"),
        "dependency-types-zod": ("782.00", "818.00"),
    }
    ...
    hierarchy_five_second = next(
        (item for item in hierarchy.iter(f"{SVG_NS}g") if item.attrib.get("data-five-second-contract") == "D-076"),
        None,
    )
    hierarchy_labels = ["".join(item.itertext()).strip() for item in hierarchy.iter(f"{SVG_NS}text")]
    check(
        hierarchy_five_second is not None
        and hierarchy_five_second.attrib.get("data-hierarchy-count-contract") == "1-4-5"
        and hierarchy_five_second.attrib.get("data-front-door-count") == "1"
        and hierarchy_five_second.attrib.get("data-domain-count") == "4"
        and hierarchy_five_second.attrib.get("data-specialist-pod-count") == "5"
        and all(label in hierarchy_labels for label in ("1 FRONT DOOR", "4 DOMAINS", "5 SPECIALIST PODS")),
        "R6-D076-HIERARCHY-VISIBLE-COUNT-CONTRACT",
        "diagram 09 visibly and structurally declares 1 front door, 4 domains and 5 specialist pods",
        results,
    )
    ...
    band_contracts = {
        ...
        "work-experience": {
            ...
            "members": 12,
            "minimum_clearance": 27.0,
        },
        "hierarchy": {
            "boundaries": (60.0, 300.0, 540.0, 780.0),
            "members": 10,
            "minimum_clearance": 58.0,
        },
    }
    for engine, expected in band_contracts.items():
        root = roots_by_engine[engine]
        contract = next(
            (item for item in root.iter(f"{SVG_NS}g") if item.attrib.get("data-band-layout-contract") == "D-073"),
            None,
        )
        boundaries = expected["boundaries"]
        separators = [] if contract is None else [
            item for item in contract.iter(f"{SVG_NS}line")
            if item.attrib.get("data-band-separator-index") is not None
        ]
        labels = [] if contract is None else [
            item for item in contract.iter(f"{SVG_NS}text")
            if item.attrib.get("data-band-label") is not None
        ]
        members = [item for item in root.iter() if item.attrib.get("data-band-member") is not None]
        declared_boundaries = tuple(
            float(value) for value in contract.attrib.get("data-band-boundaries", "").split()
        ) if contract is not None else ()
        check(
            contract is not None
            and contract.attrib.get("data-band-axis") == "y"
            and int(contract.attrib.get("data-band-count", "0")) == len(boundaries) - 1
            and declared_boundaries == boundaries,
            f"R6-D073-BAND-CONTRACT-{engine}",
            f"boundaries={declared_boundaries}; expected={boundaries}",
            results,
        )
        check(
            len(separators) == len(boundaries)
            and all(
                abs(float(item.attrib.get("y1", "nan")) - boundaries[index]) <= 0.01
                and abs(float(item.attrib.get("y2", "nan")) - boundaries[index]) <= 0.01
                and abs(float(item.attrib.get("data-separator-y", "nan")) - boundaries[index]) <= 0.01
                for index, item in enumerate(separators)
            ),
            f"R6-D073-SEPARATOR-GEOMETRY-{engine}",
            f"separator_count={len(separators)}; boundary_count={len(boundaries)}",
            results,
        )
        check(
            len(labels) == len(boundaries) - 1 and len(members) == expected["members"],
            f"R6-D073-BAND-MEMBER-COUNT-{engine}",
            f"labels={len(labels)} members={len(members)} expected_members={expected['members']}",
            results,
        )
        center_ok = True
        clearance_ok = True
        non_intersection_ok = True
        measured_clearances = []
        for member in members:
            box = geometry_box(member)
            index = int(member.attrib.get("data-band-index", "-1"))
            if not 0 <= index < len(boundaries) - 1:
                center_ok = clearance_ok = non_intersection_ok = False
                continue
            top, bottom = boundaries[index], boundaries[index + 1]
            expected_center = (top + bottom) / 2
            actual_center = box_center(box)[1]
            declared_center = float(member.attrib.get("data-band-center-y", "nan"))
            center_ok = center_ok and abs(actual_center - expected_center) <= 0.01 and abs(declared_center - expected_center) <= 0.01
            clearance = min(box[1] - top, bottom - (box[1] + box[3]))
            measured_clearances.append(clearance)
            clearance_ok = clearance_ok and clearance >= expected["minimum_clearance"] - 0.01
            non_intersection_ok = non_intersection_ok and all(
                separator < box[1] - 0.01 or separator > box[1] + box[3] + 0.01
                for separator in boundaries
            )
        check(center_ok, f"R6-D073-BAND-MEMBER-CENTER-{engine}", "every card/icon center equals its band midpoint", results)
        check(
            clearance_ok,
            f"R6-D073-BAND-SEPARATOR-CLEARANCE-{engine}",
            f"minimum measured clearance={min(measured_clearances) if measured_clearances else float('nan'):.2f}px; required>={expected['minimum_clearance']:.2f}px",
            results,
        )
        check(non_intersection_ok, f"R6-D073-SEPARATOR-NO-MEMBER-INTERSECTION-{engine}", "no separator crosses or sits behind a card/icon", results)

    regenerated_band_engines = {
        "dependency-dag": ET.fromstring(dependency_anchor().svg),
        "work-experience": ET.fromstring(experience_anchor().svg),
        "hierarchy": ET.fromstring(hierarchy_anchor().svg),
    }
    check(
        all(
            any(item.attrib.get("data-band-layout-contract") == "D-073" for item in root.iter(f"{SVG_NS}g"))
            for root in regenerated_band_engines.values()
        ),
        "R6-D073-DETERMINISTIC-BAND-BUILDERS",
        "all three source builders regenerate the D-073 band boundary and center-alignment contract",
        results,
    )
    ...
    quantitative_axes = [item for item in quantitative_root.iter(f"{SVG_NS}line") if "data-axis-id" in item.attrib]
    check(
        len(quantitative_axes) == 2
        and {item.attrib["data-axis-id"] for item in quantitative_axes} == {"automation", "control-confidence"}
        and all(item.attrib.get("data-origin-arrowhead") == "none" and item.attrib.get("data-remediation") == "D-074" for item in quantitative_axes),
        "R6-D074-QUANTITATIVE-AXIS-CONTRACT",
        "both scatter axes declare the D-074 no-origin-arrowhead contract",
        results,
    )
    check(
        all(not any(name in item.attrib for name in ("marker-start", "marker-mid", "marker-end")) for item in quantitative_axes),
        "R6-D074-QUANTITATIVE-AXIS-NO-MARKER",
        "neither scatter axis serializes an SVG marker at the shared origin or outer endpoint",
        results,
    )
    regenerated_quantitative = ET.fromstring(quantitative_anchor().svg)
    regenerated_axes = [item for item in regenerated_quantitative.iter(f"{SVG_NS}line") if "data-axis-id" in item.attrib]
    check(
        len(regenerated_axes) == 2
        and all(item.attrib.get("data-origin-arrowhead") == "none" and "marker-end" not in item.attrib for item in regenerated_axes),
        "R6-D074-DETERMINISTIC-QUANTITATIVE-AXIS",
        "source builder deterministically regenerates plain scatter axes without an origin arrowhead",
        results,
    )
    ...
    sankey_contract = next(
        (item for item in special_root.iter(f"{SVG_NS}g") if item.attrib.get("data-sankey-contract") == "D-072"),
        None,
    )
    sankey_nodes = {
    ...
    top_row_ids = ("budget", "unit", "passed")
    top_row_y_values = [geometry_box(sankey_nodes[node_id])[1] for node_id in top_row_ids]
    top_row_spread = max(top_row_y_values) - min(top_row_y_values)
    check(
        sankey_contract is not None
        and sankey_contract.attrib.get("data-inherited-contract") == "D-071"
        and sankey_contract.attrib.get("data-top-row-alignment") == "top"
        and sankey_contract.attrib.get("data-top-row-node-ids") == "budget unit passed"
        and all(sankey_nodes[node_id].attrib.get("data-top-row-member") == "true" for node_id in top_row_ids)
        and all(abs(value - 210.0) <= 0.01 for value in top_row_y_values)
        and top_row_spread <= 0.01,
        "R6-D072-SANKEY-TOP-ROW-ALIGNMENT",
        f"budget/unit/passed top-y={top_row_y_values}; spread={top_row_spread:.2f}px",
        results,
    )
    ...
    focal_ribbons_contract = next(
        (item for item in special_root.iter(f"{SVG_NS}g") if item.attrib.get("data-focal-contrast-policy") == "one-coral-high-opacity"),
        None,
    )
    focal_annotation = next(
        (item for item in special_root.iter(f"{SVG_NS}g") if item.attrib.get("data-focal-takeaway-annotation") == "true"),
        None,
    )
    focal_ribbon = next((item for item in sankey_ribbons if item.attrib.get("data-sankey-ribbon") == "unit-flaked"), None)
    non_focal_ribbons = [item for item in sankey_ribbons if item.attrib.get("data-sankey-ribbon") != "unit-flaked"]
    focal_text = " ".join("".join(item.itertext()).strip() for item in focal_annotation.iter(f"{SVG_NS}text")) if focal_annotation is not None else ""
    check(
        focal_ribbons_contract is not None
        and focal_ribbons_contract.attrib.get("data-five-second-contract") == "D-076"
        and focal_ribbons_contract.attrib.get("data-focal-ribbon") == "unit-flaked"
        and focal_ribbons_contract.attrib.get("data-focal-value") == "1000"
        and focal_ribbons_contract.attrib.get("data-total-value") == "12000"
        and focal_ribbons_contract.attrib.get("data-focal-share-percent") == "8.3"
        and focal_ribbon is not None
        and float(focal_ribbon.attrib.get("fill-opacity", "0")) >= 0.8
        and all(float(item.attrib.get("fill-opacity", "1")) <= 0.5 for item in non_focal_ribbons),
        "R6-D076-SANKEY-FOCAL-CONTRAST-CONTRACT",
        "only unit-flaked uses high-opacity coral while every non-focal ribbon remains at or below 0.50 opacity",
        results,
    )
    check(
        focal_annotation is not None
        and focal_annotation.attrib.get("data-five-second-contract") == "D-076"
        and focal_annotation.attrib.get("data-value") == "1000"
        and focal_annotation.attrib.get("data-total") == "12000"
        and focal_annotation.attrib.get("data-share-percent") == "8.3"
        and "FLAKED RERUNS" in focal_text
        and "1,000 / 12,000 MIN · 8.3% OF BUDGET" in focal_text,
        "R6-D076-SANKEY-DIRECT-TAKEAWAY",
        f"direct focal annotation={focal_text!r}",
        results,
    )
    regenerated_special = ET.fromstring(special_anchor().svg)
    check(
        any(item.attrib.get("data-sankey-contract") == "D-072" for item in regenerated_special.iter(f"{SVG_NS}g"))
        ...
        "source builder regenerates the D-072 top-aligned D-071-compatible Sankey",
        results,
    )

    review_14_archive = R6 / "history/review-14"
    review_14_manifest = review_14_archive / "P-18R6-MANIFEST.json"
    check(
        review_14_manifest.is_file()
        and sha256(review_14_manifest) == "9e88febc31f895aaada5385f2b9fc3a3384b8ff607831ac4ad9e302165b36637",
        "R6-D073-REVIEW-14-ARCHIVE-PIN",
        f"review-14 archived manifest sha256={sha256(review_14_manifest) if review_14_manifest.is_file() else 'missing'}",
        results,
    )
    target_stems = {
        "04-dependency-dag--neutral-light",
        "08-work-experience--neutral-light",
        "09-hierarchy--neutral-light",
    }
    later_review_stems = {"13-quantitative--neutral-light", "14-special-geometry--neutral-light"}
    pairwise_identity = []
    target_differences = []
    for path in sorted(ANCHORS.glob("*.svg")) + sorted(ANCHORS.glob("*.html")):
        archived = review_14_archive / "anchors" / path.name
        if path.stem in target_stems:
            target_differences.append(archived.is_file() and path.read_bytes() != archived.read_bytes())
        elif path.stem not in later_review_stems:
            pairwise_identity.append(archived.is_file() and path.read_bytes() == archived.read_bytes())
    check(
        len(pairwise_identity) == 18 and all(pairwise_identity)
        and len(target_differences) == 6 and all(target_differences),
        "R6-D073-TARGETED-ANCHOR-DELTA-ONLY",
        "9 unaffected HTML/SVG pairs remain byte-identical to review-14; diagram 04/08/09 pairs carry D-073 and diagrams 13/14 are reserved for later reviews",
        results,
    )

    review_15_archive = R6 / "history/review-15"
    review_15_manifest = review_15_archive / "P-18R6-MANIFEST.json"
    check(
        review_15_manifest.is_file()
        and sha256(review_15_manifest) == "0e2fcddc00a5b993fd34b4376c32e10a1ca0dd64013202e58ac35df801798a5b",
        "R6-D074-REVIEW-15-ARCHIVE-PIN",
        f"review-15 archived manifest sha256={sha256(review_15_manifest) if review_15_manifest.is_file() else 'missing'}",
        results,
    )
    review_16_target = {"13-quantitative--neutral-light"}
    review_15_identity = []
    review_16_differences = []
    for path in sorted(ANCHORS.glob("*.svg")) + sorted(ANCHORS.glob("*.html")):
        archived = review_15_archive / "anchors" / path.name
        if path.stem in review_16_target:
            review_16_differences.append(archived.is_file() and path.read_bytes() != archived.read_bytes())
        elif path.stem not in {"09-hierarchy--neutral-light", "14-special-geometry--neutral-light"}:
            review_15_identity.append(archived.is_file() and path.read_bytes() == archived.read_bytes())
    check(
        len(review_15_identity) == 22 and all(review_15_identity)
        and len(review_16_differences) == 2 and all(review_16_differences),
        "R6-D074-TARGETED-ANCHOR-DELTA-ONLY",
        "11 unaffected HTML/SVG pairs are byte-identical to review-15; diagram 13 differs under D-074 and diagrams 09/14 are reserved for D-076",
        results,
    )

    review_16_archive = R6 / "history/review-16"
    review_16_manifest = review_16_archive / "P-18R6-MANIFEST.json"
    check(
        review_16_manifest.is_file()
        and sha256(review_16_manifest) == "abdc0e9d7413b65f715c12a535b12abfaf33793e97f8f221e70a8d3ac58cc835",
        "R6-D076-REVIEW-16-ARCHIVE-PIN",
        f"review-16 archived manifest sha256={sha256(review_16_manifest) if review_16_manifest.is_file() else 'missing'}",
        results,
    )
    review_17_targets = {"09-hierarchy--neutral-light", "14-special-geometry--neutral-light"}
    review_16_identity = []
    review_17_differences = []
    for path in sorted(ANCHORS.glob("*.svg")) + sorted(ANCHORS.glob("*.html")):
        archived = review_16_archive / "anchors" / path.name
        if path.stem in review_17_targets:
            review_17_differences.append(archived.is_file() and path.read_bytes() != archived.read_bytes())
        else:
            review_16_identity.append(archived.is_file() and path.read_bytes() == archived.read_bytes())
    check(
        len(review_16_identity) == 24 and all(review_16_identity)
        and len(review_17_differences) == 4 and all(review_17_differences),
        "R6-D076-TARGETED-ANCHOR-DELTA-ONLY",
        "12 non-target HTML/SVG pairs are byte-identical to review-16; exactly diagram 09/14 pairs differ",
        results,
    )

    archived_hierarchy = ET.parse(review_16_archive / "anchors/09-hierarchy--neutral-light.svg").getroot()
    archived_special = ET.parse(review_16_archive / "anchors/14-special-geometry--neutral-light.svg").getroot()
    hierarchy_boxes = sorted(
        (item.attrib.get("data-node-id"), item.attrib.get("data-box-x"), item.attrib.get("data-box-y"), item.attrib.get("data-box-width"), item.attrib.get("data-box-height"))
        for item in hierarchy.iter(f"{SVG_NS}g") if item.attrib.get("data-node-id")
    )
    archived_hierarchy_boxes = sorted(
        (item.attrib.get("data-node-id"), item.attrib.get("data-box-x"), item.attrib.get("data-box-y"), item.attrib.get("data-box-width"), item.attrib.get("data-box-height"))
        for item in archived_hierarchy.iter(f"{SVG_NS}g") if item.attrib.get("data-node-id")
    )
    check(
        hierarchy_boxes == archived_hierarchy_boxes,
        "R6-D076-HIERARCHY-NODE-GEOMETRY-PRESERVED",
        "all hierarchy node boxes remain byte-equivalent at the geometry-attribute level",
        results,
    )
    archived_sankey_ribbons = {
        item.attrib["data-sankey-ribbon"]: (item.attrib.get("d"), item.attrib.get("data-value"), item.attrib.get("data-thickness"))
        for item in archived_special.iter(f"{SVG_NS}path") if item.attrib.get("data-sankey-ribbon")
    }
    current_sankey_ribbons = {
        item.attrib["data-sankey-ribbon"]: (item.attrib.get("d"), item.attrib.get("data-value"), item.attrib.get("data-thickness"))
        for item in sankey_ribbons
    }
    check(
        current_sankey_ribbons == archived_sankey_ribbons,
        "R6-D076-SANKEY-QUANTITATIVE-GEOMETRY-PRESERVED",
        "all nine Sankey paths retain exact d/value/thickness geometry from review-16",
        results,
    )
