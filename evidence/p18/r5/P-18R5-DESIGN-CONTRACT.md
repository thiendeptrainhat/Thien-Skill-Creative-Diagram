...
5. Rounded orthogonal connectors start/end on allocated boundary ports, route through stage corridors and receive label masks. At an unavoidable crossing, the horizontal route owns one true bridge/hop integrated into that route's path geometry: the straight chord is replaced by two open cubic shoulders. The route and repaint layer use the same exact path generator; the wide underlay is restricted to an exact central crown subcurve and cannot touch either shoulder join. It does not draw a circle, bubble, junction dot or closed loop. Parallel vertical corridors keep at least 56px separation; adjacent hops on one segment require pitch of at least `r1 + r2 + 12px`, otherwise the router must choose another corridor.
...
## Owner review-02 remediation lock

D-056 records review-02 as historical because the remaining crossing treatment still read as a compound wave and the apparent chord removal relied on a mask/overlay rather than the route's own geometry. Review-03 therefore:

- retires the gap-mask simulation and serializes every accepted hop into the connector path itself;
- prevents two near-parallel vertical corridors from collapsing into one visual junction by enforcing a 56px routing separation;
- prevents adjacent same-segment hops from forming an `m` silhouette by enforcing `r1 + r2 + 12px` minimum pitch and rerouting when it is unavailable; and
- retains the accepted font, measured adaptive card widths, one-line target titles, node anatomy, palette and semantic content unchanged.

This remains inside the authorized one-anchor R5 scope and does not authorize expansion to other families.

## Owner review-03 remediation lock

D-057 preserves review-03 as historical evidence after the owner accepted its crossing silhouette but identified a hairline gap at each straight-to-arc join. Review-04 therefore:

- uses one canonical cubic geometry for the route-integrated hop and visible repaint, eliminating control-point drift;
- masks only the central crown subcurve needed to cover the underpass, leaving both joins untouched by canvas-colored paint;
- verifies exact route/repaint path identity and a minimum crown-to-join clearance in static and browser QA; and
- preserves all accepted routing corridors, typography, adaptive card widths, semantics, palette and composition.
...
- Every detected crossing has one path-integrated open hop plus its non-filled underlay/repaint pair; there is no gap-mask element, underlying straight chord, bubble/junction/port dot or unsupported bridge orientation.
- Adjacent hops on the same route segment meet `r1 + r2 + 12px` minimum pitch; parallel vertical corridors meet the 56px separation rule instead of producing a compound wave.
- Route and repaint contain the same exact hop cubic commands; the central-crown underlay remains inset from both joins, so no canvas-colored stroke can create a shoulder-to-hop hairline gap.
