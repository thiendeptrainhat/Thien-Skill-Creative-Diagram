...
class OrthogonalRouter:
    ...
    def __init__(
        ...
        label_clearance: float = 10.0,
        parallel_corridor_separation: float = 56.0,
    ) -> None:
        self.clearance = clearance
        self.label_clearance = label_clearance
        self.parallel_corridor_separation = parallel_corridor_separation
    ...
    def route(self, layout: LaneLayout, edges: Sequence[EdgeSpec], label_widths: Mapping[str, float]) -> tuple[RoutedEdge, ...]:
        ...
        for edge in edges:
            ...
            for corridor_x in candidates:
                ...
                if self._clear(points, edge, obstacles) and self._parallel_corridors_clear(points, routed):
                    selected = points
                    break
            if selected is None:
    ...
    def _corridor_candidates(self, layout: LaneLayout, source_x: float, target_x: float) -> tuple[float, ...]:
        ...
        preferred = tuple(
            source_x * (1 - fraction) + target_x * fraction
            for fraction in (0.65, 0.50, 0.35, 0.75, 0.25)
        )
    ...
    def _parallel_corridors_clear(
        self,
        candidate: Sequence[Point],
        routed: Sequence[RoutedEdge],
    ) -> bool:
        """Keep overlapping vertical corridors far enough apart for legible hops."""

        candidate_verticals = self._vertical_segments(candidate)
        for previous in routed:
            for candidate_x, candidate_low, candidate_high in candidate_verticals:
                for previous_x, previous_low, previous_high in self._vertical_segments(previous.points):
                    overlap = min(candidate_high, previous_high) - max(candidate_low, previous_low)
                    if overlap <= 20:
                        continue
                    if abs(candidate_x - previous_x) < self.parallel_corridor_separation:
                        return False
        return True

    @staticmethod
    def _vertical_segments(points: Sequence[Point]) -> tuple[tuple[float, float, float], ...]:
        return tuple(
            (start.x, min(start.y, end.y), max(start.y, end.y))
...
def _split_cubic(
    ...
    p0, p1, p2, p3 = curve
    a = _lerp_point(p0, p1, amount)
    b = _lerp_point(p1, p2, amount)
    c = _lerp_point(p2, p3, amount)
    d = _lerp_point(a, b, amount)
    e = _lerp_point(b, c, amount)
    midpoint = _lerp_point(d, e, amount)
    return (p0, a, d, midpoint), (midpoint, e, c, p3)
...
def _cubic_subcurve(
    ...
    if not 0 <= start < end <= 1:
        raise ValueError("Cubic subcurve range must satisfy 0 <= start < end <= 1")
    if start == 0:
        left, _ = _split_cubic(curve, end)
        return left
    _, right = _split_cubic(curve, start)
    relative_end = (end - start) / (1 - start)
    left, _ = _split_cubic(right, relative_end)
    return left


def _open_hop_curves(
    bridge: BridgeMark,
    direction: float,
) -> tuple[tuple[Point, Point, Point, Point], tuple[Point, Point, Point, Point]]:
    if bridge.orientation != "horizontal":
        raise ValueError(f"Unsupported bridge orientation: {bridge.orientation}")
    direction = 1.0 if direction >= 0 else -1.0
    radius = bridge.radius
    center = bridge.point
    start = Point(center.x - direction * radius, center.y)
    apex = Point(center.x, center.y - radius)
    end = Point(center.x + direction * radius, center.y)
    first = (
        start,
        Point(start.x + direction * radius * 0.42, center.y - radius),
        Point(center.x - direction * radius * 0.26, center.y - radius),
        apex,
    )
    second = (
        apex,
        Point(center.x + direction * radius * 0.26, center.y - radius),
        Point(end.x - direction * radius * 0.42, center.y - radius),
        end,
    )
    return first, second


def _cubic_command(curve: tuple[Point, Point, Point, Point]) -> str:
    _, control_one, control_two, end = curve
    return (
        f"C {control_one.x:.2f} {control_one.y:.2f} "
        f"{control_two.x:.2f} {control_two.y:.2f} "
        f"{end.x:.2f} {end.y:.2f}"
    )


def bridge_open_hop_path(bridge: BridgeMark, direction: float = 1.0) -> str:
    """Return the exact open-hop geometry shared by the route and repaint pass."""

    first, second = _open_hop_curves(bridge, direction)
    start = first[0]
    return f"M {start.x:.2f} {start.y:.2f} {_cubic_command(first)} {_cubic_command(second)}"


def bridge_crown_underlay_path(bridge: BridgeMark, direction: float = 1.0) -> str:
    """Return an exact central subcurve that masks the underpass, not the joins."""

    first, second = _open_hop_curves(bridge, direction)
    crown_left = _cubic_subcurve(first, 0.62, 1.0)
    crown_right = _cubic_subcurve(second, 0.0, 0.38)
    start = crown_left[0]
    return (
        f"M {start.x:.2f} {start.y:.2f} "
        f"{_cubic_command(crown_left)} {_cubic_command(crown_right)}"
    )


def bridged_orthogonal_path(
    points: Sequence[Point],
    bridges: Sequence[BridgeMark],
    corner_radius: float = 14.0,
) -> str:
    """Serialize a continuous orthogonal path whose horizontal hops replace chords."""

    if len(points) < 2:
        raise ValueError("A route needs at least two points")
    if not bridges:
        return rounded_orthogonal_path(points, corner_radius)

    before_corner: dict[int, Point] = {}
    after_corner: dict[int, Point] = {}
    for index in range(1, len(points) - 1):
        previous, current, following = points[index - 1], points[index], points[index + 1]
        incoming = hypot(current.x - previous.x, current.y - previous.y)
        outgoing = hypot(following.x - current.x, following.y - current.y)
        corner = min(corner_radius, incoming / 2, outgoing / 2)
        before_corner[index] = Point(
            current.x + (previous.x - current.x) / incoming * corner,
            current.y + (previous.y - current.y) / incoming * corner,
        )
        after_corner[index] = Point(
            current.x + (following.x - current.x) / outgoing * corner,
            current.y + (following.y - current.y) / outgoing * corner,
        )

    by_segment: dict[int, list[BridgeMark]] = {}
    for bridge in bridges:
        if bridge.orientation != "horizontal":
            raise ValueError(f"Unsupported bridge orientation: {bridge.orientation}")
        by_segment.setdefault(bridge.segment_index, []).append(bridge)

    commands = [f"M {points[0].x:.2f} {points[0].y:.2f}"]
    last_segment = len(points) - 2
    for segment_index in range(len(points) - 1):
        start = points[0] if segment_index == 0 else after_corner[segment_index]
        end = points[-1] if segment_index == last_segment else before_corner[segment_index + 1]
        marks = by_segment.get(segment_index, [])
        horizontal = abs(start.y - end.y) < 0.001
        if marks and not horizontal:
            raise ValueError("Bridge mark is not attached to a horizontal rendered segment")
        if horizontal and marks:
            direction = 1.0 if end.x >= start.x else -1.0
            ordered_marks = sorted(marks, key=lambda mark: mark.point.x * direction)
            previous_end = start.x
            for bridge in ordered_marks:
                first, second = _open_hop_curves(bridge, direction)
                hop_start_x = first[0].x
                hop_end_x = second[-1].x
                if direction * (hop_start_x - previous_end) <= 0 or direction * (end.x - hop_end_x) <= 0:
                    raise ValueError("Bridge lacks straight lead-in/lead-out clearance")
                commands.append(f"L {hop_start_x:.2f} {bridge.point.y:.2f}")
                commands.append(_cubic_command(first))
                commands.append(_cubic_command(second))
                previous_end = hop_end_x
            commands.append(f"L {end.x:.2f} {end.y:.2f}")
        else:
            commands.append(f"L {end.x:.2f} {end.y:.2f}")

        if segment_index < last_segment:
            corner_index = segment_index + 1
            corner = points[corner_index]
            after = after_corner[corner_index]
            commands.append(f"Q {corner.x:.2f} {corner.y:.2f} {after.x:.2f} {after.y:.2f}")
    return " ".join(commands)
...
__all__ = [
    ...
    "StageSpec",
    "TypographyRequest",
    "bridge_crown_underlay_path",
    "bridge_open_hop_path",
    "default_font_catalog",
    "default_font_roles",
    "bridged_orthogonal_path",
