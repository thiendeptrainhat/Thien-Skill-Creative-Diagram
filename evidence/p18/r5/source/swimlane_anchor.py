...
def _route_path_svg(route: RoutedEdge) -> str:
    ...
    return (
        ...
        f'data-path-bridges-integrated="true" '
        f'd="{bridged_orthogonal_path(route.points, route.bridges)}" marker-end="{marker}"/>'
    )


def _bridge_svg(route: RoutedEdge) -> str:
    parts: list[str] = []
    color_class = " critical" if route.spec.critical else ""
    for bridge in route.bridges:
        if bridge.orientation != "horizontal":
            raise RuntimeError(f"Unsupported bridge orientation: {bridge.orientation}")
        x = bridge.point.x
        y = bridge.point.y
        ...
        arc = bridge_open_hop_path(bridge, direction)
        crown = bridge_crown_underlay_path(bridge, direction)
        parts.extend(
            [
                f'<g class="bridge-mark{color_class}" data-bridge-edge="{escape(route.spec.edge_id)}" '
                f'data-bridge-segment="{bridge.segment_index}" data-bridge-orientation="horizontal" '
                f'data-bridge-x="{x:.2f}" data-bridge-y="{y:.2f}" data-bridge-radius="{radius:.2f}" '
                f'data-hop-geometry-shared="true" data-underlay-scope="central-crown">',
                f'<path class="bridge-hop-underlay" data-bridge-role="underlay" d="{crown}"/>',
                f'<path class="bridge-hop" data-bridge-role="hop" d="{arc}"/>',
                "</g>",
            ]
        )
    return "".join(parts)
...
def _route_annotation_svg(route: RoutedEdge) -> str:
...
def render_svg(model: AnchorModel) -> str:
    ...
    css = f"""
      ...
      .bridge-mark{{pointer-events:none}}
      .bridge-hop-underlay{{fill:none;stroke:{TOKENS['canvas']};stroke-width:11;stroke-linecap:round;stroke-linejoin:round}}
      .bridge-hop{{fill:none;stroke:{TOKENS['connector']};stroke-width:3;stroke-linecap:round;stroke-linejoin:round}}
      .bridge-mark.critical .bridge-hop{{stroke:{TOKENS['accent']};stroke-width:3.6}}
    ...
    # Hop curves are already integrated into each base path.  A global
    # underlay/repaint pass keeps every hop visually above later underpasses
    # without using a mask to hide a straight chord.
    parts.extend(_route_path_svg(route) for route in model.routes)
    parts.extend(_bridge_svg(route) for route in model.routes)
    parts.extend(_route_annotation_svg(route) for route in model.routes)
