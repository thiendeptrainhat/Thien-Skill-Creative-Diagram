...
def main() -> None:
    ...
    bridge_pitch_detail: list[dict[str, object]] = []
    integrated_path_detail: dict[str, object] = {}
    ...
    for route in model.routes:
        rendered_path = bridged_orthogonal_path(route.points, route.bridges)
        cubic_count = rendered_path.count("C ")
        shared_geometry: list[dict[str, object]] = []
        for bridge in route.bridges:
            segment_start = route.points[bridge.segment_index]
            segment_end = route.points[bridge.segment_index + 1]
            direction = 1.0 if segment_end.x >= segment_start.x else -1.0
            hop_path = bridge_open_hop_path(bridge, direction)
            crown_path = bridge_crown_underlay_path(bridge, direction)
            route_fragment = hop_path.replace("M ", "L ", 1)
            hop_markup = f'data-bridge-role="hop" d="{hop_path}"'
            crown_markup = f'data-bridge-role="underlay" d="{crown_path}"'
            shared_ok = (
                route_fragment in rendered_path
                and hop_markup in svg_text
                and crown_markup in svg_text
                and crown_path != hop_path
            )
            shared_geometry.append({
                "segment_index": bridge.segment_index,
                "route_fragment_shared": route_fragment in rendered_path,
                "repaint_exact": hop_markup in svg_text,
                "underlay_crown_only": crown_markup in svg_text and crown_path != hop_path,
            })
            if not shared_ok:
                bridge_geometry_failures.append(f"{route.spec.edge_id}:shoulder-hop-continuity")
        integrated_path_detail[route.spec.edge_id] = {
            "bridge_count": len(route.bridges),
            "cubic_count": cubic_count,
            "shared_geometry": shared_geometry,
        }
        if route.bridges and cubic_count < len(route.bridges) * 2:
            bridge_geometry_failures.append(f"{route.spec.edge_id}:hop-not-integrated-in-route-path")

        by_segment: dict[int, list[BridgeMark]] = {}
        for bridge in route.bridges:
            by_segment.setdefault(bridge.segment_index, []).append(bridge)
        for segment_index, marks in by_segment.items():
            ordered = sorted(marks, key=lambda mark: mark.point.x)
            for left, right in zip(ordered, ordered[1:]):
                actual_pitch = right.point.x - left.point.x
                required_pitch = left.radius + right.radius + 12
                bridge_pitch_detail.append({
                    "edge_id": route.spec.edge_id,
                    "segment_index": segment_index,
                    "actual_px": round(actual_pitch, 2),
                    "required_px": round(required_pitch, 2),
                })
                if actual_pitch + 0.001 < required_pitch:
                    bridge_geometry_failures.append(
                        f"{route.spec.edge_id}:segment-{segment_index}:compound-hop:{actual_pitch:.2f}<{required_pitch:.2f}"
                    )
    bridge_markup = {
        ...
        "integrated_route_paths": svg_text.count('data-path-bridges-integrated="true"'),
        "shared_geometry_marks": svg_text.count('data-hop-geometry-shared="true"'),
        "crown_underlays": svg_text.count('data-underlay-scope="central-crown"'),
    }
    bridge_renderer_ok = (
        len(actual_bridges) > 0
        and bridge_markup["groups"] == len(actual_bridges)
        and bridge_markup["gaps"] == 0
        and bridge_markup["underlays"] == len(actual_bridges)
        and bridge_markup["hops"] == len(actual_bridges)
        and bridge_markup["port_dots"] == 0
        and bridge_markup["integrated_route_paths"] == len(model.routes)
        and bridge_markup["shared_geometry_marks"] == len(actual_bridges)
        and bridge_markup["crown_underlays"] == len(actual_bridges)
        and ".bridge-hop{fill:none" in svg_text
        and ".bridge-hop-underlay{fill:none" in svg_text
        and ".bridge-gap" not in svg_text
    ...
    results.append(check(
        ...
        bridge_renderer_ok,
        {
            "markup": bridge_markup,
            "geometry_failures": bridge_geometry_failures,
            "same_segment_pitch": bridge_pitch_detail or "no adjacent hops share a segment",
            "integrated_paths": integrated_path_detail,
        },
    ))
