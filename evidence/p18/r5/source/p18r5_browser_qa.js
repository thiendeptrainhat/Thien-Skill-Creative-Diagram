  ...
  for (const viewport of viewports) {
    ...
    const metrics = await page.evaluate(() => {
      ...
      for (const group of bridgeGroups) {
        ...
        if (group.dataset.hopGeometryShared !== "true") failures.push("bridge-geometry-not-shared");
        if (group.dataset.underlayScope !== "central-crown") failures.push("bridge-underlay-not-crown");
      ...
      let minimumJoinClearance = null;
      for (const group of bridgeGroups) {
        const route = svg.querySelector(`[data-edge-id='${group.dataset.bridgeEdge}']`);
        const hop = group.querySelector("[data-bridge-role='hop']");
        const underlay = group.querySelector("[data-bridge-role='underlay']");
        const hopPath = hop.getAttribute("d");
        const routeFragment = hopPath.replace(/^M /, "L ");
        if (!route.getAttribute("d").includes(routeFragment)) failures.push(`bridge-geometry-drift:${group.dataset.bridgeEdge}`);
        const hopStart = hop.getPointAtLength(0);
        const hopEnd = hop.getPointAtLength(hop.getTotalLength());
        const underlayStart = underlay.getPointAtLength(0);
        const underlayEnd = underlay.getPointAtLength(underlay.getTotalLength());
        const startClearance = Math.hypot(underlayStart.x - hopStart.x, underlayStart.y - hopStart.y);
        const endClearance = Math.hypot(underlayEnd.x - hopEnd.x, underlayEnd.y - hopEnd.y);
        const joinClearance = Math.min(startClearance, endClearance);
        const requiredClearance = Number(group.dataset.bridgeRadius) * 0.35;
        minimumJoinClearance = minimumJoinClearance === null ? joinClearance : Math.min(minimumJoinClearance, joinClearance);
        if (joinClearance + 0.001 < requiredClearance) failures.push(`bridge-underlay-touches-join:${group.dataset.bridgeEdge}`);
      }
