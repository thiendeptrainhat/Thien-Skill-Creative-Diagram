...
**Authority:** D-051, D-052, D-054, D-055, D-056  
...
5. Rounded orthogonal connectors start/end on allocated boundary ports, route through stage corridors and receive label masks. At an unavoidable crossing, the horizontal route owns one true bridge/hop integrated into that route's path geometry: the straight chord is replaced by two open cubic shoulders before the overlay paint pass. It does not draw a circle, bubble, junction dot or closed loop. Parallel vertical corridors keep at least 56px separation; adjacent hops on one segment require pitch of at least `r1 + r2 + 12px`, otherwise the router must choose another corridor.
...
This is the final crossing-specific remediation inside the authorized one-anchor R5 scope. It does not authorize expansion to other families.
