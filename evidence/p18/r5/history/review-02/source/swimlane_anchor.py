    ...
    for bridge in route.bridges:
        ...
        parts.extend(
            [
                ...
                f'<path class="bridge-gap" data-bridge-role="gap" '
                f'd="M {x - radius + 1:.2f} {y:.2f} L {x + radius - 1:.2f} {y:.2f}"/>',
...
def render_svg(model: AnchorModel) -> str:
    ...
    css = f"""
      ...
      .bridge-gap{{fill:none;stroke:{TOKENS['canvas']};stroke-width:10;stroke-linecap:butt}}
    ...
    # Global paint order is part of the crossing grammar: base routes first,
    # then every bridge mask/hop, then annotations.  A later base route can no
    # longer repaint a bridge into a bubble-like junction.
