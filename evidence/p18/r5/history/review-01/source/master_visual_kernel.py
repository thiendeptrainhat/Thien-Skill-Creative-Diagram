...
class IntrinsicNodeSizer:
    ...
    def measure(self, content: NodeContent) -> tuple[float, float, tuple[str, ...], tuple[float, ...]]:
        padding_x = 20.0
        min_width = 292.0
        max_width = 352.0
        ...
        natural_width = max(badge_width, transition_width, system_width + tag_total + 18) + padding_x * 2
        width = min(max_width, max(min_width, natural_width))
        title = self.metrics.wrap("node_title", content.title, width - padding_x * 2)
...
class LaneInteractionEngine:
    ...
    def layout(
        ...
        maximum_node_height = max(item[1] for item in measured.values())
        maximum_node_width = max(item[0] for item in measured.values())
        safe = 52.0
        rail_height = 138.0
        ...
        stage_centers = {stage.index: first_center + stage.index * stage_stride for stage in stages}
        natural_width = max(stage_centers.values()) + maximum_node_width / 2 + safe
        ...
        for content in contents:
            ...
            y = lane_top + lane_index[content.lane_id] * lane_height + (lane_height - height_value) / 2
            nodes.append(
                NodeGeometry(
                    content=content,
            ...
            for right in edges[left_index + 1:]:
                for a1, a2 in zip(left.points, left.points[1:]):
                    for b1, b2 in zip(right.points, right.points[1:]):
                        ...
                        x = vertical[0].x
                        y = horizontal[0].y
                        if hx1 + 10 < x < hx2 - 10 and vy1 + 10 < y < vy2 - 10:
                            owner = right if left_horizontal else left
                            bridge_map[owner.spec.edge_id].append(BridgeMark(Point(x, y), "vertical"))
        return tuple(
            RoutedEdge(
                spec=edge.spec,
                source_port=edge.source_port,
                ...
                bridges=tuple(bridge_map[edge.spec.edge_id]),
            )
            for edge in edges
        )
